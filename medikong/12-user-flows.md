...
이 섹션은 설계 확정 전 작업 분배를 위해 사용자 관점에서 가능한 큰 흐름을 먼저 나열한다. 서비스 경계, API, 데이터 모델, 동시성 처리 방식은 각 시나리오 오너가 후속 설계에서 제안하고 리뷰한다.
...
| 정상 구매 시나리오 오너 | 사용자는 한정 상품을 발견하고 정상적으로 구매 완료까지 갈 수 있는가? | 드롭 발견, 상세 확인, 오픈 대기, 구매 시도, 결제, 구매 결과 확인, 알림 확인 |
...
| 선착순 쿠폰 시나리오 오너 | 사용자는 한정 수량 쿠폰을 받고 구매에 사용할 수 있는가? | 쿠폰 발견, 발급 시도, 발급 성공 또는 소진 안내, 쿠폰함 확인, 구매 적용 |
