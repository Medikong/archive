...
`09`는 Tempo indexing을 기다리는 bounded polling에서 검색 request와 그 request의 assertion을 반복할 수 있으므로 정확한 합계가 실행마다 달라질 수 있다. 불변 통과 기준은 failures 0, distinct-root assertion 통과, 두 root에 걸친 6개 필수 pair assertion 통과다. 전체 구매 여정은 단일 trace로 판정하지 않는다. Kafka lag 검증은 향후 범위다.

2026-07-13에는 `task tests:purchase-e2e-with-log-correlation`로 Loki 기반 로그 연결을 추가 검증했다. 정상 구매의 `order.created`, `payment.approved`, `notification.requested` producer/consumer 6개 로그가 동일 `orderId` correlation을 사용했고, 각 로그의 `trace_id`와 `span_id`가 비어 있지 않았다. HTTP 주문·결제 로그는 각각 `request_id`, 같은 값의 `correlation_id`, trace/span ID로 조회됐다. 실행 후 container와 volume은 각각 0이었다.
...
| observability | Kafka lag 확인 | Kafka span graph와 Loki log correlation은 확인했으며 Kafka lag 자동 판정은 향후 범위다. |
