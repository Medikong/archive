...
| --- | --- | --- | --- |
| `order.created` | `order-service` | observability, optional payment experiment | 주문 생성 사실 |
| `payment.approved` | `payment-service` | `order-service` | 결제 승인 |
| `payment.failed` | `payment-service` | `order-service` | 결제 실패 |
| `payment.delayed` | `payment-service` | `order-service`, recovery | 결제 지연 |
| `order.confirmed` | `order-service` | `notification-service`, catalog projection optional | 주문 확정 |
| `order.cancelled` | `order-service` | `notification-service` | 주문 취소 |
| `order.reservation.expired` | `order-service` | `notification-service`, recovery | 예약 만료 |
...
Gateway가 JWT claim을 내부 헤더로 복사하는 경우 다음 이름을 기본값으로 사용한다.

| 내부 헤더 | JWT claim | 설명 |
| --- | --- | --- |
| `X-User-Id` | `sub` | 서비스 내부 사용자 식별자 |
| `X-User-Email` | `email` | 감사 로그와 운영 조회용 |
| `X-User-Role` | `role` | customer/admin 권한 분기 |
...
| order | `PENDING_PAYMENT`, `CONFIRMED`, `CANCELLED`, `EXPIRED` |
...
동적 ID는 metric label로 사용하지 않는다. `order_id`, `payment_id`, `customer_id`는 log와 trace에는 남길 수 있지만 Prometheus label에는 넣지 않는다.
