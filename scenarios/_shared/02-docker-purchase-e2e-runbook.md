...
검증 후 cleanup exit 0과 잔존 container 0 / volume 0을 확인했다. Log correlation은 `task tests:purchase-e2e-with-log-correlation`로 자동화됐고 Kafka lag는 다음 자동화 대상이다.
...
다음 단계는 Kafka lag와 notification 업무 metric까지 자동 판정하는 것이다.
