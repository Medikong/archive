...
이 명령은 UUID로 격리한 깨끗한 clone을 만들고 Git 추적 파일만 사용하는 context에서 stack과 smoke image를 새로 빌드한다. bind mount는 사용하지 않는다. non-root Alloy가 읽기 전용 Docker socket proxy를 통해 현재 Compose project의 JSON stdout만 Loki로 전달하며, 정상 구매와 결제 실패의 HTTP/Kafka trace 연결, bounded failure code, 중복 Loki record 부재, 민감 필드 부재를 판정한 뒤 container, volume, network, image와 임시 context를 정리한다.

결제 실패 중복 요청과 Kafka 재전달 멱등성을 확인할 때는 다음 명령을 사용한다.

```bash
task payment-failure-idempotency
```

이 명령은 `Python` runner와 Git 추적 파일만 복사한 깨끗한 임시 context를 사용한다. 결제 1행, 처리 이벤트 1행, Kafka 3건의 단일 `eventId`, 주문 `PAYMENT_FAILED`를 판정하고 모든 임시 자원을 정리한다.

Gateway를 제외한 내부 구매 회귀 전체를 한 번에 확인할 때는 다음 명령을 사용한다.

```bash
task purchase-internal-regression
```

이 통합 실행은 committed `HEAD`를 UUID 기반의 깨끗한 clone으로 격리하고 unit, purchase metric, 실제 concurrency, 결제 실패 중복 멱등성, HTTP trace, Kafka trace, Loki correlation, notification metric/lag의 8개 gate를 순서대로 실행한다. 각 gate는 고유 Compose project와 loopback 또는 비공개 포트를 사용한다.
...
| `payment-failure-idempotency` | HTTP 중복 요청과 Kafka 재전달 통과 | 결제 1행, 처리 이벤트 1행, Kafka 3건의 `eventId` 1개, 주문 `PAYMENT_FAILED` |
