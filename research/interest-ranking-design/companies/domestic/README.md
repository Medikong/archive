...
| Toss | 분석 | 대규모 트래픽 캐싱 전략(Universal/User-Specific 분리), 카운터 로컬 집계 후 flush | [toss/README.md](toss/README.md) |
| 쿠팡 | 분석 | 마이크로서비스 데이터 통합 서빙, 다층 캐시(read-through+실시간), 클러스터 분리 | [coupang/README.md](coupang/README.md) |
...
- [Toss](https://toss.tech/article/27600)는 대규모 트래픽을 서버 증설 없이 처리한 캐싱 전략(공통 데이터는 로컬 캐시+Pub/Sub, 개인 데이터는 압축된 Redis, 카운터는 로컬 집계 후 배치 flush)을 공개했다. interest-service의 찜/조회 카운터 설계에 직접 참고할 수 있는 구체적인 사례다.
- [쿠팡](https://medium.com/coupang-engineering/%EB%8C%80%EC%9A%A9%EB%9F%89-%ED%8A%B8%EB%9E%98%ED%94%BD-%EC%B2%98%EB%A6%AC%EB%A5%BC-%EC%9C%84%ED%95%9C-%EC%BF%A0%ED%8C%A1%EC%9D%98-%EB%B0%B1%EC%97%94%EB%93%9C-%EC%A0%84%EB%9E%B5-184f7fdb1367)은 여러 마이크로서비스 데이터를 통합해 read-heavy 트래픽을 서빙하는 다층 캐시 전략(read-through 캐시 + 실시간 캐시)을 보여준다.
