...
아래 SLO 요구치는 `half-year early-growth` API 통합테스트 벤치마크의 large dataset 결과를 하한 근거로 사용한다. 서비스별 benchmark 문서는 아카이브 evidence 범위에서 제외한다. API 통합 benchmark는 in-process 또는 testcontainer 중심의 API 1회 처리 비용이므로, capacity baseline의 k6/Kubernetes 경로에서는 Kong ingress, LB, network hop, database latency, connection pool 대기, metrics window 변동을 포함해 더 보수적인 p95 gate를 둔다.
...
실제 예매 과정의 주요 API는 기존 concert-service API 설계의 `모바일 홈 -> 공연 상세 -> 달력 -> 날짜별 회차 -> 좌석도 -> 예매 생성 -> 결제 -> 티켓 발급` 순서를 따른다.
