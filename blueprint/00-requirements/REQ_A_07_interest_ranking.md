...
| 조회수 중복 집계 방지 구현 패턴(Redis `SETNX`/`EX` 기반) | 동일 사용자의 반복 조회를 짧은 TTL 동안 하나의 키로 막아, 새로고침만으로 조회수가 늘어나지 않게 하는 방식이 일반적으로 쓰인다. | (2026-07-14 대체) 쓰기 시점 dedup(Redis) 대신, 조회를 그대로 다 기록하고 집계 시점에 `COUNT(DISTINCT user_id)`로 세는 방식을 택했다 — 같은 효과를 Redis 없이 얻는다. |
...
| `REQ.A.07.PP-003` | 조회수를 원시 트래픽 그대로 집계하면 같은 사용자의 새로고침만으로 쉽게 부풀려진다. | (2026-07-14 대체) Redis dedup 대신, 원문 기록 + 집계 시점 `COUNT(DISTINCT user_id)`로 같은 효과를 얻는다. | `REQ.A.07.FR-004` | `REQ.A.07.NFR-001` | [조회수 중복 방지 구현 패턴](https://velog.io/@horang12/%EB%8F%99%EC%9D%BC%ED%95%9C-%EC%9C%A0%EC%A0%80%EC%9D%98-%EC%A1%B0%ED%9A%8C%EC%88%98%EA%B0%80-%EC%A4%91%EB%B3%B5-%EC%A7%91%EA%B3%84-%EB%90%98%EC%A7%80-%EC%95%8A%EA%B2%8C-%ED%95%98%EB%A0%A4%EB%A9%B4) |
...
| `REQ.A.07.PP-006` | 속도/감쇠 기반 "급상승" 랭킹은 순위가 너무 자주 바뀌어 사용자 신뢰를 잃기 쉽고, 오픈 전 드롭은 애초에 급상승 판정에 필요한 신호 밀도가 부족해 비효율적이다. | 오픈 전 랭킹은 감쇠 계산 없이 **리셋 없는 누적 활성 찜 수**로 단순화한다(2026-07-14 수정, 아래 "2026-07-14 수정 이력" 참고). 실제로 폭발적인 관심을 받는 드롭은 별도 감지 로직 없이도 누적 카운트만으로 상위권에 오른다. | `REQ.A.07.FR-003` | `REQ.A.07.NFR-006` | [네이버/다음 실시간 급상승 검색어 알고리즘](https://www.clien.net/service/board/park/5599879) |
...
| `REQ.A.07.FR-003` | 시스템은 카탈로그 드롭 상태 구분 없이(2026-07-14: catalog-service 의존 제거, 아래 수정 이력 참고), 드롭별 **누적 활성 찜 수**(리셋 없음) 내림차순으로 오픈 전 랭킹을 산출한다 | 구매자, DropMong 운영자 | Must | [PAGE.A.01](../10-sitemap/buyer-mobile-web/PAGE_A_01_homepage.md) |
| `REQ.A.07.FR-004` | 시스템은 로그인 사용자의 드롭 조회를 기록하되, 같은 사용자의 반복 조회가 인기 신호를 부풀리지 않게 한다 (2026-07-14 재설계: Redis `SETNX` dedup 대신, 조회를 매번 원문 기록하고 집계 시점에 `COUNT(DISTINCT user_id)`로 세어 같은 사용자의 반복 조회를 자연스럽게 1명으로 처리한다 — Redis 불필요) | - | Must | - |
| `REQ.A.07.FR-011` | 시스템은 드롭 상태 구분 없이, KST 3시간 고정 구간(00/03/06/09/12/15/18/21시 경계)마다 직전 구간의 서로 다른 조회자 수를 배치로 집계해 "실시간 많이 보고 있는 상품" 랭킹을 산출한다 | 구매자, DropMong 운영자 | Must | [PAGE.A.01](../10-sitemap/buyer-mobile-web/PAGE_A_01_homepage.md) |
| `REQ.A.07.FR-005` | 시스템은 `OPEN` 상태 드롭에 대해 `sell_through_rate / elapsed_minutes` 공식으로 오픈 후 랭킹을 산출한다 (2026-07-14: order-service 연동 방식 별도 논의 후 착수, 후속 스코프) | 구매자, DropMong 운영자 | Must | [PAGE.A.01](../10-sitemap/buyer-mobile-web/PAGE_A_01_homepage.md) |
| `REQ.A.07.FR-006` | 시스템은 `catalog.drop.updated` 이벤트를 구독해 드롭 상태 전환(`SCHEDULED → OPEN`)에 따라 랭킹 리스트를 전환한다 (2026-07-14: FR-003이 phase 구분을 쓰지 않게 되면서 이 요구사항은 오픈 후 랭킹(FR-005) 스코프에만 남는다, 후속 스코프) | - | Must | - |
...
| `REQ.A.07.NFR-006` | 오픈 전 랭킹은 실시간 감쇠/속도 계산 없이 리셋 없는 누적 카운트만 사용한다(2026-07-14 수정: "당일" 리셋 요건 제거, 아래 수정 이력 참고) | 오픈 전 랭킹 점수 계산 로직에 시간 가중치/감쇠 항이 없고, 자정 리셋 로직도 없다 | `analysis/02-realtime-ranking-score-design.md` |
...
- (2026-07-14 보류) 드롭 상태가 `SCHEDULED`에서 `OPEN`으로 바뀌면 해당 드롭이 `ranking:upcoming`에서 빠지고 `ranking:hot` 계산 대상으로 전환된다 — `FR-006`과 함께 후속 스코프.
- 오픈 전 랭킹 카운터는 리셋되지 않는 누적 활성 찜 수다(2026-07-14 수정, "매일 00:00 리셋" 기준 삭제).
- 사용자가 찜을 해제하면 해당 드롭의 오픈 전 랭킹 카운터가 즉시 감소한다. 같은 사용자가 짧은 시간 안에 찜/해제를 반복해도 최종 카운터 값은 실제 찜 상태와 일치한다.
- (2026-07-14 보류) 재고 수량이 서로 다른 두 드롭이 동일한 소진 속도를 보이면 오픈 후 랭킹에서 비슷한 순위를 받는다 — `FR-005`와 함께 후속 스코프.
- (2026-07-14 보류) 비로그인 사용자의 조회는 랭킹 점수에 반영되지 않는다 — 조회수 자체가 `FR-004`와 함께 후속 스코프.
...
- (2026-07-14 추가) `FR-003`이 더는 `SCHEDULED` 필터를 쓰지 않으면서 `API.A.07-06`의 `operationId`(`listUpcomingRanking`)/경로(`/v1/rankings/drops/upcoming`)의 "upcoming"이라는 이름이 실제 동작(전체 드롭 대상)과 안 맞게 됐다. 외부 계약 이름 변경 여부는 팀과 확정 필요 — 지금은 이름을 유지하고 설명만 정정했다.
- (2026-07-14 추가) 오픈 후 랭킹(`FR-005`)을 order-service와 어떻게 연동할지(동기 조회 vs `order.created` 비동기 구독 + `total_quantity` 없는 raw 판매 속도로 공식 재정의) 미확정. 착수 전 별도 논의 필요.
- (2026-07-14 해소) ~~조회수는 Redis 인프라가 준비된 뒤 반영~~ — Redis 없이 `drop_views` 원문 기록 + 집계 시점 distinct count 방식으로 지금 구현한다(`FR-004`, `FR-011`).
- (2026-07-14 4차 수정, 해소) `drop_views`가 무한정 쌓이는 문제는 별도 청소 배치 없이 해결했다 — "실시간 조회 랭킹 배치 Worker"가 3시간마다 스냅샷을 `drop_view_rankings`에 저장한 직후, 이미 스냅샷으로 만든 **직전** 구간의 `drop_views` 원본을 지운다(안전 마진으로 1구간은 남겨둠). 스냅샷 이후엔 원본을 다시 읽을 일이 없다는 점을 이용한 것으로, `drop_views`는 항상 최대 2개 구간(6시간)분만 유지된다.
...
## 2026-07-14 수정 이력(2차, "실시간 많이 보고 있는 상품" 추가)

- 처음엔 조회 기반 랭킹(`FR-004`, 원래 `FR-005` 오픈 후 랭킹의 대체 아이디어)을 Redis 인프라가 필요하다는 이유로 전부 후속 스코프로 미뤘었다. 다시 검토한 결과, Redis `SETNX` dedup 없이도 같은 효과를 낼 수 있는 더 단순한 방법을 찾아 지금 구현 범위에 포함시켰다.
- 새 방식: 조회를 막지 않고 매번 원문 그대로 기록한다(`drop_views(drop_id, user_id, viewed_at)`, insert-only). 같은 사용자의 반복 조회(새로고침)는 집계 시점의 `COUNT(DISTINCT user_id)`로 자연히 1명으로만 잡혀 별도 dedup 인프라(Redis)가 필요 없다.
- `FR-011`을 신설해 "실시간 많이 보고 있는 상품"(조회 기반, 드롭 상태 무관, 최근 3시간 서로 다른 조회자 수)을 `FR-003`("기다리는 상품", 찜 기반, 리셋 없는 누적)과 나란한 별개 랭킹으로 정의했다. 두 랭킹 모두 catalog-service/order-service 의존이 없다.
- (2026-07-14 3차 수정) "최근 3시간"을 매 API 호출마다 다시 계산하는 슬라이딩 윈도우가 아니라, **KST 03/06/09/12/15/18/21/24시 정각에 정렬된 고정 버킷**으로 확정했다. 매 정각에 배치가 직전 3시간 구간을 한 번만 집계해 `drop_view_rankings` 스냅샷 테이블에 저장하고, API는 그 스냅샷만 읽는다. 이렇게 하면 사용자가 랭킹을 아무리 자주 조회해도 무거운 `COUNT(DISTINCT)` 쿼리가 반복 실행되지 않는다 — 부하테스트에서 신경 써야 할 지점이 "조회 기록 쓰기 경로의 핫키"(`HOTSPOT.A.07-01`과 동일 성격, 이미 실측 후 결정으로 미룬 항목) 하나로 좁혀진다.

## 2026-07-14 수정 이력

- `FR-003`(오픈 전 랭킹)의 산출 방식을 "`SCHEDULED` 상태 드롭 대상, 당일(00:00 리셋) 찜+조회 가중합"에서 "드롭 상태 구분 없이, 리셋 없는 누적 활성 찜 수"로 단순화했다. 이유는 두 가지다.
  1. 찜은 반복 발생하는 행동이 아니라 한 번 누르고 유지되는 상태라서, "당일 순증감"만 세면 대부분의 드롭에서 신호가 희박해진다. 반면 "지금 몇 명이 찜하고 있나"(누적치)는 항상 안정적인 신호다.
  2. `SCHEDULED` 필터를 쓰려면 `catalog.drop.updated`를 구독해 `drop_phase`를 계속 동기화해야 하는데, catalog-service가 아직 Kafka 연동이 전혀 없는 스텁 상태라(`SD.A.07` 확인, 2026-07-13) 이 의존을 피하는 쪽을 택했다.
  - 티켓팅/드롭 커머스 유사 사례(Steam Top Wishlisted, 와디즈 오픈예정 알림신청, Kickstarter Most Funded/Trending, 한터차트)를 조사한 결과, 오픈 전/후 신호를 하나의 점수로 섞지 않고 별도로 다루는 것이 업계 공통 패턴임을 확인해 오픈 전/오픈 후 랭킹 분리 자체는 유지했다.
- `FR-005`(오픈 후 랭킹), `FR-006`(카탈로그 상태 전환 구독), `FR-004`(조회수 dedup)는 이번 구현 슬라이스에서 후속 스코프로 미뤘다. `FR-003`만 지금 구현한다.
- 조회수는 "관심의 필요조건이 아니다"(조회는 했지만 찜은 안 할 수 있다)라는 점에서, 오픈 전 랭킹의 1차 신호를 찜만으로 좁힌 것도 이번 결정에 포함된다.
