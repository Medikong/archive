...
| `REQ.A.07.FR-003` | 시스템은 카탈로그 드롭 상태 구분 없이(2026-07-14: catalog-service 의존 제거), 드롭별 **전환율(누적 활성 찜 수 ÷ 누적 조회수)** 내림차순으로 오픈 전 랭킹을 산출한다. 조회수가 표본 임계값 미만이면 전환율 대신 원시 찜수로 하위 티어에 배치하고, 최근 활동(찜 또는 조회)이 일정 기간 없는 드롭은 후보에서 제외한다(2026-07-21 재설계, 아래 수정 이력 참고) | 구매자, DropMong 운영자 | Must | [PAGE.A.01](../10-sitemap/buyer-mobile-web/PAGE_A_01_homepage.md) |
...
## 2026-07-21 수정 이력 ("얼어붙은 비율" 문제 대응)

- **문제**: `FR-003`의 누적 활성 찜 수 정렬은 드롭 상태 구분이 사라지면서(위 2026-07-14 수정) 오픈 전 드롭뿐 아니라 이미 오픈·소진·종료된 드롭까지 전부 대상이 됐고, 리셋이 없어 인기 절정기에 쌓인 찜 수가 활동이 끊긴 뒤에도 그대로 남아 오래된 드롭이 계속 1위를 차지하는 기간편향이 있다(2026-07-20 멘토링 후속 논의에서 발견).
- **검토했다가 기각한 대안**:
  1. 찜을 기간(당일/N일)으로 잘라 세는 방식 — 위 2026-07-14 수정 이력에서 이미 "당일 리셋"으로 시도했다가 "찜은 반복 행동이 아니라 신호가 희박해진다"는 이유로 되돌린 전례가 있어, 윈도우 길이만 바꿔도 같은 문제를 반복할 것으로 판단해 기각.
  2. 속도/감쇠 기반 "급상승" 랭킹 — `PP-006`에서 이미 기각(네이버/다음 실시간 급상승 검색어 신뢰성 논란 + 신호 밀도 부족).
  3. 조회 기반 랭킹(`FR-011`)으로 단일화 — "조회는 관심의 필요조건이 아니다"라는 위 결정과 충돌하고, 조회 기록 없이 찜만 받은 드롭이 후보에서 아예 빠지는 새 문제가 생겨 기각.
  4. 누적 찜수와 구간 조회수의 가중합 — 구간값(3시간)과 누적값(무한 성장)을 고정 비율로 섞으면 서비스가 자랄수록 누적값 쪽이 비율을 무너뜨려(수식으로 검증됨) 가중치를 주기적으로 재튜닝해야 하는 불안정한 설계라 기각.
- **채택한 방식**: 전환율(누적 찜수 ÷ 누적 조회수) 정렬 + 최근 활동 게이트. 카운트 방식(윈도우/감쇠/가중치) 자체는 건드리지 않아 위 기각 사유들을 피하고, "최근 활동이 있었는가"라는 이진 판정 하나만 추가해 얼어붙은 비율을 걸러낸다.
  - 최근 활동 게이트 기간은 도메인 문서에 드롭의 전형적 지속 기간 근거가 없어 14일을 잠정치로 둔다 — 실사용 데이터가 쌓이면 조정 대상.
  - 조회수가 임계값(20) 미만인 드롭은 전환율 표본 부족으로 왜곡되기 쉬워, 전환율 대신 원시 찜수로 하위 티어에 배치한다.
