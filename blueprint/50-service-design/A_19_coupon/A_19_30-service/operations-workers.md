...
- 재시도는 지수 backoff와 상한을 지원하되 횟수·간격·종료 기준은 `HOTSPOT.A.19-06`의 설정 입력으로 받는다.
...
`BulkIssuePlannerWorker`는 외부 대상 포트에서 `audience_definition_ref`와 `as_of`에 맞는 식별자 페이지를 읽는다. 각 대상에 `bulk_job_id + target_ref` 업무 고유키로 `CMD.A.19-13`을 요청한다. 사용자 프로필 원본이나 전체 대상 목록을 `BulkCouponIssueJob` 안에 복제하지 않는다.
