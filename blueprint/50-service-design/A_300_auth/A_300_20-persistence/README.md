...
Context 인증의 Aggregate와 처리 상태를 PostgreSQL에 저장하고, 인증 식별자 단일 연결, 로그인 잠금, Session과 refresh token 회전, 비밀번호 재설정, outbox 전송을 데이터베이스 제약과 트랜잭션으로 보장한다.

이 문서에서 인증 Use Case의 기준 식별자는 `UC.A.300`이다. 요구사항 원천 식별자 `REQ.A.05`를 Use Case 식별자로 재사용하지 않는다.

## 연관 태그

🏷️ 요구사항 참조: [REQ.A.05](../../../00-requirements/REQ_A_05_auth_member.md) | UC 참조: [UC.A.300](../../../30-uc/UC_A_300_auth_member.md) | BC 참조: [BC.A.300](../../../40-event-storming-bounded-context/BC_A_300_auth_member.md) | 도메인 참조: [SD.A.30010](../A_300_10-domain-model/SD_A_30010_auth_domain_model.md) | 서비스 참조: [SD.A.30030](../A_300_30-service/README.md) | API 참조: [SD.A.30040](../A_300_40-api/README.md) | JWT/JWKS/Istio 기준: [SD.A.300.JWT](../jwt-jwks-istio.md)

## 저장 경계

### 소유 데이터

- Context 인증은 `Identity`, `PasswordCredential`, `VerificationChallenge`, `IdentityLink`, `Registration`, `PasswordReset`, `AuthenticationIntent`, `Session`, `SessionCredential`, 외부 projection인 `UserAuthState`의 현재 상태와 변경 이력을 저장한다. 고위험 작업의 단기 `ReauthenticationProof`는 1회 소비를 보장하는 보안 레코드로 저장한다.
- `user_id`는 Context 사용자가 발급한 외부 식별자다. 인증 DB는 값만 참조하며 다른 컨텍스트 DB에 외래 키를 만들지 않는다.
- role, permission, membership과 업무 리소스 소유권은 저장하지 않는다. 사용자 표시명, 주소, 연락처 프로필, 마케팅 속성도 저장하지 않는다.
...
| 이메일·SMS 인증번호 평문 | Challenge에는 keyed hash만 저장한다. 발송에 필요한 원문은 전용 short-TTL delivery ciphertext에만 저장한다. 개발·테스트 가상 조회 projection은 아래 전용 테이블과 키 범위에서만 예외적으로 복호화할 수 있다. |
| 웹·모바일 refresh token 평문 | SessionCredential에는 keyed hash만 저장한다. refresh 멱등 재생은 전용 short-TTL replay ciphertext만 예외로 허용한다. |
| ReauthenticationProof 원문 | proof 식별자와 secret을 분리하고 keyed hash만 저장한다. 목적, Session, 만료·소비 상태만 단기 보존한다. |
| access JWT 평문 | 짧은 수명의 발급 결과물이다. 기본적으로 `Session`만 저장하며, access JWT는 refresh replay 전용 short-TTL ciphertext에만 예외적으로 포함한다. |
| 이메일·휴대폰 원문 평문 | 정규화 값은 envelope encryption한 ciphertext로 저장하고, 정확 일치 조회는 별도 HMAC으로 처리한다. |
| 이름, 추천인 코드, 약관 동의, 프로필 | 프론트엔드와 담당 Context가 직접 처리한다. 인증 DB에는 담당 Context가 발급한 opaque 참조만 저장한다. |
| 감사 검색 원장 | Context 감사가 보관한다. 인증 DB에는 전달 전 outbox만 유지한다. |

## 기술 기준

- 기준 저장소는 PostgreSQL이다. Session 상태 확인에는 공유 Redis projection을 필수로 사용하지만 최종 기준은 PostgreSQL이다.
...
| `auth_signing_key_metadata` | access JWT의 `kid`, 공개 JWK와 key rotation 상태 | `SigningKeyPolicy` | private key 원문 없이 key metadata만 보존 |
...
| `auth_verification_challenges` | 이메일·SMS 인증번호의 발급·검증·만료 상태 | `VerificationChallenge` | challenge별 독립 Aggregate |
| `auth_verification_delivery_payloads` | provider 발송에 필요한 목적지와 code의 단기 암호문 | 발송 인프라 레코드 | challenge send별 1개, terminal 처리 후 crypto-shred |
| `auth_virtual_verification_messages` | 개발·테스트에서 최신 가상 발송 code를 조회하는 전용 projection | 개발·테스트 Adapter projection | 운영 migration 미적용, Challenge terminal 처리와 함께 crypto-shred |
| `auth_identity_links` | Identity와 `user_id` 연결 및 교체 이력 | `IdentityLink` | Aggregate Root |
| `auth_registrations` | 이메일·휴대폰 검증과 동기 회원가입 완료 상태 | `Registration` | Aggregate Root |
...
| `auth_session_credentials` | 웹·모바일 refresh token 검증, 회전, 재사용 탐지 | `SessionCredential` | Session 하위 Entity |
| `auth_user_auth_states` | Context 사용자의 계정 상태와 인증 허용 상태 | `UserAuthState` | user_id별 1행, user_version 단조 증가 |
| `auth_reauthentication_proofs` | 고위험 작업용 proof의 목적 바인딩과 1회 소비 상태 | `ReauthenticationProof` 보안 레코드 | 짧은 TTL, proof별 1회 소비 |
| `auth_idempotency_records` | 중복 Command의 처리 결과 참조 | `IdempotencyRecord` | 만료되는 인프라 레코드 |
| `auth_idempotency_replay_payloads` | refresh, 재인증, 휴대폰 교체의 승인된 credential 전달 복구 응답 단기 암호문 | 멱등 재생 인프라 레코드 | IdempotencyRecord별 최대 1개, 허용 operation allowlist |
...
| `login_failure_threshold` | SMALLINT | NOT NULL, CHECK `BETWEEN 1 AND 20`, 기본 5 | 잠금 전 허용 실패 횟수 |
| `login_failure_window_seconds` | INTEGER | NOT NULL, CHECK `BETWEEN 1 AND 86400` | 실패 횟수를 합산하는 구간 |
| `lock_duration_seconds` | INTEGER | NOT NULL, CHECK `BETWEEN 1 AND 604800` | 잠금 유지 시간 |
| `reset_failure_on_success` | BOOLEAN | NOT NULL, 기본 true | 로그인 성공 시 실패 구간 초기화 여부 |
| `session_idle_ttl_seconds` | INTEGER | NOT NULL, CHECK `BETWEEN 1 AND 604800` | 웹·모바일 공통 Session 유휴 TTL |
| `session_absolute_ttl_seconds` | INTEGER | NOT NULL, CHECK `session_absolute_ttl_seconds BETWEEN session_idle_ttl_seconds AND 31536000` | 웹·모바일 공통 Session 최대 TTL |
| `access_ttl_seconds` | INTEGER | NOT NULL, CHECK `BETWEEN 1 AND 86400`, 기본 900 | access JWT TTL |
| `refresh_ttl_seconds` | INTEGER | NOT NULL, CHECK `refresh_ttl_seconds > access_ttl_seconds AND refresh_ttl_seconds <= 31536000`, 기본 1,209,600 | 일반 refresh token TTL |
| `remember_me_ttl_seconds` | INTEGER | NOT NULL, CHECK `remember_me_ttl_seconds >= refresh_ttl_seconds AND remember_me_ttl_seconds <= 31536000`, 기본 2,592,000 | 로그인 상태 유지 TTL |
...
`auth_verification_policy_rules`는 `(policy_version, purpose, channel)`을 PK로 사용한다. `ttl_seconds`와 `resend_interval_seconds`는 1~86,400초, `max_attempts`와 `max_sends`는 1~20으로 제한하고 purpose별 허용 channel 조합을 서비스와 CHECK로 검증한다. `auth_session_revocation_policy_rules`는 `(policy_version, trigger, scope)`을 PK로 사용하고 scope를 `current_session`, `identity_sessions`, `user_sessions`, `refresh_family` 중 하나로 제한한다. 한 trigger에 여러 scope를 둘 수 있다. 정책 활성화 transaction은 두 하위 규칙 집합이 모두 유효한지 확인한 뒤 버전 전체를 active로 바꾼다.
...
JWKS 응답과 signing key rotation 순서는 [SD.A.300.JWT](../jwt-jwks-istio.md)을 따른다. 이 테이블은 그 기준을 실행하는 key metadata만 저장한다.

| 필드 | PostgreSQL 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `kid` | VARCHAR(128) | PK | JWT protected header와 JWKS를 연결하는 key ID |
| `algorithm` | VARCHAR(16) | NOT NULL, CHECK | 초기 기준은 `RS256` |
| `status` | VARCHAR(16) | NOT NULL, CHECK | `active`, `retiring`, `retired` |
| `public_jwk` | JSONB | NOT NULL | Istio가 검증에 사용하는 공개키 표현 |
| `private_key_ref` | VARCHAR(512) | NOT NULL | Secret Manager/KMS의 private key 참조이며 key 원문이 아님 |
| `activated_at` | TIMESTAMPTZ | NOT NULL | 신규 access JWT 서명 시작 시각 |
| `verify_until` | TIMESTAMPTZ | NULL | 기존 JWT 검증을 위해 공개키를 유지할 종료 시각 |
| `retired_at` | TIMESTAMPTZ | NULL | JWKS 제외 완료 시각 |
| `created_at` | TIMESTAMPTZ | NOT NULL | metadata 생성 시각 |

- active signing key는 하나만 허용한다. 새 key 활성화와 이전 key의 `retiring` 전환을 한 transaction에서 처리한다.
- `verify_until`은 공통 기준이 요구하는 rotation overlap보다 빠를 수 없다.
- private key 원문은 PostgreSQL, Redis, 로그, trace와 outbox에 저장하지 않는다.
...
| `auth_verification_policy_rules` | `ttl_seconds`, `resend_interval_seconds` | INTEGER | NOT NULL, CHECK `BETWEEN 1 AND 86400` |
| `auth_verification_policy_rules` | `max_attempts`, `max_sends` | INTEGER | NOT NULL, CHECK `BETWEEN 1 AND 20` |
...
| `channel` | VARCHAR(24) | NOT NULL, CHECK | MVP는 `email_code`, `sms_code` |
...
- 이메일·SMS 검증 요청은 `challenge_id`를 함께 받아 해당 행을 잠근 뒤 verifier를 비교한다. 짧은 인증번호의 hash만으로 challenge를 전역 조회하지 않는다.
- 발송 outbox에는 `challenge_id`와 `delivery_payload_ref`만 넣는다. relay는 아래 전용 payload를 전송 직전에만 복호화한다. Domain/Audit event와 일반 outbox payload에 목적지, code, ciphertext를 복제하지 않는다.

### `auth_verification_delivery_payloads`

HMAC만으로는 이메일·SMS code를 복원할 수 없으므로, provider relay가 사용할 최소 발송 payload를 별도 short-TTL ciphertext로 보관한다.
...
| `payload_ciphertext` | BYTEA | NULL | destination, template/locale, code의 envelope ciphertext |
...
### `auth_virtual_verification_messages` (개발·테스트 전용)

가상 Email/SMS Adapter가 실제 provider 대신 발송을 수락한 뒤 `API.A.300-30`이 조회할 projection이다. 이 테이블의 migration과 암호화 key ring은 `dev`·`test` profile에서만 로드하며 운영 schema에는 만들지 않는다.

| 필드 | PostgreSQL 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `challenge_id` | UUID | PK, FK | 가상 발송 대상 VerificationChallenge |
| `channel` | VARCHAR(24) | NOT NULL, CHECK | `email_code`, `sms_code` |
| `challenge_version` | BIGINT | NOT NULL | projection을 만든 Challenge row version |
| `code_ciphertext` | BYTEA | NULL | 조회 시에만 복호화하는 6자리 code envelope ciphertext |
| `code_key_id` | VARCHAR(128) | NOT NULL | 개발·테스트 projection 전용 key 참조 |
| `masked_destination` | VARCHAR(320) | NOT NULL | 원문을 복원할 수 없는 화면 표시값 |
| `status` | VARCHAR(16) | NOT NULL, CHECK | `pending`, `ready`, `destroyed` |
| `expires_at` | TIMESTAMPTZ | NOT NULL | Challenge 만료를 넘지 않는 projection TTL |
| `destroyed_at` | TIMESTAMPTZ | NULL | code ciphertext 삭제 또는 DEK 폐기 시각 |
| `created_at` | TIMESTAMPTZ | NOT NULL | 가상 발송 수락 시각 |

- `(challenge_id, challenge_version)`이 현재 Challenge와 일치하고 Challenge가 `issued`인 경우에만 code를 반환한다. 조회 transaction에서 Challenge를 다시 읽어 terminal 전환과 경합하면 `AUTH_VIRTUAL_MESSAGE_UNAVAILABLE`을 반환한다.
- 가상 Adapter는 delivery relay가 메시지를 수락한 뒤 projection을 `ready`로 만든다. 외부 provider를 선택한 Challenge에는 행을 만들지 않는다.
- Challenge를 `verified`, `failed`, `expired`, `revoked`로 닫는 transaction은 같은 DB의 projection도 `destroyed`로 바꾸고 `code_ciphertext = NULL`로 지운다. 정리 worker는 누락된 terminal 행과 만료 행을 반복 정리한다.
- 운영 profile에서 가상 Adapter, 이 migration 또는 projection key가 설정되면 서비스 시작을 실패시킨다. 개발 API와 key 접근 권한도 일반 Auth process와 분리한다.
...
| `status` | VARCHAR(32) | NOT NULL, CHECK | `pending_verification`, `verified`, `completed`, `failed`, `expired` |
...
| `user_id` | UUID | NULL, 외부 참조 | 프론트엔드가 User 생성 뒤 전달한 사용자 식별자 |
| `verification_completed_at` | TIMESTAMPTZ | NULL | 두 필수 Challenge 완료 시각 |
| `completion_idempotency_record_id` | UUID | UNIQUE, NULL | 회원가입 완료 요청의 IdempotencyRecord |
...
| `status_proof_hash` | BYTEA | UNIQUE, NOT NULL, CHECK 32 bytes | 상태 조회 전용 opaque token의 HMAC |
| `status_proof_key_version` | SMALLINT | NOT NULL | 상태 proof HMAC key 버전 |
| `status_proof_expires_at` | TIMESTAMPTZ | NOT NULL | terminal 뒤 짧은 조회 보존 기간까지 포함한 proof 만료 |
...
- 현재 Registration의 두 Challenge가 모두 verified이기 전에는 `verified`로 전환하거나 completion proof를 발급할 수 없다.
- 완료 요청은 `completion_idempotency_record_id`를 한 번만 설정한다. 이후 같은 record를 찾은 요청만 같은 결과를 재사용한다.
- 최종 상태와 응답 복구 TTL이 지난 뒤에는 `completion_idempotency_record_id`를 NULL로 정리할 수 있다. 진행 중인 Registration에서는 지우지 않는다.
- `completed`에는 `user_id`, `session_id`, 두 active IdentityLink와 완료 IdempotencyRecord가 필요하다. `user_id`는 최초 설정 후 변경할 수 없다.
- Registration 생성 transaction은 상태 조회 token 원문을 응답하고 keyed hash만 저장한다. 같은 operation/scope/key/request hash 재시도는 같은 Registration을 잠가 status proof hash를 새 값으로 교체하고 새 원문만 응답한다. 이전 proof는 즉시 무효화하며 token 원문이나 응답 ciphertext를 저장하지 않는다. 이 proof는 Registration 조회 전용이며 complete/challenge API의 소유 proof로 사용할 수 없다.
- terminal 상태의 `status_proof_expires_at`은 terminal 시각보다 뒤이되 정책의 짧은 상태 보존 상한을 넘지 않는다. 만료 뒤 token으로 조회해도 Registration 존재 여부를 공개하지 않는다.
- Registration과 연결된 AuthenticationIntent를 같은 transaction에서 잠그고 `intent.expires_at`을 최소한 Registration `expires_at`까지 연장한다. 단, 사전 인증 proof 최대 수명 상한을 넘을 수 없으며 상한 안에 Registration을 완료할 수 없으면 생성을 거부한다.
- `completed`이면 두 Identity 모두 같은 `user_id`에 active 연결되고 `session_id`가 있어야 한다. 이 조건은 여러 테이블을 조회하므로 서비스 트랜잭션과 유니크 인덱스로 보장한다.
- 이름, 추천인 코드와 필수 동의 payload는 이 테이블에 저장하지 않는다. 프론트엔드가 User 생성 API에 전달한다.
...
- Challenge 검증의 같은 operation/scope/key/request hash 재시도는 PasswordReset과 IdempotencyRecord를 잠근 뒤 `reset_grant_hash`를 새 난수의 HMAC으로 교체하고 commit 뒤 새 원문만 반환한다. 이전 grant는 즉시 무효화하며 replay ciphertext를 만들지 않는다.
- 동일 `identity_id`에는 진행 중 reset을 하나만 둔다. NULL identity decoy는 이 제약 대상에서 제외한다.
- PasswordReset 생성 transaction은 연결된 AuthenticationIntent를 잠그고 Intent와 owner proof의 서버 만료를 reset `expires_at` 이상으로 연장한다. 사전 인증 proof 최대 수명 안에 이를 보장할 수 없으면 reset을 만들지 않는다.
...
| `schema_version` | SMALLINT | NOT NULL | action payload schema 버전 |
...
| `delivered_at` | TIMESTAMPTZ | NULL | 로그인 성공 뒤 프론트엔드에 전달 완료한 시각 |
...
| `remember_me` | BOOLEAN | NULL | 휴대폰 로그인 시작 요청이 최초로 고정하는 Session TTL 선택 |
...
| `delivery_recovery_expires_at` | TIMESTAMPTZ | NULL | Session 발급 성공 응답 유실의 recovery-only owner proof 기한 |
...
- bootstrap의 같은 operation/scope/key/request hash 재시도는 Intent와 IdempotencyRecord를 잠근 뒤 새 owner proof와 웹 CSRF secret hash로 교체한다. commit 뒤 새 원문을 응답하고 이전 hash는 즉시 사용할 수 없게 하며 proof 응답 ciphertext를 저장하지 않는다.
- 휴대폰 로그인 Challenge를 처음 발급할 때 `remember_me`가 NULL이면 요청값으로 설정한다. 이미 값이 있으면 동일한 값만 허용하며 검증 완료 API는 이 값을 읽어 Session 정책을 결정한다.
- Registration 또는 PasswordReset을 만들 때 Intent를 `FOR UPDATE`로 잠그고 하위 작업 `expires_at`까지 연장한다. 연장 뒤 시각은 사전 인증 proof 최대 수명 상한을 넘지 않아야 하며 action payload TTL도 같은 시각으로 원자적으로 연장하거나 하위 작업 생성을 거부한다.
...
- `session_issued` 소비에는 짧은 `delivery_recovery_expires_at`을 둘 수 있다. 이 기간의 owner proof hash는 같은 Session·operation·key·request hash의 credential 전달 복구에서만 조회하고 active Intent/일반 사전 인증 쿼리에서는 제외한다. 기한 종료 worker는 hash를 지우거나 별도 tombstone으로 바꾼다.
...
| `idle_expires_at` | TIMESTAMPTZ | NOT NULL | Session 유휴 만료 시각 |
...
- `absolute_expires_at > issued_at`이어야 하고 `idle_expires_at`은 `absolute_expires_at`을 넘을 수 없다.
- Session의 `user_id`는 IdentityLink의 `user_id`와 같아야 하고 발급·재바인딩 시점에 active여야 한다.
...
- Session을 `revoked` 또는 `reuse_detected`로 바꾸는 transaction은 `Auth.SessionRevoked`와 Session 상태 cache 갱신 outbox를 함께 저장한다.
...
| `credential_type` | VARCHAR(32) | NOT NULL, CHECK | `refresh_token` |
| `credential_status` | VARCHAR(32) | NOT NULL, CHECK | `active`, `rotated_pending_delivery`, `rotated`, `expired`, `revoked`, `reuse_detected` |
...
| `refresh_family_id` | UUID | NOT NULL | 웹·모바일 공통 refresh rotation 묶음 |
...
| `delivery_recovery_expires_at` | TIMESTAMPTZ | NULL | 동일 credential 전달 요청에만 이전 hash를 인정하는 짧은 복구 기한 |
| `revoked_at` | TIMESTAMPTZ | NULL | 폐기 시각 |
| `reuse_detected_at` | TIMESTAMPTZ | NULL | 재사용 탐지 시각 |
| `row_version` | BIGINT | NOT NULL, 기본 0 | 동시 회전 방지 |

- 모든 SessionCredential은 `refresh_token`이고 하나의 `refresh_family_id`에 속한다. 웹과 모바일은 같은 저장 모델을 사용하며 전달 방식만 다르다.
- session별 active refresh credential과 refresh family별 active credential은 각각 하나다.
- credential의 `expires_at`은 소속 Session의 `absolute_expires_at`을 넘을 수 없다.
- `rotated_pending_delivery`이면 `rotated_at`, `rotated_to_credential_id`, `delivery_recovery_expires_at`이 필요하다. 이 상태는 일반 Session 인증과 refresh에서 항상 거부하고 allowlist된 동일 credential 전달 복구에서만 확인한다. 전달 확인 또는 기한 종료 뒤 `rotated`로 닫고 복구 기한을 지운다.
...
- SessionCredential에는 token 원문을 저장하지 않는다. allowlist된 refresh, 이메일 재인증과 휴대폰 교체 완료의 동일 멱등 요청 재생만 전용 short-TTL encrypted replay payload에 격리한다.
...
| `user_version` | BIGINT | NOT NULL, CHECK `> 0` | User의 단조 증가 version |
| `reason_code` | VARCHAR(64) | NULL | 외부에 직접 노출하지 않는 일반 사유 |
| `effective_at` | TIMESTAMPTZ | NOT NULL | 효력 시각 |
| `status_change_id` | VARCHAR(128) | UNIQUE, NOT NULL | User 서명 proof에서 검증한 멱등 key |
| `row_version` | BIGINT | NOT NULL, 기본 0 | 동시 반영 제어 |
| `updated_at` | TIMESTAMPTZ | NOT NULL | 마지막 반영 시각 |

- 제한·비활성 동기 요청은 더 높은 `user_version`만 반영한다. 상태 전환과 전체 Session 폐기를 한 transaction에 저장한다.
- 로그인, refresh, 재인증은 UserAuthState가 없으면 Context 사용자의 초기 상태를 확인한 뒤 행을 만들고, 행이 있으면 반드시 active인지 확인한다. 장애나 cache miss를 active로 간주하지 않는다.
- active 복귀는 User 서명 proof의 더 높은 `user_version`에서만 허용한다.

### `auth_reauthentication_proofs`

고위험 작업 직전의 최근 인증 결과를 access JWT와 분리해 저장한다. API에는 `proof_id.secret` 형식의 opaque 값을 반환하고 DB에는 secret의 keyed hash만 둔다.
...
| `operation` | VARCHAR(64) | NOT NULL | Command 이름 |
...
| `replay_payload_ref` | UUID | FK, NULL | allowlist된 refresh 또는 credential 전달 복구 응답의 전용 payload 참조 |
...
- 회원가입 시작/완료, 이메일·휴대폰 로그인, 인증번호 재전송, refresh, 번호 변경, 운영자 명령과 사용자 상태 동기 반영에 적용한다.
- IdempotencyRecord 본문에는 token, cookie, 인증번호, 암호화한 전체 응답을 저장하지 않는다. `mobile_refresh`, `reauthenticate_email`, `complete_phone_replacement`의 짧은 credential 전달 복구만 전용 replay payload reference를 둘 수 있다.
- 가입 완료와 사용자 상태 반영은 Event가 아니라 API IdempotencyRecord로 처리한다.
- 회원가입 완료 요청은 `operation=complete_registration`, `resource_type=Registration`, `resource_id=registration_id`로 저장한다. 같은 key 재시도는 완료된 Session 결과를 가리킨다.
- 회원가입 완료 요청은 User proof, IdentityLink, Session과 Registration `completed`를 한 트랜잭션에 저장한 뒤 IdempotencyRecord를 완료한다.

### `auth_idempotency_replay_payloads`

refresh, 이메일 재인증과 휴대폰 교체 완료의 같은 Idempotency-Key 재시도는 새 rotation을 만들지 않고 최초 credential 전달 결과를 돌려줘야 한다. 이 예외를 일반 IdempotencyRecord나 SessionCredential에 넣지 않고 별도 보안 레코드로 격리한다.

| 필드 | PostgreSQL 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `replay_payload_id` | UUID | PK | IdempotencyRecord의 opaque reference |
| `payload_kind` | VARCHAR(48) | NOT NULL, CHECK | `mobile_refresh_response`, `reauthentication_credential_delivery`, `phone_replacement_credential_delivery` |
| `payload_ciphertext` | BYTEA | NULL | allowlist된 access/refresh token 또는 웹 cookie·CSRF 전달 결과의 envelope ciphertext |
...
- IdempotencyRecord별 replay payload는 하나이며 `replay_payload_ref`는 `mobile_refresh`, `reauthenticate_email`, `complete_phone_replacement` operation에서만 허용한다.
- 최초 refresh transaction은 credential rotation, encrypted replay payload, completed IdempotencyRecord, outbox를 함께 저장한다.
- 이메일 재인증 transaction은 Session 재바인딩, ReauthenticationProof, credential rotation, encrypted proof·credential 전달 payload, completed IdempotencyRecord와 outbox를 함께 저장한다.
- 같은 scope/key/request hash의 재시도는 만료 전 ciphertext를 메모리에서만 복호화해 같은 응답을 반환하고 `replay_count`만 증가시킨다. 새 credential을 만들지 않는다.
- payload가 만료되면 ciphertext를 즉시 지운다. refresh 재시도에는 `AUTH_REFRESH_RETRY_EXPIRED`, 재인증·휴대폰 교체 credential 전달 복구에는 `AUTH_SESSION_DELIVERY_EXPIRED`를 반환한다. 다른 key로 rotated refresh credential을 제시하면 refresh reuse 정책을 적용한다.
- 이메일 재인증과 휴대폰 교체 완료는 같은 Session·operation·key·request hash에 한해 이전 SessionCredential hash를 `rotated_pending_delivery` 복구 proof로 사용할 수 있다. 이 분기는 일반 인증보다 먼저 IdempotencyRecord와 replay binding을 확인하고, 다른 API·Session·key·body에는 이전 credential을 절대 허용하지 않는다. 전달 성공 또는 복구 TTL 종료 시 이전 credential의 복구 상태와 ciphertext를 함께 폐기한다.
...
| 필드 | PostgreSQL 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `event_id` | UUID | PK | OutboxEvent의 멱등 식별자 |
...
| `publish_status` | VARCHAR(16) | NOT NULL, CHECK | `pending`, `publishing`, `published`, `dead_letter` |
| `publish_attempts` | INTEGER | NOT NULL, 기본 0 | 전송 시도 횟수 |
| `next_attempt_at` | TIMESTAMPTZ | NOT NULL | 다음 전송 가능 시각 |
| `lease_owner` | VARCHAR(128) | NULL | publishing batch를 소유한 relay worker 식별자 |
| `lease_until` | TIMESTAMPTZ | NULL | worker 장애 시 재회수할 lease 만료 시각 |
...
- publisher는 짧은 트랜잭션에서 `pending` 또는 lease가 만료된 `publishing` 행을 `FOR UPDATE SKIP LOCKED`로 잡아 `publishing`, `lease_owner`, `lease_until`을 저장하고 커밋한다. broker publish는 트랜잭션 밖에서 수행한다. acknowledgement 뒤 별도 트랜잭션으로 `published`를 저장하고, 실패 시 lease 소유자를 확인해 `pending` 재시도 또는 `dead_letter`로 전환한다. at-least-once 중복은 소비자가 `event_id`로 제거한다.
- `publishing`이면 `lease_owner`와 `lease_until`이 모두 필요하다. `published`이면 `published_at`이 필요하며 최종 상태로 바꿀 때 lease 필드는 NULL로 지운다.
...
| SigningKeyPolicy | `auth_signing_key_metadata` | `kid`별 공개키와 rotation 상태를 저장 | private key는 Secret Manager/KMS 참조만 저장 |
| `Registration` | `auth_registrations` | 가입 검증 완료, User proof를 받은 동기 완료와 Session 전달 상태를 1행에 저장 | 프론트엔드의 완료 재시도는 IdempotencyRecord와 기존 논리 Session으로 처리 |
...
| ActionIntentPayload | `auth_action_intent_payloads` | action별 최소 payload를 단기 ciphertext로 저장 | 로그인 뒤 프론트엔드 1회 전달 후 crypto-shred |
...
| `UserAuthState` | `auth_user_auth_states` | user_id별 현재 account status와 user_version을 1행으로 저장 | 새 Session/refresh/재인증의 fail-closed gate |
| `ReauthenticationProof` | `auth_reauthentication_proofs` | opaque proof의 hash와 목적·소비 상태를 1행으로 저장 | access JWT와 분리하고 Session 폐기 시 무효화 |
| `IdempotencyRecord` | `auth_idempotency_records` | Command key별 1행 | 민감 응답은 저장하지 않음 |
...
CREATE UNIQUE INDEX uq_auth_registrations_status_proof
    ON auth_registrations (status_proof_hash);
...
| `auth_virtual_verification_messages` | `idx_auth_virtual_messages_expiry (status, expires_at)` | 개발·테스트 projection terminal 정리와 crypto-shred |
...
| `auth_registrations` | `uq_auth_registrations_status_proof`, `idx_auth_registrations_status_updated`, `idx_auth_registrations_verification_deadline` | 상태 proof 조회와 가입 만료 확정 |
...
| `auth_user_auth_states` | PK `user_id`, UNIQUE `status_change_id` | 인증 gate 조회와 User 서명 proof 요청 멱등 반영 |
...
| `auth_outbox_events` | `idx_auth_outbox_claim (publish_status, next_attempt_at, lease_until, created_at)` | pending 또는 lease가 만료된 publishing batch 회수 |
...
| `SigningKeyMetadataRepository` | `FindActive`, `ListVerificationKeys`, `ActivateAndRetire` | status, `kid`, `verify_until` | 서명 key metadata와 공개 JWK |
...
| `VirtualVerificationMessageRepository` | `UpsertReady`, `FindIssuedForUpdate`, `DestroyByChallenge`, `DestroyExpired` | challenge ID + version + status | 개발·테스트 전용 암호화 code projection |
...
| `RegistrationRepository` | `FindByID`, `FindByIDForUpdate`, `FindByStatusProofHash`, `ClaimExpiryBatch`, `Save` | registration_id, status proof hash, expires_at | Registration |
| `PasswordResetRepository` | `FindByIDForUpdate`, `FindActiveByIdentityID`, `Save` | reset_id, nullable email Identity | PasswordReset |
| `AuthenticationIntentRepository` | `Create`, `FindByOwnerProofHashForUpdate`, `RotateOwnerProof`, `BindRememberMe`, `ExtendExpiry`, `Consume` | owner proof hash, intent ID + row version | AuthenticationIntent |
...
| `UserAuthStateRepository` | `FindByUserID`, `FindByUserIDForUpdate`, `ApplyHigherVersion` | user_id, status change ID/user version | UserAuthState |
...
| `IdempotencyRepository` | `Claim`, `Find`, `FindByIDForUpdate`, `ExtendExpiry`, `Complete`, `Fail` | operation + scope/key hash, record ID, active resource | IdempotencyRecord |
| `IdempotencyReplayPayloadRepository` | `Insert`, `FindValid`, `IncrementReplayCount`, `DestroyExpired` | replay payload ID + allowlist된 operation/binding + expiry | 암호화 refresh·credential 전달 복구 응답 |
| `OutboxRepository` | `Append`, `ClaimPublishBatch`, `MarkPublished`, `ReleaseForRetry`, `MarkDeadLetter` | status + next_attempt_at + lease_owner/lease_until | OutboxEvent |
...
1. API에서 Idempotency-Key가 필수인 명령은 `auth_idempotency_records`에서 operation/scope/key를 claim한다. 이메일·휴대폰 로그인, 회원가입 완료와 refresh는 Session 결과 참조를 반드시 기록한다. 내부 전용 일반 Session 발급만 이 단계를 생략할 수 있다.
...
`API.A.300-01`의 같은-key 재요청은 일반 결과 재생과 다르게 처리한다. IdempotencyRecord와 기존 AuthenticationIntent를 함께 잠그고 request hash가 같음을 확인한 뒤 owner proof hash와 웹 CSRF hash를 새 난수로 교체한다. 기존 proof는 commit과 동시에 무효가 되고 새 원문만 응답한다. 이 경로는 별도 replay ciphertext를 만들지 않는다.

### 회원가입 시작과 완료

| 단계 | 트랜잭션 경계 | 정합성 기준 | 실패 처리 |
| --- | --- | --- | --- |
| 가입 시작 | AuthenticationIntent 만료 연장, email/phone Identity 예약, PasswordCredential, status proof hash, Registration, idempotency claim | 정규화 값 유일성, active Registration별 Identity 예약 유일성, 비밀번호 hash 저장, 하위 작업보다 짧지 않은 owner proof TTL | 귀속 Identity는 일반화된 가입 불가, 미귀속 expired/failed 예약은 cooldown 뒤 같은 행 재사용 |
...
| 가입 검증 완료 | Challenge, Identity와 Registration `verified` 저장 | 두 Challenge와 Identity 모두 verified | 짧은 수명의 서명된 completion proof 반환 |
| 회원가입 완료 | verified Registration, User proof, IdempotencyRecord, 두 IdentityLink, Session/SessionCredential | User proof의 registration ID와 user ID 일치, Identity owner 충돌 없음 | 한 트랜잭션에서 `completed`; 실패 시 전체 롤백 |

프론트엔드가 User 생성 뒤 Auth 완료 API를 호출한다. Auth는 User proof를 검증하고 같은 트랜잭션에서 IdentityLink와 Session을 저장한다. 같은 registration과 key 재시도는 저장된 `session_id`를 사용하며 다른 user ID는 충돌로 거부한다.

Identity 귀속 충돌은 두 Identity 행을 ID 순서로 잠근 뒤 Link 쓰기 전에 판정한다. unique constraint가 실패하면 트랜잭션 전체를 롤백하고 오류를 반환한다. 별도 Inbox나 보상 트랜잭션을 만들지 않는다.

### 회원가입 기한 만료

- 만료 worker는 `pending_verification` 또는 `verified`이면서 `expires_at <= now()`인 Registration을 `expired(REGISTRATION_EXPIRED)`로 닫는다.
- 완료 API와 만료 worker는 같은 Registration row lock과 조건부 update를 사용한다. 먼저 커밋한 결과를 유지한다.

로그인과 회원가입 완료 응답 유실 복구는 token 원문 저장이나 새 Session 생성으로 처리하지 않는다. 일반 사전 인증 middleware보다 먼저 consumed AuthenticationIntent의 recovery-only owner proof와 completed IdempotencyRecord를 찾고 같은 Session·operation·key·request hash 및 `delivery_recovery_expires_at`을 모두 확인한다. 통과하면 결과가 가리키는 Session과 active SessionCredential을 잠근 뒤 기존 credential 폐기와 대체 credential 발급을 한 transaction으로 처리한다. 이메일 로그인은 비밀번호를 다시 검증하고, 휴대폰 로그인은 같은 challenge ID의 이전 verified 결과와 action key를 확인하며, 회원가입은 Registration owner proof를 확인한다. 이 proof를 다른 API나 새 로그인에 허용하지 않는다. 현재 웹 cookie가 이미 유효하면 credential을 교체하지 않고, 복구 TTL이 끝나면 `AUTH_SESSION_DELIVERY_EXPIRED`로 거부해 새 로그인 action을 요구한다.
...
4. 비밀번호가 맞은 뒤 locked와 `password_reset_required`를 판정한다. 잠금 중이면 credential을 발급하지 않고, 재설정 필요면 강제 reset 결과만 반환한다. active일 때만 정책의 `reset_failure_on_success`에 따라 실패 구간과 만료된 잠금을 초기화하고 Session, SessionCredential, IdempotencyRecord, 로그인/Session outbox를 함께 저장한다.
...
Challenge 발급 transaction은 active IdentityLink 존재 여부와 무관하게 abuse control을 통과한 정규화 번호에 실제 `phone_signin` Challenge와 delivery payload를 만들고 AuthenticationIntent의 `remember_me`를 최초 요청값으로 고정한다. SMS challenge 확인 transaction에서는 challenge와 기존 phone Identity를 함께 잠근다. code가 틀리고 실제 Identity가 있으면 LoginLockPolicy에 따라 failure count/window/lock을 원자적으로 갱신하되 외부 응답은 일반 실패로 유지한다. code가 맞으면 저장된 `remember_me`, 잠금 상태, active IdentityLink와 UserAuthState를 확인한다. link가 없으면 Session을 만들지 않고 가입/연동 필요 코드를 반환한다. verified Identity와 active link가 확인되면 failure count를 초기화하고 Session, SessionCredential, IdempotencyRecord와 outbox를 같은 transaction에 저장한다.
...
2. AuthenticationIntent를 잠가 owner proof 만료를 PasswordReset TTL까지 연장하고 PasswordReset만 만든 뒤 존재 여부를 숨긴 접수 결과를 반환한다. 시작 transaction에서는 Challenge나 발송 outbox를 만들지 않는다.
3. 별도 challenge 발급 요청에서 PasswordReset을 잠그고 선택한 방식의 VerificationChallenge, delivery payload, 발송 outbox를 저장한다.
4. challenge 성공 시 1회용 reset grant를 발급해 hash만 저장하고 PasswordReset을 `challenge_verified`로 전환한다. 같은-key 성공 재요청은 PasswordReset과 IdempotencyRecord를 잠그고 grant hash를 교체한 뒤 새 원문만 반환한다.
...
2. 기존 SessionCredential을 `rotated_pending_delivery`로 닫고 채널에 맞는 대체 refresh credential을 만든다. 동시에 목적 하나에 묶인 `auth_reauthentication_proofs`, completed IdempotencyRecord, 최초 proof·credential 전달 결과의 short-TTL replay ciphertext와 `auth.reauthentication.succeeded` 보안 감사 outbox를 저장한다.
3. opaque proof와 새 credential 원문은 commit 후 반환한다. 일반 상태에는 keyed hash만 저장하고 replay ciphertext는 `reauthenticate_email` operation과 전용 binding·TTL에서만 허용한다.
...
재인증 응답 유실 재시도는 일반 인증 전에 이전 SessionCredential hash를 찾고, 같은 Session·operation·key·request hash와 `rotated_pending_delivery`가 모두 맞을 때만 최초 replay payload를 반환한다. 다른 요청에는 이전 credential을 허용하지 않으며 복구 TTL 종료 시 `AUTH_SESSION_DELIVERY_EXPIRED`로 종료한다.
...
- 현재 SessionCredential을 회전하고 최초 credential 전달 결과의 short-TTL ciphertext, completed IdempotencyRecord, 번호 교체 event와 감사 event를 함께 저장한다.

응답이 유실된 같은 요청은 일반 인증 middleware로 보내기 전에 이전 SessionCredential hash를 찾되, `rotated_pending_delivery` 상태와 같은 Session·operation·Idempotency-Key·request hash·replay binding이 모두 맞을 때만 복구 경로로 들어간다. 이 경로는 최초 ciphertext를 반환하고 회전이나 Link 교체를 반복하지 않는다. 다른 API, 다른 key/body, 다른 Session에는 이전 credential을 인증 수단으로 인정하지 않으며 복구 TTL 종료 시 ciphertext와 복구 상태를 폐기한다.
...
5. reuse 상태, 관련 credential과 Session 폐기, 감사 outbox를 하나의 transaction에서 처리한다.
...
- UserAuthState가 restricted/deactivated로 바뀌면 해당 사용자의 active Session과 refresh family를 폐기하고 `Auth.SessionRevoked`와 cache 갱신 outbox를 같은 transaction에 저장한다.
- 이미 발급된 짧은 TTL JWT도 Istio의 Session 상태 검사에서 거부되도록 폐기 상태를 Redis projection에 반영한다.
...
- 번호 교체와 회원가입 완료처럼 여러 유일성 조건이 겹치는 transaction은 serialization failure와 deadlock을 제한 횟수만 재시도한다. 같은 Command ID와 request hash일 때만 재시도한다.
...
| verification delivery payload | 전용 DEK의 AES-256-GCM envelope encryption | Challenge TTL 이내, terminal 처리 후 crypto-shred | destination과 code만 포함 |
| virtual verification projection | dev/test 전용 DEK의 AES-256-GCM envelope encryption | Challenge terminal/만료 즉시 crypto-shred | 개발·테스트에서만 조회 가능한 6자리 code, 운영 key ring과 분리 |
...
| credential delivery replay payload | 전용 DEK의 AES-256-GCM envelope encryption | 짧은 API retry TTL, 만료 즉시 crypto-shred | allowlist된 refresh, 재인증 또는 휴대폰 교체 완료의 최초 credential 전달 응답만 포함 |
...
| --- | --- | --- | --- |
| 활성 인증 정책 조회 | active policy version과 전체 하위 rule | `uq_auth_policy_versions_active`, policy version FK | Redis policy snapshot, cache miss는 DB 전체 복원 |
...
| 회원가입 상태 | registration_id + status proof hash | PK + `uq_auth_registrations_status_proof` | Redis registration status projection, cache miss는 DB 확인 |
| 가상 인증 메시지 | challenge_id + version + issued 상태 | 개발·테스트 projection PK | cache하지 않고 조회마다 Challenge와 함께 검증 |
...
| 로그인 뒤 action 복구 | consumed Intent의 action payload reference | payload PK + expiry | 프론트엔드 응답 생성 중에만 메모리 복호화, cache 금지 |
| 재인증 proof 소비 | proof hash + user_id + session_id + purpose + 미소비 + expires_at | proof hash unique, session partial index | 사용하지 않음 |
| refresh | credential secret hash -> Session | credential secret unique | credential은 cache하지 않고 성공한 Session 상태만 Redis에 갱신 |
| access token Session 확인 | JWT의 `sid`로 Session 상태 확인 | Session PK | 공유 Redis projection 필수, cache miss는 DB 확인 |
| 사용자 인증 상태 | user_id의 active link/session과 UserAuthState | user/status 보조 인덱스 | CS 조회는 cache하지 않음 |
...
### Redis projection models

Redis에는 아래 세 projection만 둔다. PostgreSQL이 최종 원장이며 Redis 값만으로 Command의 성공을 확정하거나 일회성 credential을 소비하지 않는다.

| Projection | Key | 값 | 갱신 주체 | TTL·보존 기준 |
| --- | --- | --- | --- | --- |
| `SessionStatusProjection` | `auth:session-status:{sessionId}` | `user_id`, `session_id`, status, idle/absolute 만료 시각, status version, `revoked_until` | Session 생성·갱신·폐기 API와 `Auth.SessionRevoked` consumer | active는 최대 5분이며 Session 만료를 넘지 않음. revoked는 해당 Session의 최대 access JWT 만료까지 보존 |
| `AuthenticationPolicySnapshotProjection` | `auth:policy:active-version`, `auth:policy:snapshot:{version}` | active version과 해당 version의 전체 불변 정책 rule | `API.A.300-26`과 정책 변경 consumer | snapshot은 5분 TTL로 재검증하며 새 version 활성화 시 active pointer를 즉시 교체 |
| `RegistrationStatusProjection` | `auth:registration-status:{HMAC(statusToken)}` | registration ID, status, method별 검증 상태, row version, 만료 시각 | `API.A.300-03~06`, 만료 worker | status proof 만료를 넘지 않음. terminal 상태는 조회 보존 기간 종료 시 삭제 |

`RegistrationStatusProjection`의 key에는 status token 원문이 아니라 서버 비밀키로 계산한 HMAC만 사용한다. 같은 멱등 요청에서 status proof를 교체하면 이전 key를 제거하고 새 key를 만든다. 값에는 이메일, 휴대폰, Challenge code와 proof 원문을 넣지 않는다.

`AuthenticationPolicySnapshotProjection`은 version별 불변 snapshot과 active version pointer를 분리한다. 정책 변경은 PostgreSQL에 새 version을 commit한 뒤 새 snapshot을 저장하고 pointer를 교체한다. 이전 snapshot은 진행 중인 요청이 끝날 수 있도록 TTL까지 유지한다.

### 캐시 경계

- 모든 auth-service instance는 같은 Redis를 공유하고 위 세 projection의 key namespace를 분리한다.
- Session 상태 cache에는 `user_id`, `session_id`, status, idle/absolute 만료 시각, status version과 `revoked_until`만 둔다.
- 이메일/휴대폰 ciphertext 또는 hash, 비밀번호 hash, challenge verifier, refresh token hash는 Redis에 복제하지 않는다.
- AuthenticationIntent owner proof, Registration status token 원문, reset grant, ReauthenticationProof와 action payload는 Redis에 복제하지 않는다.
- cache TTL은 DB의 `idle_expires_at`과 `absolute_expires_at` 중 먼저 오는 시각을 넘을 수 없다. 로그아웃, 재사용 탐지와 사용자 인증 상태 변경 transaction은 commit된 outbox로 cache 갱신을 전송한다.
- cache miss는 DB 조회로 보완한다. cache hit만으로 Session 만료·폐기를 연장하거나 Registration 상태를 다음 단계로 진행하지 않는다.
- DB 장애 시 공개 탐색은 인증 DB를 호출하지 않는다. 보호 endpoint는 유효한 Session 상태 cache hit만 허용하고 cache miss를 active로 간주하지 않는다. token 발급, refresh, 인증 수단 변경과 운영자 작업은 fail closed한다.
- Redis 전체 소실 뒤에는 PostgreSQL의 active·revoked Session을 `session_id` 기준 page scan으로 다시 적재한다. 재동기화 중 DB fallback은 instance별 동시 32개와 query timeout 100ms로 제한하며 초과 요청은 `503`으로 종료한다.
- `Auth.SessionRevoked` consumer는 실패 시 1초부터 최대 30초까지 `지수 backoff`를 적용한다. Redis eviction은 사용하지 않고 active Session과 access JWT 만료 전 revoked Session을 합한 key 수의 70%에서 용량 경보를 발생시킨다.
...
- publisher는 lease를 획득한 `publishing` 행만 전송하고, 성공한 broker acknowledgement 뒤에만 `published`로 바꾼다. publish 후 상태 갱신 전에 장애가 나면 lease 만료 뒤 같은 `event_id`가 다시 전달될 수 있다.
- Context 감사와 다른 명시적 소비자는 `event_id`를 consumer idempotency key로 사용한다.
- `dead_letter` 전환은 자동 삭제가 아니다. paging/alert 대상이며 운영자가 원인을 확인한 뒤 재시도한다.
- 로그인 성공/실패, challenge 성공/실패, 잠금, Session 발급/갱신/로그아웃, refresh reuse, 비밀번호 reset, 인증 수단 연결/교체와 사용자 인증 상태 변경을 서로 다른 `event_type`으로 둔다. 가입 핵심 처리는 Integration Event 없이 API 트랜잭션으로 끝낸다.
...
- 같은 key와 같은 request hash는 같은 논리 실행과 resource ID를 가리킨다. bootstrap은 같은 Intent의 owner proof를 원자적으로 새로 발급하고, 회원가입 완료는 기존 IdentityLink와 Session 결과를 반환한다. refresh, 이메일 재인증과 휴대폰 교체 credential 전달 복구는 유효한 `replay_payload_ref`의 최초 암호화 응답을 재생한다.
...
- 일반 명령의 `processing` 레코드가 timeout을 넘기면 연결된 Aggregate와 결과 저장 여부를 확인한 뒤 복구한다. 회원가입 완료는 한 트랜잭션이므로 별도 가입 작업을 재개하지 않는다.
- login과 challenge 검증의 실패 횟수는 idempotency key가 없는 반복 요청마다 실제 시도로 센다.
- refresh, 이메일 재인증과 휴대폰 교체 완료는 같은 Session/scope/key/request hash와 허용 operation에 한해 전용 encrypted replay payload로 최초 credential 전달 응답을 재생한다. payload가 만료되면 재발급하거나 rotation을 추가하지 않고 재인증을 요구한다.

## 마이그레이션

### 생성 순서

1. `auth_policy_versions`, `auth_signing_key_metadata`와 두 정책 rule 테이블을 만들고 요구사항에 확정된 access 15분, refresh 14일, remember-me 30일, rotation 사용, 로그인 실패 5회를 seed한다. 잠금 구간/시간, Session idle/absolute TTL, 목적별 인증 제한은 운영 승인값을 migration 입력으로 확정한다.
2. `auth_identities`와 `auth_password_credentials`를 만든다.
3. `auth_verification_challenges`, `auth_verification_delivery_payloads`, `auth_action_intent_payloads`, `auth_authentication_intents`를 만든다. 다형 `context_id`에는 DB FK를 만들지 않고 context type/ID를 서비스에서 검증한다. `auth_virtual_verification_messages`는 별도 dev/test migration에서만 만든다.
4. `auth_identity_links`를 만든다.
...
7. `auth_idempotency_replay_payloads`, `auth_idempotency_records`, `auth_outbox_events`를 만들고 replay reference FK와 Registration의 `completion_idempotency_record_id` FK를 후속 `ALTER TABLE`로 연결한다.
...
- 기존 Auth DB의 role/permission 데이터는 새 인증 schema로 이관하지 않는다. 소유 업무 Context 또는 별도 인가 저장소의 이관 계획으로 분리한다.
- 기존 active Link의 `user_id`별 UserAuthState는 Context 사용자의 account status와 user version을 받아 backfill한다. 확인되지 않은 사용자를 임의 active로 두지 않으며 로그인/refresh cutover 전에 누락 0건을 검증한다.
- outbox dual-write를 먼저 켠 뒤 consumer 중복 제거를 확인하고 기존 직접 publish 경로를 제거한다.
- 이전 비동기 가입 구현이 존재한다면 신규 요청을 차단한다. 이미 User와 IdentityLink가 모두 확인된 행만 `completed`로 옮기고, 나머지는 `failed(LEGACY_REGISTRATION_INCOMPLETE)`로 종료해 새 가입을 요구한다.
- 동기 완료 상태와 proof 검증 코드를 먼저 배포한 뒤 in-flight 비동기 가입을 종료하고 가입 consumer와 Inbox 스키마를 제거한다.

### 배포와 rollback

- expand migration -> 호환 코드 배포 -> backfill -> 검증/인덱스 활성화 -> 기존 경로 제거 순서를 따른다.
- migration은 forward-only다. 동기 가입 완료 전환 뒤에는 신규 상태를 모르는 이전 버전으로 rollback하지 않고 roll-forward 복구한다. 새 인증 데이터를 삭제하는 down migration은 실행하지 않는다.
...
| `auth_virtual_verification_messages` | Challenge terminal/만료 즉시 ciphertext 제거, 메타데이터 24시간 | 같은 transaction에서 crypto-shred, 개발·테스트 정리 worker가 누락분 처리 |
...
| `auth_registrations` | completed 90일, failed/expired 30일 | terminal status 조회 보존 TTL이 끝나면 status proof hash를 먼저 제거하고, 이후 `user_id`와 process 메타데이터 삭제. 감사 event는 별도 보존 |
...
| `auth_action_intent_payloads` | 프론트엔드 전달 확인 또는 Intent 만료 즉시 ciphertext 제거, 메타데이터 24시간 | crypto-shred 뒤 batch hard delete |
...
| `auth_idempotency_records` | public Command 24시간 이상 | 진행 중 record와 Registration 응답 복구 TTL이 남은 record는 삭제하지 않는다. 최종 상태 Registration의 FK 참조는 정리 시 `ON DELETE SET NULL` |
| `auth_idempotency_replay_payloads` | 허용된 credential 전달 재시도 TTL 종료 즉시 ciphertext 제거, 메타데이터 24시간 | 전용 만료 worker와 KMS key 폐기 |
...
| `auth_signing_key_metadata` | retired 후 운영 감사 기간 | private key는 별도 key manager 정책으로 폐기하고 공개 metadata만 보존 |
...
| 인증 claim 최소화 | Session을 기준으로 `iss`, `sub`, `sid`, `aud`, `iat`, `exp`, `jti`만 발급하고 role/permission과 개인정보 제외 |
| 사용자 차단·비활성 | user_id별 UserAuthState user_version, 전체 Session 원자 폐기, 재로그인/refresh fail closed |
| 의미별 감사 event | Aggregate 변경과 같은 transaction의 민감정보 없는 OutboxEvent |
| 인증 서비스 책임 최소화 | 외부 `user_id`만 참조하며 이름/주소/프로필/약관 데이터를 저장하지 않음 |

## 확인 필요

- 첫 활성 정책의 실패 집계 구간, `lock_duration_seconds`, Session idle/absolute TTL을 운영 기준으로 확정해야 한다.
...
- 업무 Context 또는 별도 인가 저장소로 이전할 기존 role/permission 데이터의 이관·폐기 기준을 확정해야 한다.
