...
| `auth_registrations` | 이메일·휴대폰 검증, 사용자 계정 연동 대기, 연결, 자동 로그인 준비까지의 장기 작업 | `Registration` | Process Aggregate |
...
| `auth_inbox_messages` | Context 사용자 integration event의 중복 제거, 지연, 처리 결과 | 수신 메시지 인프라 레코드 | consumer+event별 1행 |
...
| `web_idle_ttl_seconds` | INTEGER | NOT NULL, CHECK `BETWEEN 1 AND 604800` | 웹 Session 유휴 TTL |
| `web_absolute_ttl_seconds` | INTEGER | NOT NULL, CHECK `web_absolute_ttl_seconds BETWEEN web_idle_ttl_seconds AND 31536000` | 웹 Session 최대 TTL |
...
| `internal_context_ttl_seconds` | INTEGER | NOT NULL, CHECK `BETWEEN 1 AND 86400` | 내부 context token TTL |
...
| `status` | VARCHAR(32) | NOT NULL, CHECK | `pending_verification`, `awaiting_user_link`, `linked`, `issuing_session`, `completed`, `failed`, `expired` |
...
| `user_id` | UUID | NULL, 외부 참조 | Context 사용자가 계정 연동 요청으로 전달한 사용자 식별자 |
| `verification_binding_id` | UUID | UNIQUE, NULL | 가입 인증 완료 snapshot 식별자 |
| `verification_registration_version` | BIGINT | NULL | 가입 인증 완료 snapshot에 고정한 Registration version. `row_version`의 후속 변경과 분리한다. |
| `verification_snapshot_hash` | BYTEA | NULL, CHECK 32 bytes | Registration과 Identity/Challenge version·검증 시각의 canonical hash |
| `verification_completed_event_id` | UUID | UNIQUE, NULL | Context 사용자에 발행한 가입 인증 완료 OutboxEvent |
| `link_request_id` | UUID | UNIQUE, NULL | Context 사용자가 발급한 사용자 계정 연동 업무 멱등 키 |
| `completion_idempotency_record_id` | UUID | UNIQUE, NULL | 최초 회원가입 완료 요청의 IdempotencyRecord. 테이블 생성 뒤 FK를 연결한다. |
...
| `link_accept_until` | TIMESTAMPTZ | NULL | Context 사용자의 계정 연동 요청 수락 기한 |
| `linked_at` | TIMESTAMPTZ | NULL | 두 IdentityLink 활성화 완료 시각 |
| `session_issue_until` | TIMESTAMPTZ | NULL | 연결 뒤 자동 로그인 Session 전달 기한 |
...
- 현재 Registration의 두 Challenge가 모두 존재하고 Challenge와 Identity가 verified이기 전에는 `awaiting_user_link`로 전환할 수 없다.
- `awaiting_user_link`에는 `verification_binding_id`, `verification_registration_version`, `verification_snapshot_hash`, `verification_completed_event_id`, `completion_idempotency_record_id`, `link_accept_until`이 모두 필요하다.
- 최초 완료 요청은 `completion_idempotency_record_id`를 한 번만 설정한다. 이후 같은 record를 찾은 요청만 상태를 진행할 수 있고 다른 key는 `IDEMPOTENCY_CONFLICT`로 거부한다.
- 최종 상태와 응답 복구 TTL이 지난 뒤에는 `completion_idempotency_record_id`를 NULL로 정리할 수 있다. 진행 중인 Registration에서는 지우지 않는다.
- `user_id`, `link_request_id`, `linked_at`, `session_issue_until`이 없으면 `linked`, `issuing_session`, `completed`가 될 수 없다. `user_id`는 최초 설정 후 변경할 수 없다.
- `link_request_id`는 Context 사용자 전체에서 전역 고유한 UUID다. Registration은 첫 유효 요청의 값을 고정하며 같은 Registration에 다른 `link_request_id`를 적용하지 않는다.
- `linked` 이후에는 두 Identity가 같은 `user_id`를 owner로 갖고 두 link가 모두 active여야 한다.
- 계정 연동 성공 트랜잭션은 `session_issue_until = min(expires_at, linked_at + 정책상 자동 로그인 발급 기간)`으로 한 번만 확정한다.
...
- `issuing_session`은 클라이언트의 완료 요청을 받은 뒤에만 진입한다. `linked` 또는 `issuing_session`에서 `session_issue_until`이 지나면 Registration을 `expired`로 닫고 새 credential을 발급하지 않는다. 이미 활성화된 IdentityLink는 유지하며 사용자는 일반 로그인을 이용한다.
...
| `source` | VARCHAR(32) | NOT NULL | 사용자 계정 연동, 담당 Context event, 운영 승인 등 변경 출처 |
...
- 일반 외부 event projection은 `operation = consumer_name`, `key_hash = HMAC(message_id)`를 사용할 수 있다. `User.AuthLinkRequested`와 `User.AuthLinkRejected`는 예외로 두며, 전송 중복은 Inbox의 `source_event_id`, 업무 중복은 `scope_hash = HMAC('user_auth_link_v1')`, `key_hash = HMAC(link_request_id)`, `request_hash = HMAC(canonical payload)`로 처리한다. Registration ID는 request hash와 `resource_id`에 넣어 같은 전역 ID를 다른 Registration에 재사용하면 충돌하도록 한다.
- 회원가입 완료 요청은 `operation=complete_registration`, `resource_type=Registration`, `resource_id=registration_id`로 저장한다. Registration마다 `processing` record는 하나만 허용한다.
- 회원가입 완료 요청의 최초 `202 Accepted`는 최종 결과가 아니다. 같은 record를 `processing`으로 유지하고 Registration이 `linked`가 된 뒤 같은 key 요청에서 Session 발급을 이어 간다. 최초 record 만료 시각은 `link_accept_until`보다 길게 잡고, 연결 성공 트랜잭션에서 `session_issue_until`과 응답 복구 기간을 포함하도록 연장한다. Session 전달 성공 또는 복구 불가 실패에서만 `completed` 또는 `failed`로 닫는다.

### `auth_idempotency_replay_payloads`

모바일 refresh, 이메일 재인증과 휴대폰 교체 완료의 같은 Idempotency-Key 재시도는 새 rotation을 만들지 않고 최초 credential 전달 결과를 돌려줘야 한다. 이 예외를 일반 IdempotencyRecord나 SessionCredential에 넣지 않고 별도 보안 레코드로 격리한다.
...
| 필드 | PostgreSQL 타입 | 제약 | 설명 |
| --- | --- | --- | --- |
| `consumer_name` | VARCHAR(64) | PK 일부 | 같은 이벤트를 소비하는 Handler 구분 |
| `source_event_id` | UUID | PK 일부 | transport 중복 제거 식별자 |
| `message_type` | VARCHAR(128) | NOT NULL | `User.AuthLinkRequested`, `User.AuthLinkRejected` 등 |
| `schema_version` | SMALLINT | NOT NULL | payload 계약 버전 |
| `business_key` | UUID | NOT NULL | Registration ID |
| `link_request_id` | UUID | NULL | 사용자 계정 연동 업무 멱등 키 |
| `causation_id` | UUID | NULL | Auth가 먼저 발행한 가입 인증 완료 이벤트 ID |
| `payload` | JSONB | NOT NULL | 재처리에 필요한 비민감 필드만 저장한 payload |
| `payload_hash` | BYTEA | NOT NULL, CHECK 32 bytes | canonical payload 충돌 검사용 hash |
| `process_status` | VARCHAR(16) | NOT NULL, CHECK | `received`, `deferred`, `processed`, `rejected`, `dead_letter` |
| `process_attempts` | INTEGER | NOT NULL, 기본 0 | 처리 시도 횟수 |
| `next_attempt_at` | TIMESTAMPTZ | NULL | deferred 재처리 시각 |
| `last_error_code` | VARCHAR(64) | NULL | 비민감 처리 실패 코드 |
| `received_at` | TIMESTAMPTZ | NOT NULL | 최초 수신 시각 |
| `processed_at` | TIMESTAMPTZ | NULL | 최종 처리 완료 시각 |

- `User.AuthLinkRequested`/`User.AuthLinkRejected` version 1은 `link_request_id`, `causation_id`, `business_key`가 모두 NOT NULL이어야 하고 `schema_version=1`이어야 한다. 요청 이벤트만 `user_id`, `user_auth_status`, `restriction_version`을 요구한다.
- transport 멱등성은 `consumer_name + source_event_id`, 업무 멱등성은 전역 고유 `link_request_id`로 분리한다. IdempotencyRecord는 Registration scope와 payload hash를 함께 저장한다.
- 같은 `source_event_id`가 다시 오면 저장된 결과를 반환한다. 다른 event ID에 같은 `link_request_id`와 같은 payload가 오면 기존 업무 결과를 재사용하고, payload가 다르면 보안 충돌로 거부한다.
- 가입 인증 완료 이벤트보다 계정 연동 요청이 먼저 도착하거나 Registration 저장이 잠시 보이지 않으면 `deferred`로 두고 backoff한다. 한도를 넘긴 메시지만 `dead_letter`로 보내며 자동 성공이나 자동 삭제로 바꾸지 않는다.
- 요청 payload에는 `registration_id`, `registration_version`, `link_request_id`, `user_id`, `user_auth_status`, `restriction_version`, verification binding과 시각만 허용한다. 거부 payload에는 사용자 계정 값 대신 공개 가능한 `reason_code`만 허용한다. 이메일, 휴대폰, 프로필, 동의 원문, challenge proof는 저장하지 않는다.
- 오래되었거나 binding이 일치하지 않거나 발행자 검증에 실패한 메시지는 Inbox를 `rejected`로 닫고 보안 감사 OutboxEvent만 남긴다. 정상 Registration 상태와 최초 클라이언트 IdempotencyRecord는 바꾸지 않는다.
- 유효한 요청의 연결 처리 또는 binding이 확인된 거부 처리는 Inbox, 업무 IdempotencyRecord, Registration, 결과 OutboxEvent, Inbox 최종 상태를 각각 하나의 로컬 트랜잭션에서 커밋한다. 연결 성공 경로만 Identity/IdentityLink와 UserAuthState를 함께 저장한다.
...
| `Registration` | `auth_registrations` | 가입 인증 완료 통지, 사용자 계정 연동, Session 전달 상태를 1행에 저장 | Context 사용자가 보낸 메시지 처리와 클라이언트 완료 요청의 Session 발급을 분리 |
...
| 수신 메시지 | `auth_inbox_messages` | Context 사용자의 integration event 수신·지연·처리 상태를 저장 | transport 중복 제거와 업무 멱등성을 분리 |
...
CREATE UNIQUE INDEX uq_auth_idempotency_registration_completion_processing
    ON auth_idempotency_records (operation, resource_type, resource_id)
    WHERE operation = 'complete_registration'
...
ALTER TABLE auth_inbox_messages
    ADD CONSTRAINT ck_auth_inbox_user_link_v1
    CHECK (
        message_type NOT IN (
            'User.AuthLinkRequested',
            'User.AuthLinkRejected'
        )
        OR (
            schema_version = 1
            AND link_request_id IS NOT NULL
            AND causation_id IS NOT NULL
        )
    );

CREATE INDEX idx_auth_inbox_ready
    ON auth_inbox_messages (
        process_status,
        next_attempt_at,
        received_at
    )
...
| `auth_registrations` | `uq_auth_registrations_status_proof`, `idx_auth_registrations_status_updated`, `idx_auth_registrations_verification_deadline`, `idx_auth_registrations_link_deadline`, `idx_auth_registrations_session_deadline` | 상태 proof 조회, 중단된 process 재처리와 가입 단계별 기한 만료 확정 |
| `auth_inbox_messages` | PK `(consumer_name, source_event_id)`, `idx_auth_inbox_ready`, `idx_auth_inbox_business_key` | Context 사용자 이벤트 중복 제거, 수신 복구, 지연 재처리와 업무 키 조회 |
...
| `RegistrationRepository` | `FindByID`, `FindByIDForUpdate`, `FindByStatusProofHash`, `FindByVerificationBindingID`, `FindByLinkRequestID`, `ClaimVerificationDeadlineBatch`, `ClaimLinkDeadlineBatch`, `ClaimSessionDeadlineBatch`, `Save` | registration_id, status proof hash, binding_id, link_request_id, 단계별 deadline | Registration |
...
| `InboxMessageRepository` | `Claim`, `ClaimReadyBatch`, `FindByBusinessKey`, `MarkDeferred`, `MarkProcessed`, `MarkRejected`, `MarkDeadLetter` | consumer + source_event_id, registration_id + link_request_id, process_status + next_attempt_at | 수신 메시지와 처리 결과 |
...
| 가입 인증 완료 통지 | 클라이언트 IdempotencyRecord 선점, Registration을 `pending_verification -> awaiting_user_link`로 변경하고 불변 verification version/binding, IdempotencyRecord 참조, 수락 기한, `EVT.A.300-35`, OutboxEvent 저장 | 두 Challenge와 Identity 모두 verified, 이벤트 ID와 snapshot hash 고정, Registration당 processing 완료 요청 1개 | commit 뒤 relay가 `Auth.RegistrationVerificationCompleted` version 1 발행. 다른 완료 key는 충돌로 거부 |
| 사용자 계정 연동 요청 성공 | Inbox/업무 IdempotencyRecord, Registration, 두 Identity/Link, UserAuthState, 연결 결과 outbox, 최초 클라이언트 IdempotencyRecord 갱신 | 발행자, causation, 불변 binding/version, 전역 고유 link_request_id, `user_id`, restriction version, `received_at <= link_accept_until` 검증 후 두 Link와 `linked_at`/`session_issue_until`을 원자적으로 확정 | active이면 `linked`와 client record TTL 연장, 제한 상태면 Link를 보존하고 Registration과 client record를 `failed(USER_AUTH_RESTRICTED)`로 닫음 |
| Context 사용자 거부 | Inbox/업무 IdempotencyRecord, Registration에 `link_request_id`를 고정하고 `awaiting_user_link -> failed`, 공개 failure code, `EVT.A.300-37`, 결과 outbox, 최초 클라이언트 IdempotencyRecord `failed` | 현재 불변 binding/version과 causation이 모두 일치하는 `User.AuthLinkRejected`만 업무 거부로 수락 | Identity, IdentityLink, UserAuthState를 쓰지 않음 |
| 잘못되거나 오래된 연동 메시지 | Inbox `rejected`, 보안 감사 outbox | 발행자, causation, binding/version 또는 고정 link_request_id 불일치 | Registration과 최초 클라이언트 IdempotencyRecord를 바꾸지 않음 |
| 자동 로그인 시작 | Registration `linked -> issuing_session`, 기존 클라이언트 IdempotencyRecord `processing` 유지 | Registration owner proof와 `session_issue_until`, 두 active Link 확인 | Authorization Source 장애면 기한 안에서 `issuing_session`에 유지하고, 기한 경과 시 `expired`로 종료 |
...
Context 사용자와의 가입 연동은 동기 호출이나 분산 transaction으로 묶지 않는다. Auth가 가입 인증 완료 상태와 OutboxEvent를 함께 저장하고, Context 사용자는 이를 소비해 `user_id`를 발급한 뒤 `User.AuthLinkRequested` version 1을 보낸다. Auth는 source event ID를 Inbox에서, 전역 고유 `link_request_id`를 IdempotencyRecord에서 각각 중복 제거한다. 같은 link request에 다른 `user_id` 또는 binding이 오거나 같은 Registration에 다른 link request가 오면 보안 충돌로 거부하며 사용자 계정 병합, 재귀속, 자동 삭제로 보정하지 않는다.

계정 연동 consumer는 SessionCredential을 발급하지 않는다. cookie와 refresh token 원문은 broker 응답으로 전달할 수 없으므로 Registration을 `linked`까지만 진행한다. 클라이언트가 같은 완료 API와 같은 Idempotency-Key를 다시 호출하면 `issuing_session`에서 AccessGrant와 Session을 만들고 응답 채널에 credential을 직접 전달한다. `User.AuthLinkRequested`가 가입 인증 완료 이벤트보다 먼저 도착하면 Inbox를 `deferred`로 유지한다. 수락 시각은 producer의 `occurred_at`이 아니라 Auth가 기록한 Inbox `received_at`으로 판정하며, `link_accept_until` 뒤의 요청은 fail closed로 거부한다. Context 사용자가 이미 만든 임시 사용자 계정의 실패 처리는 Context 사용자 책임이다.

예상 가능한 Identity 귀속 충돌은 두 Identity 행을 ID 순서로 잠근 뒤 Link 쓰기 전에 판정하고 같은 트랜잭션에서 거부 결과를 저장한다. 최종 DB unique constraint가 예외적으로 실패하면 PostgreSQL 트랜잭션 전체를 롤백한 뒤, 제약 이름과 잠긴 현재 상태를 다시 확인하는 별도 정합성 복구 트랜잭션에서 Inbox rejected, Registration 실패, 클라이언트 IdempotencyRecord 실패와 거부 OutboxEvent를 저장한다. 실패한 트랜잭션 안에서 거부 이벤트를 이어 쓰지 않는다.

### 회원가입 기한 만료

- 만료 worker는 `idx_auth_registrations_verification_deadline`에서 `pending_verification AND expires_at <= now()` 행을 가져와 Registration `expired(REGISTRATION_EXPIRED)`와 감사 OutboxEvent를 함께 저장한다. 이 처리로 active Identity 예약을 해제하며 owner 없는 Identity는 cooldown 뒤에만 재예약한다.
- 만료 worker는 `idx_auth_registrations_link_deadline`에서 `awaiting_user_link AND link_accept_until <= now()` 행을 `SKIP LOCKED`로 가져온다. 먼저 같은 Registration에 `received_at <= link_accept_until`인 `received`/`deferred` Inbox 메시지가 있는지 확인한다. 있으면 해당 메시지를 우선 처리하고 `link_accept_until + inbox_processing_grace`까지 만료를 유예한다. 유예 기간에도 처리되지 못하면 dead-letter와 운영 경보를 남긴 뒤 timeout을 확정한다.
- 기한 내 수신 메시지가 없거나 처리 유예가 끝나면 한 트랜잭션에서 Registration을 `expired(USER_LINK_TIMEOUT)`, 최초 클라이언트 IdempotencyRecord를 `failed`, `EVT.A.300-37`과 Context 사용자용 거부 OutboxEvent를 저장한다. link request가 없으므로 결과 이벤트의 `link_request_id`는 NULL이다.
- `linked` 또는 `issuing_session`의 `session_issue_until <= now()` 행은 별도 deadline index에서 가져온다. Registration을 `expired(SESSION_DELIVERY_EXPIRED)`, 최초 클라이언트 IdempotencyRecord를 `failed`, 감사 OutboxEvent를 함께 저장하고 기존 IdentityLink는 유지한다.
- worker와 Event/API Handler는 같은 Registration row lock과 조건부 update를 사용한다. 먼저 커밋한 쪽만 상태를 바꾸고 다른 쪽은 현재 최종 결과를 재사용한다.
- Inbox `dead_letter`는 Registration의 업무 결과가 아니다. 처리 메시지가 없어도 deadline worker가 반드시 클라이언트 `processing` record와 Registration을 종료한다.
...
2. 기존 SessionCredential을 `rotated_pending_delivery`로 닫고 채널에 맞는 대체 credential과 CSRF key version을 만든다. 동시에 목적 하나에 묶인 `auth_reauthentication_proofs`, completed IdempotencyRecord, 최초 proof·credential 전달 결과의 short-TTL replay ciphertext와 `auth.reauthentication.succeeded` 보안 감사 outbox를 저장한다.
...
- 번호 교체, 사용자 계정 연동 요청 반영처럼 여러 유일성 조건이 겹치는 transaction은 serialization failure와 deadlock을 제한 횟수만 재시도한다. 같은 Command ID와 request hash일 때만 재시도한다.
...
| 회원가입 상태 | registration_id + status proof hash | PK + `uq_auth_registrations_status_proof` | 짧은 상태 poll cache 가능, DB가 기준 |
...
- Context 감사와 Context 사용자는 `event_id`를 consumer idempotency key로 사용한다. Auth가 Context 사용자의 이벤트를 받을 때도 `consumer_name + source_event_id`로 transport 중복을 제거한다.
- `dead_letter` 전환은 자동 삭제가 아니다. paging/alert 대상이며 운영자가 원인을 확인한 뒤 재시도한다.
- 가입 인증 완료, 사용자 계정 연결/거부, 회원가입 완료, 로그인 성공/실패, challenge 성공/실패, 잠금, session 발급/갱신/로그아웃, refresh reuse, 비밀번호 reset, 인증 수단 연결/교체, 권한 변경을 서로 다른 `event_type`으로 둔다.
...
- 같은 key와 같은 request hash는 같은 논리 실행과 resource ID를 가리킨다. bootstrap은 같은 Intent의 owner proof를 원자적으로 새로 발급하고, 장기 실행 회원가입 완료는 `processing` 동안 현재 Registration 상태를 반환하고 최종 단계까지 진행한다. 모바일 refresh, 이메일 재인증과 휴대폰 교체 credential 전달 복구는 유효한 `replay_payload_ref`의 최초 암호화 응답을 재생한다.
...
7. `auth_idempotency_replay_payloads`, `auth_idempotency_records`, `auth_outbox_events`, `auth_inbox_messages`를 만들고 replay reference FK와 Registration의 `completion_idempotency_record_id` FK를 후속 `ALTER TABLE`로 연결한다.
...
- 기존 `requesting_user` 행은 새 가입 인증 완료 이벤트를 자동 발행하지 않는다. 기존 request ID로 Context 사용자 결과를 읽기 전용 조회해 사용자 생성 성공 여부를 먼저 확정한다. 결과가 없거나 불명확하면 행을 `failed(LEGACY_USER_RESULT_UNKNOWN)`로 격리하고 새 Registration 또는 수동 처리를 요구한다.
- 성공 결과가 확인된 기존 `requesting_user` 행은 검증 완료 당시 Challenge/Identity version으로 `verification_registration_version`, binding/hash, causation, deadline과 클라이언트 IdempotencyRecord 참조를 backfill한다. 브로커 발행자를 위조한 합성 이벤트를 만들지 않고, 배포 기간에만 허용한 `LegacyRegistrationLinkBackfillHandler`가 원래 request ID, 조회 증빙 hash, migration batch ID를 감사 outbox에 남기며 동일한 Identity 귀속 불변조건을 적용한다.
- 기존 `linking` 또는 `issuing_session` 행에 `user_id`가 저장되어 있으면 새 계정 연동 요청을 만들지 않는다. Link 정합성을 검증하고 클라이언트 IdempotencyRecord 참조와 `session_issue_until`을 backfill한다. 안전한 credential 전달 기한을 복원할 수 없으면 Registration을 `expired(SESSION_DELIVERY_EXPIRED)`로 닫고 IdentityLink는 유지해 일반 로그인을 요구한다.
- nullable 컬럼 확장, 구·신 상태를 모두 읽는 호환 코드, outbox/inbox dual consumer, in-flight drain, 상태/멱등 참조 backfill, 동기 Port 제거 순서로 전환한다.

### 배포와 rollback

- expand migration -> 호환 코드 배포 -> backfill -> 검증/인덱스 활성화 -> 기존 경로 제거 순서를 따른다.
- migration은 forward-only다. 호환 배포 기간에는 feature flag와 구·신 상태 변환 read/write path로만 애플리케이션을 되돌릴 수 있다. 비동기 cutover와 동기 Port 제거 뒤에는 신규 상태를 모르는 이전 버전으로 rollback하지 않고 roll-forward 복구한다. 새 인증 데이터를 삭제하는 down migration은 실행하지 않는다.
...
| `auth_idempotency_records` | public Command 24시간 이상, 외부 event 90일 | 진행 중 record와 Registration 응답 복구 TTL이 남은 record는 삭제하지 않는다. 최종 상태 Registration의 FK 참조는 정리 시 `ON DELETE SET NULL` |
...
| `auth_inbox_messages` | processed/rejected 후 90일 | terminal 행만 batch delete, deferred/dead-letter 자동 삭제 금지 |
