...
| `auth_inbox_messages` | 권한 원천 등 필요한 integration event의 중복 제거와 처리 결과 | 수신 메시지 인프라 레코드 | consumer+event별 1행 |
...
- 일반 외부 event projection은 `operation = consumer_name`, `key_hash = HMAC(message_id)`를 사용한다. 가입 완료는 Event가 아니라 API IdempotencyRecord로 처리한다.
...
| `message_type` | VARCHAR(128) | NOT NULL | role/permission 변경 등 실제 외부 Event type |
| `schema_version` | SMALLINT | NOT NULL | payload 계약 버전 |
| `business_key` | UUID | NOT NULL | 원천 업무 식별자 |
| `causation_id` | UUID | NULL | 원인 Event 식별자 |
...
- `consumer_name + source_event_id`로 전송 중복을 제거한다.
- 같은 event ID의 다른 payload는 `rejected`로 닫고 보안 감사 대상으로 남긴다.
- 가입 완료는 프론트엔드의 직접 호출로 처리하며 이 Inbox를 사용하지 않는다.
- 사용자 계정 상태 변경은 운영 프론트엔드의 동기 호출로 처리하며 이 Inbox를 사용하지 않는다.
...
| `auth_inbox_messages` | PK `(consumer_name, source_event_id)`, `idx_auth_inbox_ready`, `idx_auth_inbox_business_key` | role/permission 등 외부 Event 중복 제거와 재처리 |
...
| `InboxMessageRepository` | `Claim`, `ClaimReadyBatch`, `FindByBusinessKey`, `MarkDeferred`, `MarkProcessed`, `MarkRejected`, `MarkDeadLetter` | consumer + source_event_id, business key, process_status + next_attempt_at | 수신 메시지와 처리 결과 |
...
| 회원가입 완료 | verified Registration, User proof, IdempotencyRecord, 두 IdentityLink, Session/SessionCredential, AccessGrant | User proof의 registration ID와 user ID 일치, Identity owner 충돌 없음 | 한 트랜잭션에서 `completed`; 실패 시 전체 롤백 |
...
- 로그인 성공/실패, challenge 성공/실패, 잠금, session 발급/갱신/로그아웃, refresh reuse, 비밀번호 reset, 인증 수단 연결/교체, 권한 변경을 서로 다른 `event_type`으로 둔다. 가입 핵심 처리는 Integration Event 없이 API 트랜잭션으로 끝낸다.
...
- 같은 key와 같은 request hash는 같은 논리 실행과 resource ID를 가리킨다. bootstrap은 같은 Intent의 owner proof를 원자적으로 새로 발급하고, 회원가입 완료는 기존 IdentityLink와 Session 결과를 반환한다. 모바일 refresh, 이메일 재인증과 휴대폰 교체 credential 전달 복구는 유효한 `replay_payload_ref`의 최초 암호화 응답을 재생한다.
...
- 기존 비동기 가입 상태는 새 요청을 받지 않는다. 이미 User와 IdentityLink가 모두 확인된 행만 `completed`로 옮기고, 나머지는 `failed(LEGACY_REGISTRATION_INCOMPLETE)`로 종료해 새 가입을 요구한다.
- 새 상태 enum과 proof 검증 코드를 먼저 배포한 뒤 in-flight 비동기 가입을 종료하고 기존 가입 consumer를 제거한다.
