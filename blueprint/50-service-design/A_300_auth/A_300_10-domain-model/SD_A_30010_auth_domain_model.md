...
- `Registration`은 이메일과 휴대폰 소유 확인, 가입 인증 완료 통지, Context 사용자의 계정 연동 요청 반영, 자동 로그인을 하나의 장기 실행 작업으로 추적한다. `user_id` 발급과 사용자 계정 생성은 Context 사용자가 조정한다.
...
| `AGG.A.300-05` | Registration | 이메일·휴대폰 확인, 가입 인증 완료 통지, Context 사용자의 계정 연동 요청 반영, 자동 Session 발급을 멱등하게 추적한다. | 시작됨 -> 검증 대기 -> 사용자 연결 대기 -> 연결 -> 완료/실패/만료 |
...
| `channel` | VerificationChannel | MVP는 email_code, sms_code. 이메일 link는 별도 계약을 추가하기 전까지 사용하지 않는다. |
...
Registration은 여러 Aggregate와 Context에 걸친 가입 상태를 추적하는 영속 Process Manager다. Auth는 사용자 생성을 직접 요청하거나 조정하지 않는다. 두 소유 확인이 끝나면 가입 인증 완료 이벤트를 발행하고, Context 사용자가 `user_id`를 발급한 뒤 보낸 계정 연동 요청을 받아 IdentityLink와 Session 단계를 진행한다. 사용자 프로필을 소유하지 않고 외부 이벤트 상태와 opaque reference만 관리한다.
...
| `status` | RegistrationStatus | pending_verification, awaiting_user_link, linked, issuing_session, completed, failed, expired |
...
| `verification_binding_id` | ExternalReferenceId? | 가입 인증 완료 snapshot을 구분하는 불투명 식별자 |
| `verification_registration_version` | int64? | 가입 인증 완료 snapshot에 고정한 Registration version. 이후 `version`과 분리한다. |
| `verification_snapshot_hash` | Bytes32? | Registration과 두 Identity/Challenge version, 검증 시각을 묶은 canonical hash |
| `verification_completed_event_id` | ExternalEventId? | Context 사용자에 발행한 가입 인증 완료 이벤트 식별자 |
| `link_request_id` | ExternalRequestId? | Context 사용자가 발급한 사용자 계정 연동 업무 멱등 키 |
| `completion_idempotency_ref` | ExternalReferenceId? | 최초 회원가입 완료 요청의 IdempotencyRecord 불투명 참조 |
...
| `link_accept_until` | time.Time? | Context 사용자의 계정 연동 요청을 받을 수 있는 기한 |
| `linked_at` | time.Time? | 두 IdentityLink가 같은 `user_id`로 활성화된 시각 |
| `session_issue_until` | time.Time? | 연결 뒤 자동 로그인 Session을 전달할 수 있는 기한 |
...
- 현재 Registration에 연결된 email과 phone Challenge가 모두 존재하고 verified가 되기 전에는 가입 인증 완료 이벤트를 발행하지 않는다.
- method별 발급/재발급은 현재 Challenge 참조 하나만 설정한다. 재발급 시 이전 issued Challenge를 revoked로 닫은 뒤 새 ID로 교체한다.
- 가입 인증 완료 시 `verification_binding_id`, `verification_registration_version`, `verification_snapshot_hash`, `verification_completed_event_id`, `completion_idempotency_ref`를 한 번만 확정한다. 재발행은 같은 이벤트 ID와 payload를 사용한다.
- 최초 `202 Accepted` 이후에는 `completion_idempotency_ref`가 가리키는 같은 완료 요청만 Registration을 진행할 수 있다. 다른 멱등 키는 새 실행을 만들지 않고 충돌로 거부한다.
- Context 사용자의 `User.AuthLinkRequested` version 1은 전역 고유 `link_request_id`로 업무 멱등성을 보장한다. transport `event_id`는 Inbox에서 별도로 중복 제거하며, `causation_id`와 verification binding이 현재 Registration에 고정된 가입 인증 완료 snapshot과 일치해야 한다. 같은 Registration에 다른 `link_request_id`가 오면 충돌로 거부한다.
- 유효한 계정 연동 요청을 받은 뒤 두 Identity의 `owner_user_id`를 같은 `user_id`로 설정하고 active IdentityLink를 하나의 트랜잭션에서 만든다. Auth가 `user_id`를 생성하거나 다른 값으로 바꾸지 않는다.
- 두 IdentityLink가 활성화될 때 `linked_at`과 `session_issue_until`을 한 번만 확정한다. `session_issue_until`은 Registration 만료 시각을 넘을 수 없다.
- 연결 트랜잭션이 실패하면 Identity owner와 두 IdentityLink는 모두 변경되지 않는다. 같은 메시지 전체를 재시도하며 부분 성공이나 중복 Link를 남기지 않는다.
- Session은 두 Link가 모두 active이고 AccessGrant를 읽은 뒤 클라이언트의 완료 요청에서 정확히 한 번 발급한다. 비동기 이벤트 소비 중에는 cookie나 token 원문을 만들지 않는다.
...
stateDiagram-v2
    ...
    pending_verification --> awaiting_user_link: 가입 인증 완료 이벤트 발행
    awaiting_user_link --> linked: User.AuthLinkRequested 수신과 두 Link 활성
    linked --> issuing_session: 클라이언트 완료 요청
    issuing_session --> completed: Session 발급
    pending_verification --> expired: TTL 만료
    awaiting_user_link --> failed: binding 확인된 User.AuthLinkRejected 또는 Identity 귀속 충돌
    awaiting_user_link --> expired: link_accept_until 경과
    linked --> failed: 제한 사용자
    linked --> expired: session_issue_until 경과
    issuing_session --> expired: session_issue_until 경과
    completed --> [*]
    failed --> [*]
    expired --> [*]
```

오래되었거나 binding이 일치하지 않는 메시지는 Inbox와 보안 감사에서만 거부한다. Registration 상태와 `version`은 바꾸지 않는다.
...
| `VO.A.300-15` | RegistrationVerificationBinding | registration_id, registration_version, identity_ids, challenge_ids, challenge_versions, verified_at, binding_id, snapshot_hash, link_accept_until | 가입 인증 완료 이벤트와 Context 사용자의 계정 연동 요청을 같은 검증 snapshot에 묶는다. 이메일·휴대폰 원문과 challenge proof는 포함하지 않는다. |
...
| `STATE.A.300-07` | RegistrationStatus | pending_verification, awaiting_user_link, linked, issuing_session, completed, failed, expired | 가입 인증 완료 이벤트와 사용자 계정 연동 요청을 분리하고 완료 전 단계를 멱등하게 재시도한다. |
...
| `CMD.A.300-16` | 회원가입 진행 | Registration | 가입 인증 완료 통지와 클라이언트 Session 발급 단계 전진 |
...
| `CMD.A.300-25` | 회원가입 사용자 연결 반영 | Registration/Identity/IdentityLink/UserAuthState | Context 사용자의 계정 연동 요청을 검증하고 두 IdentityLink를 활성화하거나 거부 |
...
| `EVT.A.300-35` | 회원가입 인증 완료됨 | 두 가입 Challenge가 verified이고 RegistrationVerificationBinding 확정 | Context 사용자에 가입 인증 완료 integration event로 전달 |
| `EVT.A.300-36` | 회원가입 사용자 연결됨 | Context 사용자의 계정 연동 요청에 따라 두 IdentityLink 활성 | Context 사용자에 연결 완료 사실 전달 |
| `EVT.A.300-37` | 회원가입 사용자 연결 거부됨 | binding이 확인된 사용자 생성 거부, 계정 연동 기한 만료 또는 유효한 요청의 Identity 귀속 충돌 | Context 사용자에 일반화된 거부 사유 전달. 오래되었거나 binding이 일치하지 않는 메시지는 Registration을 바꾸지 않고 Inbox/보안 감사에서만 거부한다. |

`BC.A.300`의 `EVT.A.300-12~17`, `EVT.A.300-21`처럼 이름에 `감사 전송됨`이 붙은 항목은 Aggregate가 만드는 Domain Event가 아니라 outbox 전달 결과다. 서비스/영속성 설계에서 위 `EVT.A.300-22~37` Domain Event를 Audit 또는 Context 사용자 integration event로 변환하고 전달 상태를 관리한다.
...
| Rule ID | 이름 | 규칙 | 적용 대상 |
| --- | --- | --- | --- |
| `RULE.A.300-01` | 이메일과 휴대폰 모두 확인 | 이메일 회원가입은 두 소유 확인이 모두 끝나야 Context 사용자에 가입 인증 완료 이벤트를 발행한다. | Registration |
...
| `RM.A.300-05` | 회원가입 상태 | 소유 확인, 가입 인증 완료 통지, 사용자 연결, Session 단계 제공 | 이메일/휴대폰 원문 제외 |
...
| `FR-004~007`, `FR-027`, `NFR-018` | Registration, Identity, PasswordCredential, VerificationChallenge | 가입 인증 완료 이벤트, Context 사용자의 계정 연동 요청, 동의/프로필 참조와 멱등성 |
...
- Context 사용자의 `User.AuthLinkRequested`/`User.AuthLinkRejected` version 1 발행, 만료와 재처리 contract.
