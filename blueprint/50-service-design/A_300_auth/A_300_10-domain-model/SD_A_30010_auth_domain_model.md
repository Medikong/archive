...
- 두 Identity owner, active IdentityLink, Session, AccessGrant와 Registration `completed`를 하나의 트랜잭션에서 저장한다.
...
| Ingress -> 서비스 | mTLS와 외부에서 들어온 내부용 헤더가 제거된 요청 | 사용자 credential 또는 검증된 signed request context를 대상 서비스가 확인하고, Ingress는 업무 Principal을 임의 생성하지 않는다. |
...
- registration completion proof와 user creation proof의 TTL, 키 회전과 검증 실패 contract.
