...
| 인증 허용 상태 | active IdentityLink와 UserAuthState를 확인한다. |
| 노출 범위 | public |
...
- 보장하는 업무 결과: credential, Link와 사용자 인증 허용 상태를 확인하고 정확히 하나의 논리 Session을 발급한다.
...
- 웹은 access JWT와 HttpOnly refresh cookie를 받고 auth-flow cookie를 즉시 폐기한다. 모바일은 access JWT와 opaque refresh token을 받는다.
...
4. 실패 count 초기화, Session·refresh credential, Intent 소비와 감사 OutboxEvent를 저장한다.
...
| 저장 모델 | 전략 | 적용 근거 |
| --- | --- | --- |
| `Identity`, `PasswordCredential`, `IdentityLink`, `UserAuthState` | 우회 | 비밀번호, 잠금과 사용자 제한은 cache 차이로 계정 상태가 노출되지 않도록 PostgreSQL에서 확인한다. |
| `AuthenticationPolicySnapshotProjection` (`P`) | 사용 | 로그인 잠금, Session과 token TTL 정책은 요청마다 공통으로 조회된다. |
| `SessionStatusProjection` (`S`) | 갱신 | 로그인 성공 transaction에서 생성한 Session을 commit 후 `active`로 적재한다. |

## 상태 변경과 트랜잭션

- 시작 상태: active 사전 인증 Intent와 로그인 가능한 email Identity.
- 성공 종료 상태: active Session·SessionCredential과 소비된 Intent.
...
| Session·인증 저장소 장애 | Session을 부분 발급하지 않는다. | `Retry-After`에 따라 같은 key로 재시도한다. |
...
| Aggregate / Entity | `Identity`, `PasswordCredential`, `IdentityLink`, `Session`, `SessionCredential`, `AuthenticationIntent` |
| Repository / Read Model | `IdentityRepository`, `IdentityLinkRepository`, `SessionRepository`, `IdempotencyRecordRepository` |
| Port / Adapter | `PasswordHasherPort`, `AccessTokenIssuerPort`, `RateLimitPort` |
...
- 로그인 p50/p95/p99, 실패율, lock rate와 token 발급 실패율을 집계한다.
...
- 웹 응답에는 access JWT와 refresh cookie만, 모바일 응답에는 access JWT와 refresh token만 전달된다.
