...
| --- | --- | --- | --- |
| 운영 사용자·운영자 API | [openapi/openapi.yaml](openapi/openapi.yaml) | `API.A.300-01~29` | 운영 서버와 클라이언트 생성 입력 |
...
6. `user_id`는 Context 사용자가 발급한다. Auth는 가입 인증 완료 이벤트를 발행하고, Context 사용자가 보낸 멱등 계정 연동 요청으로만 `user_id`를 받는다. 인증 API는 검증된 Identity를 해당 `user_id`에 연결하며 사용자 계정을 병합하지 않는다.
...
- 같은 key와 같은 hash는 같은 논리 실행과 결과 참조를 재사용한다. 일반 명령은 완료된 결과를 재생하지만, 장기 실행인 `API.A.300-06`은 `processing` 동안 현재 Registration 상태를 반환하고 같은 실행을 `202`에서 최종 `200`까지 진행한다.
...
| `AUTH_USER_LINK_REJECTED` | `409` | binding이 확인된 Context 사용자 업무 거부 또는 유효한 요청의 Identity 귀속 충돌 | 공개 사유를 일반화하고 새 가입 또는 지원 절차를 안내한다. 오래되었거나 binding이 일치하지 않는 메시지는 공개 Registration 오류로 바꾸지 않는다. |
| `AUTH_USER_LINK_TIMEOUT` | `410` | 계정 연동 요청 수락 기한 만료 | 새 Registration으로 다시 시작한다. |
...
| `API.A.300-03~06`, `API.A.300-28` | 가입 시작, challenge 발급/검증, 가입 인증 완료 통지, `CMD.A.300-25` 사용자 연결 반영, Session 완료/상태 조회 | Registration, VerificationChallenge, Identity, IdentityLink, PasswordCredential, UserAuthState, Session | `EVT.A.300-03~07`, `EVT.A.300-22`, `EVT.A.300-24`, `EVT.A.300-33`, `EVT.A.300-35~37`, `User.AuthLinkRequested` version 1 |
...
Auth가 Context 사용자에 발행하는 이벤트:

```json
{
  "eventId": "evt_01JXYZ...",
  "eventType": "Auth.RegistrationVerificationCompleted",
  "eventVersion": 1,
  "occurredAt": "2026-07-10T08:00:00Z",
  "aggregateId": "reg_01JXYZ...",
  ...
  "data": {
    ...
    "assuranceSummary": {
      "emailVerifiedAt": "2026-07-10T07:58:00Z",
      "phoneVerifiedAt": "2026-07-10T07:59:00Z"
    },
    "linkAcceptUntil": "2026-07-10T08:30:00Z"
  }
}
```

Context 사용자가 Auth에 보내는 계정 연동 요청:
...
```json
{
  ...
  "aggregateId": "usr_01JXYZ...",
  "correlationId": "30d9fa85-0a18-4263-98b6-231dca5a6fb8",
  "causationId": "evt_01JXYZ...",
  "data": {
...
- `User.AuthLinkRejected` version 1은 같은 envelope과 `causationId`, `authRegistrationId`, `registrationVersion`, verification binding, 전역 고유 `linkRequestId`, 공개 가능한 `reasonCode`를 전달한다. `userId`, `userAuthStatus`, `restrictionVersion`은 포함하지 않는다.
- `registrationVersion`은 가입 인증 완료 시 binding에 고정한 snapshot version이다. Context 사용자는 받은 값을 그대로 되돌려주며 Auth의 이후 row version으로 바꾸지 않는다. `linkAcceptUntil` 판정에는 producer 시각이 아니라 Auth Inbox의 `receivedAt`을 사용한다.
- 오래되었거나 producer·causation·binding/version이 일치하지 않는 메시지는 Inbox와 보안 감사에서만 거부한다. 정상 Registration 상태를 실패로 바꾸거나 아래 업무 결과 이벤트를 발행하지 않는다.
- broker ACL 또는 workload identity로 producer를 검증하고 `eventId`와 `linkRequestId`를 서로 다른 멱등 키로 처리한다.
- 이벤트에는 이메일·휴대폰 원문, challenge proof, 비밀번호, Session credential을 넣지 않는다.

연결 성공 결과:

```json
{
  "eventId": "evt_01JAUTHLINKED...",
  "eventType": "Auth.RegistrationUserLinked",
  "eventVersion": 1,
  "occurredAt": "2026-07-10T08:00:02Z",
  "aggregateId": "reg_01JXYZ...",
  "correlationId": "30d9fa85-0a18-4263-98b6-231dca5a6fb8",
  "causationId": "evt_01JUSER...",
  "data": {
    "authRegistrationId": "reg_01JXYZ...",
    "registrationVersion": 5,
    "linkRequestId": "ulnk_01JXYZ...",
    "userId": "usr_01JXYZ...",
    "linkedAt": "2026-07-10T08:00:02Z"
  }
}
```

업무 거부 또는 연동 기한 만료 결과:

```json
{
  "eventId": "evt_01JAUTHREJECTED...",
  "eventType": "Auth.RegistrationUserLinkRejected",
  "eventVersion": 1,
  ...
  "data": {
    ...
    "linkRequestId": null,
    "userId": null,
    "reasonCode": "LINK_TIMEOUT",
    "rejectedAt": "2026-07-10T08:30:00Z"
  }
}
```

- `Auth.RegistrationUserLinkRejected`의 `reasonCode`는 `USER_CREATION_REJECTED`, `IDENTITY_OWNERSHIP_CONFLICT`, `LINK_TIMEOUT`만 공개한다. 기한 만료는 받은 연동 요청이 없으므로 `linkRequestId`와 `userId`가 NULL이다.
- Context 사용자는 성공 결과로 임시 사용자 계정을 활성화하고, 거부 결과로 실패·보상 처리를 수행한다.
...
- Controller, Application Service, Repository, 가입 인증 완료 publish, 사용자 계정 연동 consume, Outbox/Inbox 저장을 별도 span으로 만든다.
...
- Context 사용자 Event 처리 지연, Inbox deferred/dead-letter와 Auth transaction 실패를 구분해 가입 지연 원인을 확인할 수 있어야 한다.
...
- Context 사용자 또는 Event channel 지연은 Registration을 `awaiting_user_link`로 유지하고 `202` 상태 조회로 표현한다. Auth는 기다리는 동안 새 `user_id`를 임의 생성하지 않는다.
...
| 같은 가입 key와 같은 body 재요청 | 같은 Registration과 현재 상태 반환, 가입 인증 완료 event·IdentityLink·Session 중복 생성 없음 |
| 같은 `User.AuthLinkRequested` event 재전달 | Inbox 결과 재사용, Identity owner와 Link 변화 없음 |
| 같은 link request에 다른 `user_id` 전달 | `AUTH_IDEMPOTENCY_CONFLICT`, 재귀속·병합 없음 |
