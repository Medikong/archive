...
| `X-CSRF-Token` | 웹 사전 인증과 refresh·로그아웃 | `__Host-dm_auth` 또는 `__Host-dm_refresh` cookie에 묶인 CSRF token을 보낸다. Bearer 보호 API에는 적용하지 않는다. |
...
```http
Set-Cookie: __Host-dm_refresh=<opaque-random>; Path=/api/v1/auth/sessions; HttpOnly; Secure; SameSite=Strict
...
- 이후 웹 사전 인증과 refresh·logout은 `Origin` allowlist, Fetch Metadata, `X-CSRF-Token`, `__Host-dm_auth` 또는 `__Host-dm_refresh` 바인딩을 검증한다. Bearer 보호 API에는 CSRF 검사를 적용하지 않는다.
