...
- 역할: Context 인증의 REST API, 이벤트 형식, 요청/응답, 오류, 보안 경계를 정의한다.
...
`archive` 저장소 루트에서 다음 명령으로 참조를 포함한 명세 전체를 검증한다.
...
운영 배포 검증은 운영 bundle에 `/api/v1/dev/` 경로가 있으면 실패해야 한다. 개발 Gateway는 개발 bundle을 별도로 사용하고 `DevAccessToken`과 소유 인증 방식의 AND 조건을 적용하며, 실패 응답도 OpenAPI의 `404 ErrorResponse` 형식을 따른다.
...
1. 웹과 모바일은 모두 짧은 수명의 access JWT를 보호 API의 Bearer credential로 사용한다.
2. Auth는 두 채널의 Session과 회전형 opaque refresh token을 서버에서 관리한다. 웹 refresh token은 HttpOnly cookie로, 모바일 refresh token은 플랫폼 보안 저장소로 전달한다.
3. 이메일, 휴대폰 번호, 인증번호, 비밀번호, token 원문은 JWT claim, 내부 `X-User-*` 헤더, 로그, trace, Domain Event에 넣지 않는다.
4. 공개 인증 API는 계정 존재 여부를 직접 알려주지 않는다. 상세 실패 원인은 별도 인가 경계에서 승인된 운영자 Query와 감사 이벤트에서만 확인한다.
...
7. 상태 변경 API는 명시적인 멱등성 규칙을 사용한다. 인증번호 재전송처럼 사용자가 새 실행을 의도한 경우에는 새 `Idempotency-Key`를 사용한다.
8. 운영자 수동 처리는 외부 인가 결정, 최근 강한 인증, 승인 참조, 사유, 낙관적 동시성 검사를 모두 통과해야 한다.
9. API transaction과 integration event 발행은 `OutboxEvent`를 이용한다. 외부 소비자는 at-least-once 전달을 전제로 `event_id` 기준 멱등성을 보장한다.
10. MVP 이메일·SMS 소유 확인은 모두 사용자가 인증번호를 입력하는 code 방식을 사용한다. 링크·provider proof는 별도 방식을 정의하기 전까지 받지 않는다.
...
API 응답은 모두 `Cache-Control: no-store`로 외부 저장을 금지한다. 이 규칙과 auth-service가 PostgreSQL 부하를 줄이기 위해 사용하는 내부 Redis projection은 별개다.

| 분류 | Redis projection | 적용 대상 | 원칙 |
| --- | --- | --- | --- |
| `S` | `SessionStatusProjection` | 보호 API의 Session 확인, 로그인·refresh·로그아웃·Session 폐기 | 읽기, 갱신 또는 revoked 상태 반영. key 삭제만으로 폐기를 표현하지 않음 |
| `P` | `AuthenticationPolicySnapshotProjection` | 로그인, Challenge, Session, 인증 수단 정책 조회 | version별 불변 snapshot을 읽고 정책 변경 뒤 active version 교체 |
| `R` | `RegistrationStatusProjection` | 회원가입 생성·Challenge 처리·완료·상태 조회 | status proof HMAC key를 사용하고 Registration 상태 변경 뒤 갱신 |

각 API 문서의 `저장 모델과 캐시` 절은 다음 전략을 구분한다.

- `사용`: Redis projection을 먼저 읽고 캐시에 값이 없을 때 PostgreSQL에서 복원한다.
- `갱신`: PostgreSQL 커밋 결과를 Redis에 반영한다.
- `무효화`: Session은 삭제하지 않고 `revoked`로 덮어쓴다. 교체된 proof처럼 다시 사용할 수 없는 key만 제거한다.
- `우회`: 보안 판정, row lock과 1회 소비가 필요해 캐시를 판단 근거로 사용하지 않는다.
- `사용하지 않음`: 반복 조회 효과가 없거나 민감정보 복제 위험이 더 큰 데이터다.

Redis 구조와 보존 기준은 [영속성 설계의 Redis projection models](../A_300_20-persistence/README.md#redis-projection-models)를 따른다. rate limit counter는 만료 시간이 있는 보안 상태이고 `IdempotencyRecord`는 정확성 모델이므로 이 캐시 분류에 포함하지 않는다.
...
| `X-Refresh-Token` | 모바일 refresh·로그아웃 | 회전형 opaque refresh credential이다. URL, query, 로그와 request body에 넣지 않는다. |
...
| `X-CSRF-Token` | 웹 사전 인증과 refresh·로그아웃 | `__Host-dm_auth` 또는 `__Secure-dm_refresh` cookie에 묶인 CSRF token을 보낸다. Bearer 보호 API에는 적용하지 않는다. |
| `Authorization: Bearer <access-jwt>` | 웹·모바일 보호 API | Auth가 발급한 짧은 수명 access JWT를 보낸다. |
| `X-Authorization-Decision` | 운영자 API | 별도 인가 경계가 action·subject·resource·만료를 서명한 결정 proof다. Auth는 role이나 permission을 직접 조회하지 않는다. |
...
오류 응답은 `application/json`을 사용한다.
...
`ErrorResponse`는 모든 서비스에서 재사용할 수 있도록 `code`, `status`, `message`, `requestId`만 제공한다. `code`는 서비스별 namespace를 사용할 수 있는 안정적인 값이고, `status`는 실제 HTTP 상태와 같아야 한다. `message`에는 이메일, 휴대폰 번호, 사용자 존재 여부, 내부 식별자, credential, stack trace와 내부 오류 원인을 넣지 않는다.

임의의 `details` 객체는 공개 정보 범위를 넓히므로 두지 않는다. 서비스별 차이는 `code`로 구분하고 재시도 시각은 `Retry-After`, 요청 추적은 `X-Request-Id`처럼 의미가 정해진 HTTP header로 확장한다. 새 공통 필드가 필요하면 개별 API가 임의로 추가하지 않고 `ErrorResponse` version을 올려 함께 반영한다.
...
웹 로그인과 회원가입 완료 응답은 access JWT를 본문으로 반환하고 refresh cookie를 설정하며 사전 인증 cookie를 같은 응답에서 폐기한다.

```http
Set-Cookie: __Secure-dm_refresh=<opaque-random>; Path=/api/v1/auth/sessions; HttpOnly; Secure; SameSite=Strict
Set-Cookie: __Host-dm_auth=; Path=/; HttpOnly; Secure; SameSite=Lax; Max-Age=0
```

- refresh cookie는 하위 경로로 제한하므로 `__Secure-` prefix를 사용하고 `Domain` 속성을 지정하지 않는다.
- 사전 인증 cookie는 `Path=/`, `Secure`, `Domain` 미지정 조건을 지키며 `__Host-` prefix를 유지한다.
- access JWT는 웹앱 메모리에만 보관하고 LocalStorage, SessionStorage와 cookie에 저장하지 않는다.
- refresh token은 JavaScript에 노출하지 않고 refresh·logout 경로에만 전송되는 cookie로 제한한다.
- `rememberMe`는 refresh family의 absolute TTL만 선택하며 access JWT TTL은 늘리지 않는다.
- refresh와 logout은 ambient cookie를 사용하므로 `Origin`, Fetch Metadata와 cookie에 묶인 `X-CSRF-Token`을 검증한다.
- 보호 API는 Bearer JWT를 사용하므로 로그인 Session cookie와 Session-bound CSRF token을 사용하지 않는다.
...
웹과 모바일 access JWT의 allowlist는 다음과 같다.
...
신뢰 경계는 외부 요청의 `X-User-*`, `X-Session-*`, `X-Token-*` 헤더를 제거한 뒤 검증된 JWT와 active Session으로 다음 헤더만 생성한다.
...
`X-User-Email`, role, permission, membership과 업무 ACL 헤더는 사용하지 않는다. 내부 서비스가 사용자 프로필이나 인가 정보가 필요하면 해당 소유 Context를 별도로 조회한다.
...
모든 Endpoint는 요청·응답·헤더·오류의 정확한 구조와 wire 예시를 OpenAPI에서 관리하고 개별 Markdown에는 책임, 트랜잭션, 멱등성, 보안 근거와 운영 판단만 둔다. 이 README는 모든 API에 적용되는 설계 원칙, 공통 HTTP 정책, 보안 정책, 시퀀스, 이벤트 형식과 운영 기준을 정의한다.

## 오류 형식

| Error Code | HTTP | 공개 조건 | 처리 |
| --- | --- | --- | --- |
| `AUTH_INPUT_INVALID` | `400` | 요청 schema 또는 형식 오류 | 공개 message는 잘못된 입력값과 내부 field binding을 포함하지 않는다. |
...
| `AUTH_FORBIDDEN` | `403` | 별도 인가 경계의 decision proof 누락·만료·대상 불일치 | 리소스 존재 여부를 추가로 노출하지 않는다. |
...
- 이후 웹 사전 인증과 refresh·logout은 `Origin` allowlist, Fetch Metadata, `X-CSRF-Token`, `__Host-dm_auth` 또는 `__Secure-dm_refresh` 바인딩을 검증한다. Bearer 보호 API에는 CSRF 검사를 적용하지 않는다.
...
- refresh token, reset grant, ReauthenticationProof 원문은 hash 또는 서버측 reference로 검증한다. Auth는 role/permission/membership snapshot을 저장하지 않는다.
...
| --- | --- | --- |
| [`SCN.A.01-01`](../../../80-sequence/A_01_user/SCN_A_01_01_user_provisioning_auth_link.md) | 회원가입과 자동 로그인 | `API.A.300-03~06`, `API.A.300-28`, `API.A.01-01` |
| [`SCN.A.01-04`](../../A_01_user/A_01_50-sequence/SCN_A_01_04_change_user_status.md) | 사용자 계정 상태 변경과 Session 폐기 | `API.A.300-31` |
| [`SCN.A.300-02`](../A_300_50-sequence/SCN_A_300_02_phone_signin_refresh.md) | 휴대폰 로그인과 refresh token 회전 | `API.A.300-08`, `API.A.300-09`, `API.A.300-14` |
| [`SCN.A.300-03`](../A_300_50-sequence/SCN_A_300_03_phone_replacement.md) | 휴대폰 번호 교체 | `API.A.300-17`, `API.A.300-21~23` |
| [`SCN.A.300-05`](../A_300_50-sequence/SCN_A_300_05_web_jwt_authentication.md) | 웹 JWT 로그인, 보호 API 인증, refresh와 로그아웃 | `API.A.300-01`, `API.A.300-07`, `API.A.300-14~16` |
| [`SCN.A.310-01`](../A_300_50-sequence/SCN_A_310_01_password_reset.md) | 비밀번호 재설정 | `API.A.300-10~13` |
...
| `API.A.300-16` | 현재 인증 컨텍스트 조회 | Session read model, IdentityLink 요약 | event 없음 |
...
- 이미 서명이 검증된 access JWT라도 Session 상태를 확인할 수 없으면 보호 API 요청을 인증 성공으로 처리하지 않는다.
...
- JWT claim과 REST response schema는 서로 독립적으로 버전을 관리한다. REST version 변경이 JWT claim 확장을 자동으로 의미하지 않는다.
...
| 웹 로그인 성공 | access JWT를 body로 반환하고 opaque refresh token은 HttpOnly cookie로만 전달 |
...
| `auth.policy.write` 외부 인가 결정 없이 정책 변경 시도 | `403 AUTH_FORBIDDEN` |
...
- Session idle/absolute TTL과 `rememberMe`별 refresh family TTL을 확정한다.
- ReauthenticationProof 목적 목록과 최근 인증 허용 시간을 확정한다.
- 운영자 수동 처리 approval provider와 `evidenceRef` 접근 권한을 확정한다.
- 웹 보호 API는 Istio Ingress가 access JWT와 Session 폐기 상태를 검증하고 최소 사용자 컨텍스트를 만든다. Ingress는 업무 권한을 판단하거나 업무 응답을 조합하지 않는다. 세부 처리 순서는 `SCN.A.300-05`를 따른다.
