...
| API 유형 | Command |
| 인증 | 웹·모바일 공통 Bearer access JWT와 `replace_phone` 목적 ReauthenticationProof |
...
| 저장 모델 | 전략 | 적용 근거 |
| --- | --- | --- |
| `SessionStatusProjection` | 사용 | 휴대폰 교체를 요청한 사용자의 Session이 아직 유효한지 먼저 확인한다. |
| `AuthenticationPolicySnapshotProjection` | 사용 | reauth proof 수명과 휴대폰 교체 정책을 활성 정책 version으로 검사한다. |
| `ReauthenticationProof`, `Identity`, `IdentityLink`, `IdempotencyRecord` | 우회 | proof 소비, 기존 휴대폰 소유권과 새 번호 중복 여부를 PostgreSQL 트랜잭션으로 확정한다. |
