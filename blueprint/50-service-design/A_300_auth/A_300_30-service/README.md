...
PAGE.A.300의 회원가입 화면이 이름, 추천인 코드, 동의와 인증 입력을 한 번에 받더라도 서비스 경계까지 하나일 필요는 없다. 사용자 프로필·추천인 입력은 Context 사용자가 등록 초안으로 보관하고 `profile_request_id`를 발급한다. 동의 담당 Context는 `agreement_receipt_id`를 발급한다. Context 인증은 이 불투명 참조와 최종 `user_id`만 사용한다.
...
| `SVC.A.300-02` | RegistrationService | 가입 시작, 검증 완료 통지, Context 사용자의 계정 연동 요청 반영과 클라이언트 완료 요청의 자동 로그인을 조정한다. | Registration, Identity, IdentityLink, PasswordCredential, UserAuthState, Session | Context 사용자 이벤트, Authorization Source |
...
| `SVC.A.300-11` | UserAuthStateService | Context 사용자 제한·비활성 version을 반영하고 새 인증/refresh gate와 전체 Session 폐기를 관리한다. | UserAuthState, AccessGrant, Session | Context 사용자 |
...
| `UserAuthStateRepository` | 사용자별 restriction version과 active/restricted/deactivated 상태 저장 |
...
| `CMD.A.300-03 이메일 회원가입 시작` | `StartEmailRegistrationHandler` | profile_request_id, agreement_receipt_id, email, password, phone, client_action_id | Intent TTL 연장, Registration 상태 proof, Identity, PasswordCredential 생성 또는 미귀속 예약 재사용 | 로컬 1회 |
...
도메인 모델에서 회원가입 장기 실행 작업, 비밀번호 재설정, 권한 반영과 일괄 세션 폐기를 다음 Command로 보강했다. BC.A.300의 Element Catalog를 갱신할 때 같은 식별자와 이름을 사용한다.

| Command | Handler | 입력 핵심값 | 처리 결과 | 요구사항 |
| --- | --- | --- | --- | --- |
| `CMD.A.300-16 회원가입 진행` | `AdvanceRegistrationHandler` | registration id, client action id | 가입 인증 완료 통지 또는 클라이언트 완료 요청의 Session 단계를 현재 상태부터 재개 | `FR-005`, `FR-007` |
...
| `CMD.A.300-25 회원가입 사용자 연결 반영` | `ApplyRegistrationUserLinkHandler` | User.AuthLinkRequested/User.AuthLinkRejected event | 두 가입 IdentityLink 활성화 또는 Registration 거부 | `FR-005`, `FR-007` |

Context 사용자의 차단·비활성 이벤트는 `HandleUserAccountRestrictedHandler`가 source event/version으로 멱등 소비한다. UserAuthState를 restricted/deactivated로 저장하고 active AccessGrant와 해당 사용자의 전체 Session을 같은 transaction에서 폐기한다. 더 높은 version의 명시적 해제 event만 active로 되돌릴 수 있다.

`ApplyRegistrationUserLinkHandler`는 Context 사용자의 계정 연동 요청과 업무 거부를 처리한다. `event_id`는 Inbox에서 중복 제거하고 전역 고유 `link_request_id`는 IdempotencyRecord에서 업무 중복을 제거한다. Registration에 고정된 verification binding/version, `causation_id`, 발행자, Inbox `received_at`, 수락 기한, 두 Identity의 verified/owner 상태를 다시 확인한 뒤에만 두 IdentityLink를 같은 트랜잭션에서 활성화한다. 오래되었거나 binding이 일치하지 않는 메시지는 Inbox와 보안 감사만 거부하고 정상 Registration을 실패시키지 않는다.

`RegistrationDeadlineWorker`는 `pending_verification`의 `expires_at`, `awaiting_user_link`의 `link_accept_until`, `linked`/`issuing_session`의 `session_issue_until`을 각각 선점한다. 기한 전에 수신된 `received`/`deferred` Inbox 메시지가 있으면 먼저 처리하고 설정된 `inbox_processing_grace`까지 만료를 유예한다. 유예가 끝나거나 기한 내 메시지가 없을 때 Registration, 필요한 최초 완료 IdempotencyRecord, Domain/Audit OutboxEvent를 한 트랜잭션에서 종료하며 이미 활성화된 IdentityLink를 되돌리지 않는다.
...
| `QRY.A.300-06` | `GetRegistrationStatusHandler` | pending_verification, awaiting_user_link, linked, issuing_session, completed, failed, expired | auth-flow proof 또는 조회 전용 RegistrationStatusToken 필수 |
...
1. BFF는 이름과 추천인 입력을 Context 사용자에 전달해 `profile_request_id`를 받고, 필수 동의 처리에서 `agreement_receipt_id`를 받는다.
2. `StartEmailRegistrationHandler`는 두 불투명 참조, 이메일, 비밀번호, 휴대폰 인증 식별자만 받는다. 연결된 AuthenticationIntent와 owner proof의 서버 만료를 Registration TTL까지 상한 안에서 연장하고, Registration 상태 조회 전용 token의 hash, Registration과 두 Identity 예약을 같은 트랜잭션에서 만든다. 같은 key/body 재요청은 같은 Registration의 status proof를 원자적으로 교체해 새 원문만 응답한다. 이 token은 Challenge 검증·가입 완료·Session 발급 권한으로 사용할 수 없으며 원문이나 응답 ciphertext를 저장하지 않는다. 이 단계에서는 Challenge를 발급하거나 메시지를 보내지 않는다.
3. 클라이언트가 각 `POST .../challenges`를 호출하면 `RequestVerificationChallengeHandler`가 선택한 method의 Challenge와 발송 outbox를 원자적으로 만든다.
4. 클라이언트가 회원가입 완료를 요청하면 `AdvanceRegistrationHandler`가 이메일·휴대폰 challenge의 verified 상태를 다시 확인한다. 최초 `complete_registration` IdempotencyRecord를 Registration에 고정하고 RegistrationVerificationBinding과 가입 인증 완료 시점의 Registration version을 확정한다. Registration을 `awaiting_user_link`로 바꾸며 `EVT.A.300-35 회원가입 인증 완료됨`과 `Auth.RegistrationVerificationCompleted` version 1 OutboxEvent를 같은 트랜잭션에 저장한다.
5. API는 이벤트 수락이 커밋되면 `202 Accepted`와 상태 조회 위치를 반환한다. 요청 스레드에서 Context 사용자 응답을 기다리지 않는다.
6. Context 사용자는 가입 인증 완료 이벤트를 소비해 사용자 계정을 만들고 `user_id`를 발급한 뒤 `User.AuthLinkRequested` version 1을 보낸다. 사용자 생성이 업무 규칙상 거부되면 `User.AuthLinkRejected` version 1을 보낸다.
7. `ApplyRegistrationUserLinkHandler`는 producer, `event_id`, `causation_id`, verification binding/version, 전역 고유 `link_request_id`, Inbox 수신 시각을 검증한다. 유효한 요청이면 `user_id`, `user_auth_status`, `restriction_version`을 기록하고 두 IdentityLink를 하나의 트랜잭션에서 활성화한다. 같은 트랜잭션에서 `linked_at`과 `session_issue_until`을 확정한다. 사용자가 active이면 Registration을 `linked`로 바꾸고 최초 완료 IdempotencyRecord의 TTL을 연장한다. restricted/deactivated이면 Link를 유지한 채 Registration과 최초 완료 IdempotencyRecord를 `failed(USER_AUTH_RESTRICTED)`로 닫는다. binding이 확인된 `User.AuthLinkRejected`는 Link를 만들지 않고 Registration과 최초 완료 IdempotencyRecord를 실패로 닫는다.
8. 클라이언트는 상태 조회에서 `linked`를 확인한 뒤 최초 요청과 같은 `Idempotency-Key`로 완료 요청을 재개한다. `AdvanceRegistrationHandler`는 Registration을 `issuing_session`으로 바꾸고 Authorization Source에서 초기 AccessGrant를 조회한다. 다른 key는 새 실행을 만들지 않고 충돌로 거부한다.
9. 다음 로컬 트랜잭션에서 email IdentityLink를 인증 근거로 `authentication_method=registration_verified`인 Session, SessionCredential, AccessGrant, 선택된 AuthenticationIntent 소비, Registration `completed`, OutboxEvent를 함께 저장한다.
10. Session credential은 클라이언트 요청에 직접 반환한다. 비동기 consumer나 상태 조회 API는 cookie, refresh token, access token 원문을 만들거나 반환하지 않는다. 상태 조회 token은 terminal 상태 뒤 짧은 보존 TTL까지만 유효하며 조회 외 명령에는 사용할 수 없다.

[`SCN.A.300-01 이메일 회원가입과 자동 로그인`](../../../80-sequence/A_300_auth/SCN_A_300_01_email_registration.md)을 이 처리 순서의 기준 시퀀스로 사용한다. 서비스 문서는 Handler·트랜잭션·이벤트 책임을 정의하고, 참여자 사이의 전체 호출 순서는 시퀀스 문서에서 관리한다.

### Context 사용자 계정 연동 이벤트 계약

Auth는 Context 사용자에 `user_id` 발급을 동기 요청하지 않는다. Auth가 가입 인증 완료 사실을 발행하고, Context 사용자가 사용자 계정과 `user_id`를 만든 뒤 Auth에 계정 연동을 요청한다. 실제 IdentityLink 변경은 이를 소유하는 Auth가 수행한다.

#### `Auth.RegistrationVerificationCompleted` version 1

| 필드 | 설명 |
| --- | --- |
| `event_id` | Registration과 같은 트랜잭션에 저장한 전역 고유 OutboxEvent ID |
| `aggregate_id` | Context 인증 Registration 식별자 |
| `registration_version` | 이벤트를 만든 Registration version |
| `verification_binding_id` | 가입 인증 snapshot 불투명 식별자 |
| `verification_snapshot_hash` | 두 Identity/Challenge version과 검증 시각을 묶은 canonical hash |
| `profile_request_id` | Context 사용자가 보관하는 프로필·추천인 등록 초안의 불투명 참조 |
| `agreement_receipt_id` | 동의 담당 Context가 발급한 필수 동의 receipt의 불투명 참조 |
| `assurance_summary` | 이메일·휴대폰 확인 방식과 완료 시각. 실제 주소와 번호는 포함하지 않는다. |
| `link_accept_until` | Context 사용자의 계정 연동 요청을 받을 수 있는 기한 |

#### `User.AuthLinkRequested` version 1

| 필드 | 설명 |
| --- | --- |
| `event_id` | transport 중복 제거용 Context 사용자 event ID |
| `causation_id` | 원인이 된 `Auth.RegistrationVerificationCompleted` event ID |
| `link_request_id` | Context 사용자가 발급한 계정 연동 업무 멱등 키 |
| `auth_registration_id` | 연결할 Auth Registration 식별자 |
| `verification_binding_id` | Auth 이벤트에서 받은 binding ID |
| `verification_snapshot_hash` | Auth 이벤트에서 받은 snapshot hash |
| `registration_version` | Auth 이벤트에서 받은 Registration version |
| `user_id` | Context 사용자가 발급한 불변 사용자 식별자 |
| `user_auth_status` | `active`, `restricted`, `deactivated` |
| `restriction_version` | UserAuthState에 반영할 단조 증가 version |
| `issued_at` | `user_id` 발급 시각 |

`User.AuthLinkRejected` version 1은 `event_id`, `causation_id`, `link_request_id`, `auth_registration_id`, `registration_version`, verification binding과 공개 가능한 `reason_code`만 전달한다. 프로필·동의 검증이 업무 규칙상 거부된 경우에만 사용하며 transient consumer 장애를 거부 이벤트로 바꾸지 않는다.

계약 규칙은 다음과 같다.

- Auth는 broker ACL 또는 workload identity로 producer가 Context 사용자인지 확인한다.
- 전송 중복은 `consumer_name + event_id`, 업무 중복은 전역 고유 `link_request_id`로 분리한다. IdempotencyRecord는 전역 User Auth Link namespace를 scope로 사용하고 `auth_registration_id`는 resource와 payload hash에 함께 저장한다.
- 다른 event ID로 같은 `link_request_id`와 같은 payload가 오면 기존 결과를 반환한다. 같은 `link_request_id`에 다른 `user_id`, binding 또는 payload가 오거나 같은 Registration에 다른 `link_request_id`가 오면 `IDEMPOTENCY_CONFLICT`로 거부한다.
- `causation_id`, binding ID/hash, binding에 고정된 Registration version, 두 Challenge와 Identity의 verified 상태를 확인한다. 수락 기한은 producer 시각이 아니라 Auth Inbox의 `received_at <= link_accept_until`로 판정한다.
- 오래되었거나 binding/version이 일치하지 않거나 발행자가 잘못된 메시지는 Inbox `rejected`와 보안 감사만 저장하고 Registration 상태를 유지한다.
- 유효한 요청을 소비하는 로컬 트랜잭션에서 Inbox, 업무 IdempotencyRecord, Registration, 두 Identity/IdentityLink, UserAuthState, `EVT.A.300-36`, 결과 OutboxEvent를 함께 저장한다.
- binding이 확인된 `User.AuthLinkRejected`는 별도 로컬 트랜잭션에서 Inbox와 업무 IdempotencyRecord, Registration의 `link_request_id`와 `failed`, 최초 완료 IdempotencyRecord `failed`, `EVT.A.300-37`, 결과 OutboxEvent를 저장한다. Identity/IdentityLink/UserAuthState는 변경하지 않는다.
- 제한·비활성 사용자도 귀속 사실인 IdentityLink와 UserAuthState는 보존하되 Session은 발급하지 않는다. Registration과 최초 완료 IdempotencyRecord를 `USER_AUTH_RESTRICTED`로 함께 종료한다.
- 늦거나 순서가 뒤바뀐 event는 Inbox에서 지연 재처리한다. 수락 기한이 지난 요청은 fail closed로 거부하며 Auth가 사용자 계정을 삭제하거나 병합하지 않는다.
- `linked` 또는 `issuing_session`에서 `session_issue_until`이 지나면 Registration만 `expired`로 닫는다. IdentityLink는 유지하므로 사용자는 일반 로그인으로 인증할 수 있다.

#### `Auth.RegistrationUserLinked` version 1

| 필드 | 설명 |
| --- | --- |
| `event_id` | 결과 OutboxEvent ID |
| `causation_id` | 처리한 `User.AuthLinkRequested` event ID |
| `correlation_id` | 가입 작업 전체 correlation ID |
| `auth_registration_id` | Context 인증 Registration 식별자 |
| `registration_version` | 연결 결과가 반영된 Registration version |
| `link_request_id` | Context 사용자가 발급한 전역 고유 연동 요청 ID |
| `user_id` | 연결된 사용자 식별자 |
| `linked_at` | 두 IdentityLink 활성화 완료 시각 |

#### `Auth.RegistrationUserLinkRejected` version 1

| 필드 | 설명 |
| --- | --- |
| `event_id` | 결과 OutboxEvent ID |
| `causation_id` | 처리한 User Event ID. 연동 기한 만료이면 가입 인증 완료 Event ID |
| `correlation_id` | 가입 작업 전체 correlation ID |
| `auth_registration_id` | Context 인증 Registration 식별자 |
| `registration_version` | 거부 또는 만료가 반영된 Registration version |
| `link_request_id` | 거부된 연동 요청 ID. 요청을 받지 못한 기한 만료이면 NULL |
| `user_id` | 요청에 포함된 사용자 식별자. 사용자 생성 거부나 기한 만료이면 NULL |
| `reason_code` | `USER_CREATION_REJECTED`, `IDENTITY_OWNERSHIP_CONFLICT`, `LINK_TIMEOUT` 중 공개 가능한 일반화 코드 |
| `rejected_at` | 거부 또는 만료 확정 시각 |

Context 사용자는 연결 성공 결과로 임시 사용자 계정을 활성화하고, 거부 결과로 실패·보상 처리를 수행한다. 오래되었거나 binding이 일치하지 않는 메시지는 이 업무 결과 이벤트를 만들지 않으며 보안 감사와 Inbox 결과로만 남긴다.
...
| --- | --- | --- | --- |
| Web | HttpOnly/Secure/SameSite cookie credential | Session-bound CSRF token, 필요 시 BFF 내부 context token | cookie 원문 없이 SessionCredential hash와 CSRF key version 저장 |
| Mobile | opaque refresh token | access JWT | refresh 원문 없이 digest 저장, access JWT는 저장하지 않음 |
| Ingress/BFF 내부 | 기존 Session을 확인한 signed request context | internal context JWT | 기본적으로 저장하지 않고 짧은 TTL 사용 |
...
| 사용자 차단·비활성 이벤트 | 해당 `user_id`의 전체 Session | Context 사용자 이벤트 version 확인 후 모두 폐기 |
...
- Authorization Source, 이메일/SMS, Audit Context 호출과 broker publish/ack 대기를 로컬 DB 트랜잭션 안에서 수행하지 않는다. Context 사용자와는 동기 호출하지 않고 outbox/inbox로 연동한다.
...
| 가입 인증 완료 통지 | 없음 | 없음 | 단일 트랜잭션: 클라이언트 IdempotencyRecord, Registration과 두 verified Identity/Challenge 확인, 불변 verification binding/version, Registration `awaiting_user_link`, `EVT.A.300-35`, Context 사용자용 outbox |
| 사용자 계정 연동 요청/거부 반영 | 없음 | 없음 | 단일 트랜잭션: Inbox/업무 IdempotencyRecord, Registration 검증, 성공 시 두 IdentityLink/UserAuthState/`EVT.A.300-36`, binding 확인된 거부 시 Registration/클라이언트 IdempotencyRecord 실패와 `EVT.A.300-37`, 결과 outbox, Inbox 최종 상태 |
| 회원가입 기한 만료 | 없음 | 없음 | 단일 트랜잭션: 검증 대기는 Registration과 감사 outbox, link 기한 만료는 Registration/클라이언트 IdempotencyRecord/`EVT.A.300-37`/Context 사용자 결과 outbox, Session 기한 만료는 Registration/클라이언트 IdempotencyRecord/감사 outbox |
| 회원가입 자동 로그인 | 클라이언트 요청에서 Registration `linked -> issuing_session`과 두 active Link 확인 | ResolveAccessGrant | Session, AccessGrant, Registration `completed`, AuthenticationIntent 소비, 완료 outbox |
...
| 사용자 제한 이벤트 | UserAuthState source version 확인 | 없음 | UserAuthState, active AccessGrant, 전체 Session, 소비 기록, 감사 outbox |

회원가입 완료의 첫 로컬 트랜잭션은 클라이언트 IdempotencyRecord 참조, verification binding, `awaiting_user_link`, 가입 인증 완료 outbox를 함께 저장한다. Context 사용자의 각 계정 연동 요청·거부 메시지는 중간 선점을 따로 커밋하지 않고 하나의 consumer 트랜잭션에서 Inbox 최종 결과와 Registration 결과까지 저장한다. 재시도가 필요한 순서 역전만 Inbox를 `deferred`로 커밋한다. 이후 클라이언트 완료 요청에서 `ResolveAccessGrant`가 실패하면 Registration을 `issuing_session`으로 유지하고, 재시도에서 Session 발급만 이어서 처리한다. 비동기 consumer는 전달할 수 없는 Session credential을 미리 만들지 않는다.
...
| 사용자 인증 제한 변경 | `auth.user_auth_state.changed` | status, restriction version, source |
...
| 가입 인증 완료 통지 | registration id + client action id | `processing` 상태에서 같은 verification binding과 같은 OutboxEvent를 사용하고 현재 Registration 상태 반환 |
| 사용자 계정 연동 요청 | consumer + source event id, 전역 고유 link request id | transport 중복은 Inbox 결과, 업무 중복은 같은 `user_id`/Link 결과 반환. 같은 Registration의 다른 request 또는 다른 payload는 conflict |
| 회원가입 Session 완료 | registration id + client action id | `linked` 뒤 같은 논리 Session을 정확히 한 번 발급하고, 응답 유실 복구는 SessionCredential만 교체 |
...
| 권한/사용자 상태 이벤트 | source event id + source version | 같은 version 재적용 금지 |

IdempotencyRecord는 key, command type, 전체 canonical request의 keyed HMAC fingerprint, 결과 참조와 만료 시각만 저장한다. 비밀번호나 token 원문과 단순 hash를 저장하지 않는다. 가입 완료 요청의 최초 `202`는 최종 응답이 아니므로 IdempotencyRecord를 `processing`으로 유지하고 Registration에 record 참조를 고정한다. 같은 key 재시도에서는 현재 Registration 상태를 반환하며 `linked` 뒤 Session 발급 단계를 이어서 처리한다. 다른 key는 두 번째 processing record를 만들지 않고 충돌로 거부한다. User Link 성공은 원래 record의 TTL을 연장하고, 거부·기한 만료는 `failed`, Session 전달 성공은 `completed`로 닫는다. 로그인 실패가 completed이면 저장된 실패를 재생하고 failure count를 다시 올리지 않는다. 성공 응답 유실 때만 이메일 비밀번호 또는 같은 phone challenge 결과를 다시 확인한 뒤 기존 SessionCredential을 교체한다. TTL 뒤에는 새 로그인 action을 요구하며 같은 논리 Session을 하나 더 만들지 않는다. refresh, 이메일 재인증과 휴대폰 교체 완료 credential 전달 복구는 allowlist된 별도 encrypted replay 규칙을 따른다.
...
| --- | --- | --- | --- | --- |
| Context 사용자 이벤트 채널 | relay/consumer ack 설정 | outbox/inbox at-least-once, backoff | publisher/consumer 별도 격리 | 가입 인증 완료 통지와 계정 연동 요청 |
...
| Context 사용자 또는 이벤트 채널 지연 | 기존 로그인/refresh/로그아웃 | 회원가입의 사용자 계정 연결과 자동 로그인 | Registration을 `awaiting_user_link`로 유지하고 backlog age/기한 초과를 경보 |
...
선택적 개인화 컴포넌트의 인증 오류는 공개 페이지 전체 오류로 전파하지 않는다. 다만 이 제한은 화면/BFF의 Auth Boundary 처리이며, Context 인증이 가짜 개인화 데이터나 성공 응답을 만드는 근거가 아니다.
...
| RegistrationLinkPolicy | link accept TTL, Inbox processing grace, automatic Session issue TTL | 각각 `link_accept_until`, `inbox_processing_grace`, `session_issue_until` 계산에 사용하며 초기값은 운영 확정 |
...
- 내부 헤더는 ingress/BFF가 검증한 요청에서만 신뢰하고 외부 요청의 `X-User-*` 값은 제거한다.
...
| `USER_LINK_REJECTED` | Context 사용자 거부 또는 유효하지 않은 계정 연동 요청 | 사유에 따라 새 가입 또는 운영 확인 |
| `USER_LINK_TIMEOUT` | `link_accept_until`까지 유효한 계정 연동 요청 미수신 | 새 가입 필요 |
...
| 요구사항 | 서비스 반영 |
| --- | --- |
| `FR-001`, `FR-002`, `FR-023`, `FR-024`, `FR-025` | AuthenticationGateService가 접근 분류를 제공하고 공개/선택적 영역의 대체 처리는 BFF/UI 경계에 둔다. |
| `FR-003`, `FR-009` | AuthenticationIntent와 내부 redirect 검증, 로그인 성공 뒤 intent 소비로 반영한다. |
| `FR-004`, `FR-005`, `FR-006`, `FR-007`, `FR-027` | RegistrationService, VerificationService, 가입 인증 완료 outbox, Context 사용자 계정 연동 consumer와 클라이언트 완료 요청의 자동 Session 발급으로 반영한다. |
...
| Contract test | `Auth.RegistrationVerificationCompleted`, `User.AuthLinkRequested`/거부 event version 1, AuthorizationSource version, Email/SMS idempotency key, Audit event schema |
| Failure injection test | 외부 timeout, breaker open/half-open, DB commit 실패, outbox relay 중단·재개, 늦은 이벤트 순서 역전 |
...
- 회원가입 Event가 중복·역순 전달되어도 `user_id` 귀속과 IdentityLink가 중복되거나 다른 사용자로 바뀌지 않는다.
...
- Context 사용자 `profile_request_id`, 동의 담당 Context `agreement_receipt_id` 계약과 초안/receipt 만료 시점.
