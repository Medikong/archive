...
| `SVC.A.300-02` | RegistrationService | 가입 시작, 검증 완료 확인, `user_id` 발급 요청, Identity 연결과 자동 로그인을 조정한다. | Registration, Identity, IdentityLink, PasswordCredential, Session | Context 사용자, Authorization Source |
| `SVC.A.300-03` | VerificationService | 이메일/SMS challenge 발급, 발송 요청, 검증, 만료, 재전송과 제한 정책을 처리한다. | VerificationChallenge, Identity | 이메일 발송, 가상 SMS |
...
| `SVC.A.300-06` | PasswordRecoveryService | 복구 challenge, PasswordReset 발급·소비, 비밀번호 교체와 세션 무효화를 처리한다. | VerificationChallenge, PasswordReset, PasswordCredential, Session | 이메일 발송, 가상 SMS |
| `SVC.A.300-07` | IdentityLinkService | 현재 `user_id`에 추가 Identity를 연결하고 충돌 또는 수동 검토 상태를 처리한다. | Identity, IdentityLink, VerificationChallenge | 이메일 발송, 가상 SMS |
| `SVC.A.300-08` | PhoneChangeService | 대체 인증과 새 번호 SMS 검증 후 같은 `user_id`의 휴대폰 IdentityLink를 교체한다. | Identity, IdentityLink, VerificationChallenge, Session | 가상 SMS |
...
| `RegistrationRepository` | Registration 단계와 외부 요청 참조 저장 |
| `PasswordResetRepository` | reset grant 단일 소비와 만료 상태 저장 |
| `AuthenticationIntentRepository` | 로그인 전 intent 저장과 단일 소비 |
...
| `CMD.A.300-02 로그인 의도 보존` | `PreserveAuthenticationIntentHandler` | internal target, action intent, client channel | AuthenticationIntent 생성 또는 갱신 | 로컬 1회 |
| `CMD.A.300-03 이메일 회원가입 시작` | `StartEmailRegistrationHandler` | profile_request_id, agreement_receipt_id, email, password, phone, client_action_id | Registration, Identity, PasswordCredential 생성 또는 미귀속 예약 재사용 | 로컬 1회 |
...
| `CMD.A.300-06 인증 식별자 연결` | `CompleteRegistrationHandler` 또는 `CompleteIdentityLinkHandler` | registration/link request id | IdentityLink 활성화 | 외부 호출 후 로컬 1회 |
...
| `CMD.A.300-08 휴대폰 번호 로그인` | `SignInWithPhoneHandler` | verified phone challenge, remember_me, intent id | 연결된 `user_id` 확인 후 Session 발급 | 외부 호출 후 로컬 1회 |
...
| --- | --- | --- | --- | --- |
| `CMD.A.300-16 회원가입 진행` | `AdvanceRegistrationHandler` | registration id | `user_id` 요청, Link, Session 단계를 현재 상태부터 재개 | `FR-005`, `FR-007` |
...
| `QRY.A.300-06` | `GetRegistrationStatusHandler` | pending_verification, requesting_user, linking, issuing_session, completed, failed, expired | registration proof 필수 |
...
2. `StartEmailRegistrationHandler`는 두 불투명 참조, 이메일, 비밀번호, 휴대폰 인증 식별자만 받아 Registration과 두 Identity 예약을 만든다. 이 단계에서는 Challenge를 발급하거나 메시지를 보내지 않는다.
3. 클라이언트가 각 `POST .../challenges`를 호출하면 `RequestVerificationChallengeHandler`가 선택한 method의 Challenge와 발송 outbox를 원자적으로 만든다.
4. 현재 Registration의 이메일·휴대폰 challenge가 모두 verified가 되면 Registration을 `requesting_user`로 전환한다.
5. `AdvanceRegistrationHandler`가 로컬 트랜잭션 밖에서 Context 사용자에 `user_id` 발급을 요청한다.
6. 응답받은 `user_id`, `user_auth_status`, `restriction_version`을 UserAuthState와 Registration에 기록하고 두 IdentityLink를 활성화한다. active면 `linking`을 거쳐 `issuing_session`으로 바꾸고, 제한 상태면 `failed(USER_AUTH_RESTRICTED)`로 닫아 자동 로그인으로 진행하지 않는다.
7. Authorization Source에서 초기 AccessGrant를 조회한다.
8. 다음 로컬 트랜잭션에서 email IdentityLink를 인증 근거로 `authentication_method=registration_verified`인 Session, SessionCredential, AccessGrant, 선택된 AuthenticationIntent 소비, Registration `completed`, OutboxEvent를 함께 저장한다.
9. 두 로컬 트랜잭션이 모두 성공한 뒤에만 회원가입 완료와 자동 로그인을 클라이언트에 알린다.
...
sequenceDiagram
    ...
    participant Auth as Context 인증
    ...
    BFF->>Auth: 회원가입 시작 + opaque references
    Auth->>DB: Registration, Identity 예약 저장
    Auth-->>BFF: registration_id, required methods
    BFF->>Auth: method별 challenge 발급 요청
    Auth->>DB: challenge + delivery outbox 저장
    DB-->>Provider: outbox relay로 발송
    BFF->>Auth: 이메일·휴대폰 검증
    Auth->>DB: challenge와 Identity verified 저장
    BFF->>Auth: 회원가입 완료 요청
    Auth->>User: user_id 발급 요청
    User-->>Auth: user_id
    Auth->>DB: user_id, IdentityLink, issuing_session 상태 저장
    Auth->>Authz: 초기 AccessGrant 조회
    Authz-->>Auth: roles, permissions, version
    Auth->>DB: Session, AccessGrant, completed, outbox 원자 저장
    Auth-->>BFF: 자동 로그인 결과
```

### Context 사용자 `user_id` 발급 계약

Port 이름은 `UserContextPort.IssueUserId`로 고정한다. 이 계약은 `user_id`를 Auth가 만들지 못하게 하는 경계다.

| 구분 | 필드 | 설명 |
| --- | --- | --- |
| 요청 | `auth_registration_id` | Context 인증 Registration 식별자 |
| 요청 | `user_creation_request_id` | Registration마다 고정하는 Context 사용자 호출 ID이자 멱등 키 |
| 요청 | `profile_request_id` | Context 사용자가 보관하는 프로필·추천인 등록 초안의 불투명 참조 |
| 요청 | `agreement_receipt_id` | 동의 담당 Context가 발급한 필수 동의 receipt의 불투명 참조 |
| 요청 | `assurance_summary` | 이메일·휴대폰 확인 여부와 완료 시각만 포함하며 실제 주소/번호는 포함하지 않는다. |
| 응답 | `user_id` | Context 사용자가 발급한 불변 사용자 식별자 |
| 응답 | `issued_at` | 발급 시각 |
| 응답 | `user_auth_status` | 초기값 `active`. restricted/deactivated 응답이면 가입 자동 로그인을 중단한다. |
| 응답 | `restriction_version` | UserAuthState에 저장할 Context 사용자 상태 version |
| 응답 | `idempotency_replayed` | 같은 키에 기존 결과를 반환했는지 여부 |

계약 규칙은 다음과 같다.

- 같은 `user_creation_request_id`에는 항상 같은 `user_id`를 반환한다.
- 같은 요청 ID에 다른 `profile_request_id` 또는 `agreement_receipt_id`가 들어오면 `IDEMPOTENCY_CONFLICT`를 반환한다.
- Context 인증은 두 reference가 가리키는 이름, 동의, 추천인 값을 조회하거나 복제하지 않는다.
- Context 사용자 응답이 성공하기 전에는 IdentityLink를 활성화하지 않는다.
- `user_id` 발급 응답을 받은 뒤 첫 로컬 저장이 실패하면 같은 요청 ID로 `IssueUserId`를 다시 호출해 같은 값을 받는다. `user_id`와 Link 저장이 끝났다면 `issuing_session`부터 재개하고 새 `user_id`를 요청하지 않는다.
- 재시도 불가 오류가 난 Registration은 `failed`와 `failure_code`를 남기고 운영 알림을 만든다. 사용자 계정 삭제나 병합으로 보정하지 않는다.
- 프로필 초안이 만료·거부되면 `USER_CREATION_REJECTED`, 일시 장애면 `USER_CONTEXT_UNAVAILABLE`로 구분한다.

## 검증 Challenge와 발송 Adapter

### 발급과 검증

- VerificationChallenge purpose는 `signup_email`, `signup_phone`, `phone_signin`, `password_reset`, `identity_link`, `phone_change`로 구분하고, 이메일과 SMS 방식은 channel로 나눈다.
- challenge 원문 token과 SMS 코드는 저장하지 않는다. keyed digest 또는 검증용 hash만 저장한다.
...
| Port | 입력 | 성공 의미 | 실패 처리 |
| --- | --- | --- | --- |
| `EmailVerificationSender.Send` | delivery_id, template_id, destination, opaque verification link, locale, expires_at | 제공자가 발송 요청을 수락함 | 재시도 가능/불가를 구분해 delivery 상태 기록 |
| `SmsVerificationSender.Send` | delivery_id, template_id, destination, one-time code, locale, expires_at | 가상 SMS 시스템이 발송 요청을 수락함 | 제한, 잘못된 번호, 일시 장애를 구분 |

발송은 API 요청 안에서 직접 호출하지 않는다. challenge와 `VerificationDeliveryRequested` OutboxEvent를 같은 트랜잭션에 넣고 relay가 Adapter를 호출한다. Domain/Audit payload에는 secret을 넣지 않으며 delivery event에는 평문 code/link 대신 짧은 수명의 암호화 `delivery_payload_ref`만 넣는다. relay는 전송 직전에 메모리에서만 복호화하고 terminal 전달 뒤 payload를 삭제하거나 crypto-shred한다.

`delivery_id`를 provider idempotency key로 전달한다. Provider가 멱등성을 지원하지 않으면 중복 발송 가능성을 관측하고 사용자 재전송 간격으로 피해를 제한한다.

가상 SMS 테스트 번호와 고정 코드는 개발·테스트 환경의 allowlist에서만 허용한다. 운영 설정에서 발견되면 서비스 시작을 실패시킨다.
...
1. `phone_signin` 목적의 VerificationChallenge를 발급하고 가상 SMS로 보낸다.
...
6. 허용 상태면 failure count를 초기화하고 해당 `user_id`의 AccessGrant를 조회해 Session을 발급한다.
...
| 휴대폰 Identity 교체 | 기존 휴대폰 Identity로 로그인한 다른 Session | 이메일 재인증으로 재바인딩한 현재 Session은 유지하고, 나머지 기존 phone 기반 Session은 폐기 |
...
- 이메일 비밀번호 재인증 성공 시 현재 Session을 같은 `user_id`의 active 이메일 IdentityLink에 재바인딩하고 `authentication_method=email_password`, `last_authenticated_at`을 갱신한다. SessionCredential과 CSRF token도 회전한 뒤 proof를 발급한다.

## 비밀번호 재설정

1. `RequestPasswordResetHandler`는 이메일 또는 휴대폰 인증 식별자를 받고 존재 여부를 공개하지 않는 접수 결과를 반환한다.
...
5. 이메일 링크 또는 SMS code 검증 후 PasswordReset을 `challenge_verified`로 바꾼다.
...
- Context 사용자, Authorization Source, 이메일/SMS, Audit Context 호출을 로컬 DB 트랜잭션 안에서 수행하지 않는다.
...
| --- | --- | --- | --- |
| 회원가입 시작 | Registration, Identity 예약, PasswordCredential | 없음 | 없음 |
| Challenge 발급 | Registration/PasswordReset 등 상위 작업과 기존 issued Challenge 확인 | 없음 | 새 Challenge, 상위 작업 참조, delivery payload/outbox |
| 회원가입 사용자 발급 | Registration을 `requesting_user`로 전환하고 identity 예약 확인 | IssueUserId | `user_id`, IdentityLink, Registration `linking` -> `issuing_session`, 연결 outbox |
| 회원가입 자동 로그인 | Registration `issuing_session`과 두 active Link 확인 | ResolveAccessGrant | Session, AccessGrant, Registration `completed`, AuthenticationIntent 소비, 완료 outbox |
...
| 비밀번호 재설정 | 없음 | 없음 | PasswordCredential 교체, PasswordReset 소비, 전 Session 폐기, 감사 outbox |
| 휴대폰 번호 교체 | 검증 proof 상태 확인 | 없음 | 기존/새 Identity와 IdentityLink, 대상 Session, 감사 outbox |
...
회원가입 완료의 첫 로컬 트랜잭션은 identity 값의 유일성 예약과 Registration 상태만 짧게 갱신한다. `IssueUserId` 뒤에는 받은 `user_id`와 Link를 먼저 저장해 외부 호출 결과를 잃지 않는다. 이후 `ResolveAccessGrant`가 실패하면 Registration을 `issuing_session`으로 유지하고, 재시도에서 Session 발급만 이어서 처리한다. 예약 만료와 재획득 규칙은 Persistence 설계에서 정한다.
...
BC.A.300의 `EVT.A.300-12~17`, `EVT.A.300-21`은 위 외부 감사 event type으로 변환해 발행한다. “감사 전송됨”은 Aggregate 상태가 아니라 relay 전달 결과이므로 Domain Event와 Outbox delivery 상태를 구분한다. 도메인 모델의 `EVT.A.300-22~34`는 Challenge, Registration, PasswordReset, Session, AccessGrant, AuthenticationIntent, 사용자 제한, 정책 변경, 로그인 실패와 잠금 사실을 보강한다.
...
| `user_id` 발급 | auth registration id | Context 사용자가 같은 `user_id` 반환 |
| 회원가입 완료 | registration id + client action id | 같은 completed 상태와 같은 논리 Session을 사용하고, 응답 유실 복구는 SessionCredential만 교체 |
...
| Identity 연동/휴대폰 교체 | operation id | 같은 최종 연결 상태 반환 |
| 수동 처리 | operation id + approval version | 같은 operation 재실행 금지 |
| 권한/사용자 상태 이벤트 | source event id + source version | 같은 version 재적용 금지 |

IdempotencyRecord는 key, command type, 전체 canonical request의 keyed HMAC fingerprint, 결과 참조와 만료 시각만 저장한다. 비밀번호나 token 원문과 단순 hash를 저장하지 않는다. 로그인 실패가 completed이면 같은 key 재시도는 저장된 실패를 재생하고 failure count를 다시 올리지 않는다. 성공 응답 유실 때만 이메일 비밀번호 또는 같은 phone challenge 결과를 다시 확인한 뒤 기존 SessionCredential을 교체한다. TTL 뒤에는 새 로그인 action을 요구하며 같은 논리 Session을 하나 더 만들지 않는다. refresh는 별도 encrypted replay 규칙을 따른다.
...
| --- | --- | --- | --- | --- |
| Context 사용자 | 2초 | 멱등 요청만 1회, jitter 적용 | 최근 20회 중 50% 실패 시 30초 open | 회원가입 완료에만 사용 |
...
| 이메일 Provider | worker당 3초 | delivery 정책에 따라 backoff | provider별 독립 breaker | 사용자 API와 비동기 분리 |
| 가상 SMS Provider | worker당 3초 | delivery 정책에 따라 backoff | provider별 독립 breaker | resend 제한과 별개 |
...
| Context 사용자 장애 | 기존 로그인/refresh/로그아웃 | 회원가입의 `requesting_user` 단계 | Registration을 재시도 가능한 `requesting_user`로 유지 |
...
- 이메일 link token, SMS code, PasswordReset proof도 원문을 저장하지 않는다.
...
- Handler, Repository, PasswordHasher, User Context, Authorization Source, delivery relay, Audit relay를 span으로 구분한다.
...
| `SESSION_DELIVERY_EXPIRED` | 회원가입 완료 credential 복구 기간 종료 | 일반 로그인 필요 |
...
| `USER_CONTEXT_UNAVAILABLE` | `user_id` 발급 의존성 일시 장애 | 같은 registration id로 가능 |
...
| `FR-004`, `FR-005`, `FR-006`, `FR-007`, `FR-027` | RegistrationService, VerificationService, UserContextPort와 자동 Session 발급으로 반영한다. |
...
| Contract test | UserContextPort 멱등 발급, AuthorizationSource version, Email/SMS idempotency key, Audit event schema |
...
- 회원가입 중 Context 사용자 호출이 재시도되어도 `user_id`와 IdentityLink가 중복 생성되지 않는다.
...
- 휴대폰 Identity 교체 시 기존 phone 기반 다른 Session의 폐기 batch 크기와 완료 지연 허용치. 현재 Session은 email Link 재바인딩 후 유지한다.
