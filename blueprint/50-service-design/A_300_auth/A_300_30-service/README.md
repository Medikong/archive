...
- access JWT 발급, opaque refresh token 회전과 Session 무효화 기준.
...
- role, permission, membership의 저장·판정과 업무 인가 정책.
- Audit Context의 이벤트 보존, 검색, 조사 화면.
- 서비스 자체 인증에 쓰는 mTLS, workload identity와 service account.

Context 인증이 다른 컨텍스트와 공유하는 사용자 식별자는 `user_id`다. 이메일과 휴대폰 번호는 인증 식별자이므로 Context 인증 안에서만 다루고, JWT claim이나 내부 `X-User-*` 헤더에 넣지 않는다. role/permission/membership도 JWT와 Auth의 내부 사용자 컨텍스트에 넣지 않는다.
...
flowchart LR
    Client["Web / Mobile"]
    Ingress["Istio Ingress / JWT + Session check"]
...
| --- | --- | --- | --- | --- |
| `SVC.A.300-01` | SessionStatusService | HTTP ext_authz Adapter가 JWT에서 추출한 `sid`의 active, expired, revoked 상태를 fail closed로 판정한다. | Session, UserAuthState | 없음 |
| `SVC.A.300-02` | RegistrationService | 가입 시작, 검증 완료 proof와 프론트엔드의 동기 완료 요청을 처리한다. | Registration, Identity, IdentityLink, PasswordCredential, Session | 없음 |
...
| `SVC.A.300-04` | SignInService | 이메일/비밀번호 또는 연결된 휴대폰 Identity를 검증하고 로그인 실패·잠금과 Session 발급을 조정한다. | Identity, IdentityLink, PasswordCredential, Session | SessionService |
| `SVC.A.300-05` | SessionService | 웹·모바일 공통 access JWT와 opaque refresh token 발급, rotation, 재사용 탐지와 로그아웃을 처리한다. | Session, SessionCredential, UserAuthState | AccessTokenIssuer, SessionStatusProjection |
...
| `SVC.A.300-09` | AuthOperationService | 신뢰된 운영자 호출의 승인 proof와 감사 근거를 검증하고 잠금 해제·인증 수단 수동 처리를 수행한다. 호출자 인가는 외부 경계에서 완료되어야 한다. | Identity, IdentityLink, Session | Approval System |
...
| `AccessTokenIssuerPort` | [SD.A.300.JWT](../jwt-jwks-istio.md)을 따르는 signed access JWT 발급 |
| `RefreshCredentialIssuerPort` | 웹·모바일 공통 opaque refresh token 발급 |
| `JwksProviderPort` | active/retiring signing key의 공개 JWK 조회와 `kid` rotation |
| `SessionStatusProjectionPort` | Istio Session 검사에 사용할 active/revoked 상태 cache 갱신 |
...
- Query Handler는 상태를 바꾸지 않는다. 민감 정보 마스킹과 외부 인가 결과 확인은 Query Handler 앞의 정책에서 강제한다.
- 외부 네트워크 호출 중에는 DB 트랜잭션을 열어 두지 않는다. 외부 결과를 받은 뒤 Aggregate를 다시 읽고 version을 검증한다.
...
- 비밀번호, 인증번호, refresh cookie/token, access token 원문은 Handler 로그·trace·Domain Event에 넣지 않는다.
...
| --- | --- | --- | --- | --- |
| `CMD.A.300-01 Session 상태 확인` | `CheckSessionStatusHandler` | verified sub, sid, jti | active, expired, revoked 결과 | 읽기 전용 |
...
| `CMD.A.300-09 Session 발급` | `IssueSessionHandler` | verified user_id, client channel, policy | Session, access JWT와 refresh token | 로컬 1회, 내부 호출 전용 |
...
도메인 모델의 가입 검증, 비밀번호 재설정과 Session 폐기 규칙을 다음 Command로 구체화한다. BC.A.300의 Element Catalog를 갱신할 때 같은 식별자와 이름을 사용한다.
...
| `QRY.A.300-02` | `ResolveAuthenticationPrincipalHandler` | user_id, session_id, token_id와 Session 상태 | Istio가 검증한 JWT claim과 active Session 필수 |
| `QRY.A.300-03` | `GetMyAuthenticationMethodsHandler` | 마스킹된 Identity 종류, 검증/연결/잠금 상태 | 본인 Session 필수 |
| `QRY.A.300-04` | `GetAuthenticationStatusHandler` | Registration, IdentityLink, 잠금, Session 요약과 감사 전송 상태 | 신뢰된 운영자 authorization decision과 조회 사유 필수 |
| `QRY.A.300-05` | `GetSessionPolicyHandler` | access/refresh/remember-me TTL, rotation, lock policy version | 신뢰된 workload 또는 운영자 authorization decision 필수 |
...
6. `CompleteRegistrationHandler`는 proof와 verified Registration을 확인하고 두 IdentityLink, Session, SessionCredential과 Registration `completed`를 한 트랜잭션에 저장한다.
7. Session credential은 Auth가 프론트엔드에 직접 전달한다.

같은 Registration과 `Idempotency-Key` 재시도는 같은 User와 논리 Session 결과를 반환한다. 가입용 Event, Inbox, link 상태 polling과 별도 Agreement 서비스는 사용하지 않는다. 전체 참여자 순서는 [`SCN.A.01-01`](../../../80-sequence/A_01_user/SCN_A_01_01_user_provisioning_auth_link.md)을 따른다.
...
6. failure count 초기화, Session과 refresh credential 저장, 선택된 AuthenticationIntent 소비와 감사 OutboxEvent를 한 트랜잭션에 저장한다.
7. commit 뒤 access JWT와 채널에 맞는 refresh token 전달 결과를 반환한다.
...
6. 허용 상태면 failure count를 초기화하고 AuthenticationIntent에 고정한 `rememberMe` 정책으로 Session, access JWT와 refresh token을 발급한다.
...
| 채널 | access JWT | refresh token 전달 | 저장 기준 |
| --- | --- | --- | --- |
| Web | 응답 body로 전달하고 웹앱 메모리에만 보관 | Auth host의 HttpOnly·Secure·SameSite cookie | refresh 원문 없이 digest 저장, access JWT는 저장하지 않음 |
| Mobile | 응답 body로 전달하고 플랫폼 보안 메모리 정책 적용 | 응답 body로 전달하고 플랫폼 보안 저장소에 보관 | refresh 원문 없이 digest 저장, access JWT는 저장하지 않음 |
| Istio Ingress 경유 요청 | Bearer JWT를 Istio가 검증 | 보호 업무 API에는 전달하지 않음 | [SD.A.300.JWT](../jwt-jwks-istio.md)의 최소 인증 헤더 생성 |

access JWT claim, JWKS와 Istio 내부 인증 헤더는 [SD.A.300.JWT](../jwt-jwks-istio.md)을 따른다.

웹 refresh cookie가 자동 전송되는 refresh/logout 요청은 정확한 Origin, Fetch Metadata, cookie path와 SameSite 정책을 검증한다. 업무 보호 API는 Authorization Bearer JWT를 사용하므로 Session에 묶인 CSRF token을 사용하지 않는다. 사전 인증용 `__Host-dm_auth` cookie의 CSRF 처리 규칙은 별도로 유지한다.
...
- TTL 정책을 바꿔도 이미 발급된 artifact의 `exp`를 소급해 늘리지 않는다. 긴급 단축은 대상 Session을 폐기하고 Session 상태 projection을 갱신해 적용한다.

### JWT/JWKS/Istio 인증 처리

`AccessTokenIssuerPort`, `JwksProviderPort`와 `SessionStatusService`는 [SD.A.300.JWT](../jwt-jwks-istio.md)의 claim allowlist, key rotation, 검증 순서, 내부 인증 헤더와 fail-closed 규칙을 구현한다. 이 문서는 Handler 조정과 Session 상태 변경 책임만 정의한다.

### Refresh rotation

1. token 원문에서 lookup digest를 계산해 SessionCredential을 찾는다.
2. Session, credential과 UserAuthState의 기본 상태를 읽는다.
3. UserAuthState active를 확인한 뒤 로컬 트랜잭션을 시작하고 상태·credential row를 다시 잠금 조회한다.
...
동시에 여러 refresh 요청이 들어오면 row lock을 먼저 획득한 하나만 성공한다. 웹과 모바일 클라이언트는 refresh 요청을 한 번에 하나만 실행한다. 응답을 받지 못하면 같은 refresh token과 같은 `Idempotency-Key`로만 재시도해 최초 암호화 응답을 재생받는다. 재생 전에도 UserAuthState와 Session active를 확인하며 제한·폐기 뒤에는 반환하지 않는다. 같은 token을 새 key로 보내면 재사용 탐지 정책을 적용한다.
...
Access JWT는 이미 발급된 뒤에도 Istio가 `sid`의 Session 상태를 확인한다. 로그아웃, 비밀번호 재설정, refresh 재사용과 사용자 제한으로 Session이 폐기되면 남은 JWT TTL과 관계없이 업무 서비스에 전달하지 않는다.

## 인증과 인가 경계

- Auth는 `user_id`, Identity, credential, Session과 인증 허용 상태만 소유한다.
- access JWT와 Istio 내부 사용자 헤더에는 role, permission, membership과 업무 리소스 정보를 넣지 않는다.
- 로그인과 refresh는 인가 원천에 의존하지 않는다. 인가 서비스 장애가 새 Session이나 access JWT 발급을 막지 않는다.
- 업무 서비스는 검증된 `user_id`를 기준으로 자신이 소유한 membership, role, permission과 리소스 소유권을 확인한다.
- Auth 운영 API를 호출할 수 있는지에 대한 판단은 별도 인가 경계가 수행하고, Auth는 신뢰된 workload identity와 승인 proof만 검증한다.

### ReauthenticationProof

`ReauthenticationProof`는 고위험 작업 직전에 비밀번호나 후속 강한 인증을 다시 확인했다는 단기 증명이며 access JWT와 다른 Value Object다.
...
- role, permission, membership이나 업무 리소스 권한을 증명하거나 대신하지 않는다.
...
`ApplyManualIdentityOperationHandler`는 별도 인가 경계가 승인한 호출에서 `operation_id`, actor user_id, authorization decision reference, 승인자, 사유 코드, 증빙 참조와 만료 시각을 받는다. Auth는 신뢰된 workload identity와 decision의 대상·action·만료·일회성, approval/evidence reference만 검증하고 호출자의 role이나 permission을 다시 판정하지 않는다. 증빙 원문은 인증 DB에 복제하지 않고 opaque reference를 OutboxEvent에 저장해 Context 감사가 장기 보존한다. 인증 DB의 IdempotencyRecord는 operation 결과 참조만 보존한다.

수동 처리의 권한 규칙과 승인자 등급은 별도 인가·승인 경계가 소유한다. Auth는 최근 strong auth가 필요한 작업이면 목적이 일치하는 ReauthenticationProof를 추가로 검증한다. `revoke_sessions`의 공개 동기 API는 단일 Session target만 허용한다. 사용자 전체 폐기는 cursor batch와 별도 operation 상태가 필요한 내부/후속 비동기 명령으로 분리한다.
...
- 이메일/SMS와 Audit Context 호출을 로컬 DB 트랜잭션 안에서 수행하지 않는다. 가입 프론트엔드가 User와 Auth를 각각 호출한다.
...
| 회원가입 완료 | verified Registration과 User proof 확인 | 없음 | IdentityLink, Session, SessionCredential과 Registration `completed`를 한 트랜잭션에 저장 |
...
| 이메일/휴대폰 로그인 성공 | credential/연결 상태 읽기 | 없음 | 실패 횟수 초기화, Session, refresh credential, 감사 outbox |
...
| token refresh | Session/credential 상태 읽기 | 없음 | credential row lock, rotation/reuse 상태, 새 access JWT, 감사 outbox |
...
| 수동 인증 수단 변경 | 신뢰된 workload와 authorization/approval reference 확인 | Approval System | Identity/IdentityLink/Session, 감사 outbox |
| 사용자 상태 동기 적용 | user version 확인 | 없음 | UserAuthState와 전체 Session을 한 트랜잭션에 반영 |

회원가입 완료는 하나의 동기 Command다. User proof 검증이 실패하면 IdentityLink와 Session을 만들지 않는다. 같은 완료 요청 재시도는 기존 논리 Session을 재사용한다.
...
비밀번호, 인증번호, access token, refresh cookie/token 원문, 이메일/휴대폰 원문, provider secret, 이름, 주소, 동의 원문은 넣지 않는다. Audit Context에 검색용 연락처가 필요하더라도 별도 개인정보 토큰화 방식을 먼저 설계하고 이 allowlist를 암묵적으로 넓히지 않는다.

### 감사 이벤트 매핑

| 상태 변경 | Audit Event | 필수 reason/result |
| --- | --- | --- |
| 가입 검증 완료 | `auth.registration.verification_completed` | event version, result |
...
| Session 발급/refresh | `auth.session.issued`, `auth.session.refreshed` | channel, session id, policy version |
...
BC.A.300의 `EVT.A.300-12~17`, `EVT.A.300-21`은 위 외부 감사 event type으로 변환해 발행한다. “감사 전송됨”은 Aggregate 상태가 아니라 relay 전달 결과이므로 Domain Event와 Outbox delivery 상태를 구분한다. 도메인 모델의 `EVT.A.300-22~37`은 Challenge, Registration, PasswordReset, Session, AuthenticationIntent, 사용자 제한, 정책 변경과 로그인 실패·잠금 사실을 보강한다. 회원가입은 감사 사실을 기록하더라도 동기 API 트랜잭션으로 완료한다.
...
| --- | --- | --- | --- |
| auth DB 장애 | 공개 탐색, JWT 서명 검증과 유효한 Session 상태 cache hit | 가입, 로그인, refresh, 로그아웃, Session 상태 cache miss | 보호 API는 Session 상태를 확정할 수 없으면 fail closed, 즉시 경보 |
...
| 설정 그룹 | 항목 | 초기값/원칙 |
| --- | --- | --- |
| TokenTtlPolicy | access, refresh, remember-me, Session idle/absolute TTL | 15분, 14일, 30일, Session TTL은 운영 확정 |
| SigningKeyPolicy | algorithm, active/retiring key, rotation overlap, JWKS cache TTL | 초기 `RS256`, access TTL보다 긴 검증 overlap |
...
- refresh credential은 충분한 entropy의 opaque 값으로 만들고 keyed digest만 저장한다.
...
- 공개 오류는 계정 존재 여부를 드러내지 않는다. 상세 사유는 외부 인가를 통과한 CS 조회에서만 마스킹해 제공한다.
- 로그인, 복구, challenge 발급에는 대상+IP+device 기준 rate limit을 함께 적용한다.
- 웹 cookie는 HttpOnly, Secure, 적절한 SameSite와 path/domain 최소 범위를 사용한다.
- JWT, JWKS와 Istio 검증 및 내부 인증 헤더는 [SD.A.300.JWT](../jwt-jwks-istio.md)을 준수한다.
- 수동 처리와 고위험 조회는 최신 Session, 목적별 재인증 proof, authorization decision reference와 승인 기록을 확인한다.
...
- Query 응답의 이메일/휴대폰은 본인 또는 외부 인가를 통과한 운영자에게도 마스킹해서 제공한다.
...
| Session | `auth_session_issue_total`, `auth_refresh_total`, `auth_refresh_reuse_total` | channel, result |
...
구조화 로그의 공통 필드는 `timestamp`, `level`, `service`, `operation`, `result`, `reason_code`, `request_id`, `trace_id`, `correlation_id`, 필요 시 내부 `user_id`, `identity_id`, `session_id`, `token_id`, `policy_version`이다. 동적 ID는 로그에만 두고 metric label로 올리지 않는다.

로그인 실패는 INFO/WARN 등급을 공격 징후 기준으로 나누고, 예상 가능한 잘못된 비밀번호를 ERROR stack trace로 남기지 않는다. 반대로 저장소 오류, outbox 저장 실패, 외부 연동 명세 위반은 원인을 숨기지 않고 ERROR로 남긴다.

### Trace

- Handler, Repository, PasswordHasher, JWT signer, Session status check, 가입 검증 완료, 회원가입 완료, delivery relay, Audit relay를 span으로 구분한다.
...
| 요구사항 | 서비스 반영 |
| --- | --- |
| `FR-001`, `FR-002`, `FR-023`, `FR-024`, `FR-025` | Istio 보호 Route와 SessionStatusService가 JWT·Session 인증을 강제하고 공개/선택적 영역의 대체 처리는 UI 경계에 둔다. |
...
| `FR-020`, `FR-021` | Auth는 고위험 작업용 ReauthenticationProof만 발급하며 업무 권한 판정과 passkey 구현은 별도 경계·후속 범위로 남긴다. |
...
| 요구사항 | 서비스 반영 |
| --- | --- |
| `NFR-001`, `NFR-016`, `NFR-017` | Route별 공개/보호 인증 분류와 선택적 개인화 장애 격리 기준을 둔다. |
...
| `NFR-011`, `NFR-012`, `NFR-021` | 고위험 인증 proof와 후속 passkey, 외부 break-glass 승인 연동 지점을 둔다. |
| `NFR-013` | ID proof, 외부 access, 내부 Session artifact를 목적별로 구분한다. |
| `NFR-014` | Context 인증은 프로필과 권한을 저장·해석하지 않고 `user_id`와 인증/Session만 다룬다. |
...
| 단계 | 검증 항목 |
| --- | --- |
| Domain unit test | Registration 완료 조건, challenge TTL/소비, Identity 잠금, 단일 연결, Session rotation/reuse, PasswordReset 단일 소비, access claim allowlist |
...
| 호환성 테스트 | registration completion proof, user creation proof, JWT claim/JWKS, Session 상태 응답, Email/SMS idempotency key, Audit event schema |
...
| End-to-end test | 이메일 가입+자동 로그인, 휴대폰 로그인, 공통 refresh rotation, 비밀번호 재설정, 휴대폰 교체, 로그아웃 뒤 기존 JWT 차단 |
...
- 로그인과 refresh는 인가 원천에 의존하지 않으며 access JWT에는 인증 claim만 들어간다.
- Session 상태를 확인할 수 없으면 Istio 보호 Route가 요청을 fail closed로 거부한다.
...
- 별도 인가 경계가 Auth 운영 API에 전달할 authorization decision reference의 서명, audience, TTL과 재사용 방지 기준.
