---
id: BC.A.300
title: Auth Context 이벤트스토밍과 바운디드 컨텍스트
...
# Auth Context 이벤트스토밍과 바운디드 컨텍스트
...
- 핵심 용어: 사용자 계정, 인증 식별자, 인증 수단, `user_id`, redirect target, 세션, refresh rotation, 인증 게이트, 감사 이벤트, Audit Context.
...
- 이 BC가 참조하거나 요청하는 것: 홈/상품 상세/마이 같은 진입 화면, User Context가 발급한 `user_id`, 드롭 참여 가능성 판단에 필요한 사용자 인증 보유 여부, 판매자/운영자 역할 claim, CS/운영자 화면의 조회 요구.
...
- 경계 원칙: 인증 서비스는 `user_id`를 소유하지 않고, credential, IdentityLink, session, role/permission claim만 다룬다. 사용자 계정과 프로필은 User Context 책임이다.
- 감사 원칙: Auth Context는 감사 이벤트 저장소를 소유하지 않는다. 인증 결과별 감사 이벤트를 Audit Context로 전송하고, 수집/보관/검색/보존 기간은 Audit Context가 결정한다.
...
flowchart LR
  ...
  userContext[Context<br>User Context]
  auditContext[Context<br>Audit Context]
  futureProvider[External System<br>후속 인증 제공자]
  opsConsole[External System<br>운영 콘솔]

  subgraph AuthBC[Auth Context]
    direction LR
...
| Command | CMD.A.300-01 | 인증 게이트 확인 | Auth Context | 사용자의 화면/행동/API 접근이 공개인지 인증 필요인지 판단한다. |
| Command | CMD.A.300-02 | 로그인 의도 보존 | Auth Context | 로그인 진입 시 복귀 위치와 intent를 보존한다. |
| Command | CMD.A.300-03 | 이메일 회원가입 | Auth Context | 이메일, 비밀번호, 이름, 휴대폰 번호, 약관 동의 기반 가입을 요청한다. |
| Command | CMD.A.300-04 | 이메일 소유 확인 | Auth Context | 회원가입 또는 복구 과정에서 이메일 소유를 확인한다. |
| Command | CMD.A.300-05 | 휴대폰 소유 확인 | Auth Context | 가상 SMS 인증으로 휴대폰 소유를 확인한다. |
| Command | CMD.A.300-06 | 인증 식별자 연결 | Auth Context | User Context가 발급한 `user_id`와 검증된 인증 식별자를 연결한다. |
| Command | CMD.A.300-07 | 이메일 로그인 | Auth Context | 이메일과 비밀번호로 기존 인증 식별자를 검증한다. |
| Command | CMD.A.300-08 | 휴대폰 번호 로그인 | Auth Context | 연결된 휴대폰 인증 식별자로 기존 `user_id` 로그인을 요청한다. |
| Command | CMD.A.300-09 | 세션 발급 | Auth Context | access token, refresh token, remember-me 세션을 발급한다. |
| Command | CMD.A.300-10 | 토큰 재발급 | Auth Context | refresh rotation 정책에 따라 새 token을 발급한다. |
| Command | CMD.A.300-11 | 로그아웃 | Auth Context | 현재 세션 또는 refresh token을 폐기한다. |
| Command | CMD.A.300-12 | 인증 수단 연동 요청 | Auth Context | 현재 `user_id`에 추가 인증 식별자 연결을 요청한다. |
| Command | CMD.A.300-13 | 인증 수단 수동 처리 | Auth Context | CS/플랫폼 운영자의 승인과 사유로 인증 수단 해제 또는 재연동을 처리한다. |
| Command | CMD.A.300-14 | 휴대폰 번호 셀프 변경 | Auth Context | 안전하게 로그인된 사용자가 대체 인증 수단으로 재인증한 뒤 새 휴대폰 번호로 교체한다. |
| Command | CMD.A.300-15 | 휴대폰 번호 변경 재인증 | Auth Context | 이메일 로그인/재인증과 새 휴대폰 SMS 인증으로 셀프 변경 가능 조건을 확인한다. |
| Aggregate | AGG.A.300-01 | Identity | Auth Context | 이메일, 휴대폰 번호, ProviderSubject 같은 인증 식별자와 검증 상태를 관리한다. |
| Aggregate | AGG.A.300-02 | IdentityLink | Auth Context | `identity_id`와 User Context가 발급한 `user_id`의 연결 상태를 관리한다. |
| Aggregate | AGG.A.300-03 | Session | Auth Context | access token, refresh token, remember-me, rotation, 만료 상태를 관리한다. |
| Domain Event | EVT.A.300-01 | 인증 게이트 적용됨 | Auth Context | 개인/결제/드롭 참여 행동에 인증 필요 판정이 적용된 결과다. |
| Domain Event | EVT.A.300-02 | 로그인 의도 보존됨 | Auth Context | 로그인 후 복귀할 redirect target 또는 intent가 저장된 결과다. |
| Domain Event | EVT.A.300-03 | 인증 식별자 연결됨 | Auth Context | 검증된 인증 식별자가 User Context가 발급한 `user_id`에 연결된 결과다. |
| Domain Event | EVT.A.300-04 | 이메일 인증 식별자 확인됨 | Auth Context | 이메일 소유 확인이 완료된 결과다. |
| Domain Event | EVT.A.300-05 | 휴대폰 인증 식별자 확인됨 | Auth Context | 가상 SMS 인증을 통과해 휴대폰 소유가 확인된 결과다. |
| Domain Event | EVT.A.300-06 | 사용자 로그인됨 | Auth Context | 이메일 또는 연결된 휴대폰 인증 식별자로 사용자가 확인된 결과다. |
| Domain Event | EVT.A.300-07 | 세션 발급됨 | Auth Context | 인증 성공 후 token과 만료 시각이 발급된 결과다. |
| Domain Event | EVT.A.300-08 | 토큰 재발급됨 | Auth Context | refresh token 검증과 rotation 후 새 token이 발급된 결과다. |
| Domain Event | EVT.A.300-09 | 사용자 로그아웃됨 | Auth Context | 세션 또는 refresh token이 폐기된 결과다. |
| Domain Event | EVT.A.300-10 | 인증 수단 연동 요청됨 | Auth Context | 추가 인증 식별자 연결 시도가 확인된 결과다. |
| Domain Event | EVT.A.300-11 | 인증 수단 수동 변경됨 | Auth Context | CS/플랫폼 운영자 절차로 인증 수단이 해제 또는 재연동된 결과다. |
| Domain Event | EVT.A.300-12 | 로그인 성공 감사 전송됨 | Auth Context | 로그인 성공 사실을 Audit Context로 전송한 결과다. |
| Domain Event | EVT.A.300-13 | 로그인 실패 감사 전송됨 | Auth Context | 로그인 실패 사실을 Audit Context로 전송한 결과다. |
| Domain Event | EVT.A.300-14 | 토큰 재발급 감사 전송됨 | Auth Context | token 재발급 사실을 Audit Context로 전송한 결과다. |
| Domain Event | EVT.A.300-15 | 로그아웃 감사 전송됨 | Auth Context | 로그아웃 사실을 Audit Context로 전송한 결과다. |
| Domain Event | EVT.A.300-16 | 인증 수단 변경 감사 전송됨 | Auth Context | 인증 수단 연동/해제/재연동 결과를 Audit Context로 전송한 결과다. |
| Domain Event | EVT.A.300-17 | 계정 잠금 감사 전송됨 | Auth Context | 로그인 실패 임계치 도달과 잠금 적용 사실을 Audit Context로 전송한 결과다. |
| Domain Event | EVT.A.300-18 | 휴대폰 번호 변경 요청됨 | Auth Context | 사용자가 기존 `user_id`에 연결된 휴대폰 인증 식별자 교체를 요청한 결과다. |
| Domain Event | EVT.A.300-19 | 새 휴대폰 인증 식별자 확인됨 | Auth Context | 새 휴대폰 번호의 SMS 소유 확인이 완료된 결과다. |
| Domain Event | EVT.A.300-20 | 휴대폰 인증 식별자 교체됨 | Auth Context | 기존 휴대폰 인증 식별자가 닫히고 새 휴대폰 인증 식별자가 같은 `user_id`에 연결된 결과다. |
| Domain Event | EVT.A.300-21 | 휴대폰 번호 변경 감사 전송됨 | Auth Context | 휴대폰 인증 식별자 교체 결과를 Audit Context로 전송한 결과다. |
| Policy | POLICY.A.300-01 | 공개 탐색 허용 | Auth Context | 홈, 목록, 상세, 검색, 공지는 비회원 접근을 막지 않는다. |
| Policy | POLICY.A.300-02 | 내부 복귀 위치만 허용 | Auth Context | redirect target은 검증된 내부 경로 또는 intent id만 허용한다. |
| Policy | POLICY.A.300-03 | 인증 식별자 단일 연결 | Auth Context | 하나의 인증 식별자는 하나의 `user_id`에만 연결된다. |
| Policy | POLICY.A.300-04 | 로그인 실패 5회 잠금 | Auth Context | 동일 인증 식별자의 실패가 5회에 도달하면 전역 잠금 정책을 적용한다. |
| Policy | POLICY.A.300-05 | 고위험 인증 수단 변경 분기 | Auth Context | 안전하게 로그인되어 있고 대체 인증 수단으로 재인증한 사용자는 휴대폰 번호 셀프 변경을 할 수 있으며, 접근 불가 또는 대체 인증 불가 상태는 CS/플랫폼 운영자 수동 처리로 보낸다. |
| Policy | POLICY.A.300-06 | 후속 인증 MVP 제외 | Auth Context | Apple/Google, 네이버/토스/PASS/카카오톡, passkey는 요구사항에는 남기되 MVP 구현에서 제외한다. |
| Policy | POLICY.A.300-07 | 인증 책임 최소화 | Auth Context | 인증 서비스는 `user_id`를 소유하지 않고 인증 식별자 연결과 세션만 관리한다. |
| Business Rule | RULE.A.300-01 | 이메일과 휴대폰 모두 확인 | Auth Context | 이메일 회원가입은 이메일 인증과 가상 SMS 휴대폰 인증 식별자 연결을 모두 만족해야 완료된다. |
| Business Rule | RULE.A.300-02 | 연결된 휴대폰만 로그인 | Auth Context | 휴대폰 번호 로그인은 이미 연결된 휴대폰 인증 식별자가 있을 때만 기존 `user_id`로 로그인한다. |
| Business Rule | RULE.A.300-03 | 인증 식별자 연결만 수행 | Auth Context | Auth Context는 사용자 계정을 만들거나 병합하지 않고 인증 식별자와 `user_id` 연결만 관리한다. |
| Business Rule | RULE.A.300-04 | 세션 TTL과 회전 관리 | Auth Context | access token 15분, refresh token 14일, remember-me 30일, refresh rotation 사용을 기본값으로 둔다. |
| Business Rule | RULE.A.300-05 | 감사 이벤트 분리 전송 | Auth Context | 인증 결과별 감사 이벤트를 구분해 Audit Context로 전송한다. |
| Business Rule | RULE.A.300-06 | 휴대폰 번호 변경은 계정 병합이 아님 | Auth Context | 휴대폰 번호 변경은 기존 `user_id`의 휴대폰 인증 식별자를 교체하는 작업이며 다른 `user_id`와의 병합으로 처리하지 않는다. |
| Hotspot | HOTSPOT.A.300-01 | SMS 제한 정책 | Auth Context | 가상 SMS 인증번호 TTL, 재전송 제한, 실패 횟수, 테스트 번호 정책을 정해야 한다. |
| Hotspot | HOTSPOT.A.300-02 | 수동 처리 승인 필드 | Auth Context | CS/플랫폼 운영자의 승인자, 사유, 증빙, 감사 필드 기준을 정해야 한다. |
| Hotspot | HOTSPOT.A.300-03 | 번호 변경 복구 기준 | Auth Context | 이메일 재인증이 불가능하거나 계정 접근이 불가능할 때 CS 전환 기준을 정해야 한다. |
| Hotspot | HOTSPOT.A.300-04 | 후속 ProviderSubject 정책 | Auth Context | Apple/Google과 국내 인증 ProviderSubject 저장, 충돌, 연동 정책을 정해야 한다. |
| Hotspot | HOTSPOT.A.300-05 | 초기 TTL 운영 확인 | Auth Context | 초기 token TTL과 remember-me 기본값이 MVP 운영 정책에 맞는지 확인해야 한다. |
| Context | CTX.A.300-01 | User Context | Auth Context 외부 | `user_id` 발급과 사용자 계정 생명주기를 담당한다. |
| Context | CTX.A.300-02 | Audit Context | Auth Context 외부 | 인증 감사 이벤트의 수집, 저장, 검색, 보존 기간을 담당한다. |
...
| Read Model | RM.A.300-01 | 인증 진입 컨텍스트 | Auth Context | 로그인 후 복귀할 redirect target, intent, 지원 인증 수단 노출 상태를 제공한다. |
| Read Model | RM.A.300-02 | 인증 상태 조회 | Auth Context | 로그인 실패, 잠금, 인증 수단 연결 상태, 감사 상태를 CS/운영자에게 제공한다. |
| Read Model | RM.A.300-03 | 세션 정책 조회 | Auth Context | access/refresh/remember-me TTL과 refresh rotation 설정을 제공한다. |
...
| EVT.A.300-03 인증 식별자 연결됨 | [REQ.A.05](../00-requirements/REQ_A_05_auth_member.md), [UC.A.300](../30-uc/UC_A_300_auth_member.md) | 검증된 이메일/휴대폰 인증 식별자가 User Context가 발급한 `user_id`에 연결되어야 한다. |
...
| EVT.A.300-12 로그인 성공 감사 전송됨 | [REQ.A.05](../00-requirements/REQ_A_05_auth_member.md), [UC.A.300](../30-uc/UC_A_300_auth_member.md) | 로그인 성공은 별도 감사 이벤트로 Audit Context에 전송한다. |
...
| EVT.A.300-21 휴대폰 번호 변경 감사 전송됨 | [REQ.A.05](../00-requirements/REQ_A_05_auth_member.md), [UC.A.300](../30-uc/UC_A_300_auth_member.md) | 휴대폰 인증 식별자 교체는 Audit Context로 별도 감사 이벤트를 전송해야 한다. |
...
| RULE.A.300-03 인증 식별자 연결만 수행 | [REQ.A.05](../00-requirements/REQ_A_05_auth_member.md) | 이메일, 휴대폰, ProviderSubject가 같거나 달라도 Auth Context는 사용자 계정 생성이나 병합을 수행하지 않는다. |
...
| RULE.A.300-05 감사 이벤트 분리 전송 | [REQ.A.05](../00-requirements/REQ_A_05_auth_member.md), [UC.A.300](../30-uc/UC_A_300_auth_member.md) | Auth Context는 로그인 성공/실패, token 갱신, 로그아웃, 계정 잠금, 인증 수단 변경을 서로 다른 감사 이벤트로 전송한다. |
...
| CTX.A.300-01 User Context | [REQ.A.05](../00-requirements/REQ_A_05_auth_member.md), [UC.A.300](../30-uc/UC_A_300_auth_member.md) | 사용자 식별자는 `user_id` 중심이지만, Auth Context는 사용자 프로필과 계정 생명주기를 소유하지 않는다. |
| CTX.A.300-02 Audit Context | [REQ.A.05](../00-requirements/REQ_A_05_auth_member.md), [UC.A.300](../30-uc/UC_A_300_auth_member.md) | 인증 관련 행위는 민감 원문 없이 감사 이벤트로 남아야 하지만, 저장과 검색 책임은 Auth Context 밖으로 분리한다. |
...
| 이메일 회원가입 | 요청한다 | User Context | 인증 검증이 끝나면 `user_id` 발급을 요청한다. |
...
| User Context | 요청한다 | 인증 식별자 연결 | 발급한 `user_id`를 Auth Context에 전달한다. |
...
| 로그인 성공 감사 전송됨 | 요청한다 | Audit Context | Auth Context 밖의 감사 수집 체계로 전송한다. |
| 로그인 실패 감사 전송됨 | 요청한다 | Audit Context | Auth Context 밖의 감사 수집 체계로 전송한다. |
| 계정 잠금 감사 전송됨 | 요청한다 | Audit Context | Auth Context 밖의 감사 수집 체계로 전송한다. |
| 토큰 재발급 감사 전송됨 | 요청한다 | Audit Context | Auth Context 밖의 감사 수집 체계로 전송한다. |
| 로그아웃 감사 전송됨 | 요청한다 | Audit Context | Auth Context 밖의 감사 수집 체계로 전송한다. |
| 인증 수단 변경 감사 전송됨 | 요청한다 | Audit Context | Auth Context 밖의 감사 수집 체계로 전송한다. |
| 휴대폰 번호 변경 감사 전송됨 | 요청한다 | Audit Context | Auth Context 밖의 감사 수집 체계로 전송한다. |
...
| 인증 식별자 연결만 수행 | 규정한다 | IdentityLink | Auth Context는 사용자 계정 생성이나 병합을 수행하지 않는다. |
| 세션 TTL과 회전 관리 | 규정한다 | Session | token TTL과 refresh rotation을 설정으로 관리한다. |
| 감사 이벤트 분리 전송 | 규정한다 | Audit Context | 인증 결과별 이벤트를 구분해 전송하고 저장 책임은 Audit Context에 둔다. |
...
| 용어 | 의미 | 혼동하기 쉬운 용어 |
| --- | --- | --- |
| 사용자 계정 | User Context가 소유하는 내부 사용자 주체이며 `user_id`로 식별된다. | 인증 식별자 |
| 인증 식별자 | 이메일, 휴대폰 번호, ProviderSubject처럼 로그인 또는 소유 증명에 쓰는 식별자다. | 사용자 계정 |
| 인증 수단 | 인증 식별자를 만들거나 검증하는 방식이다. | 인증 식별자 |
| 인증 수단 연동 | User Context가 발급한 `user_id`에 새 인증 식별자를 연결하는 작업이다. | 사용자 계정 병합 |
...
| 감사 이벤트 | Auth Context가 인증 결과별 의미를 구분해 Audit Context로 전송하는 이벤트다. | 애플리케이션 로그 |
| Audit Context | 감사 이벤트 수집, 저장, 검색, 보존 기간을 담당하는 별도 컨텍스트다. | Auth Context |
...
| 서비스 | 인증 게이트, 인증 검증, User Context `user_id` 발급 요청, 인증 식별자 연결, 로그인, token 갱신, 로그아웃, 휴대폰 번호 셀프 변경, 인증 수단 수동 처리 유스케이스 서비스를 분리한다. | SVC.A.300 예정 |
...
| 구독 Event | User Context의 `user_id` 발급 결과와 주문/쿠폰/드롭 참여 BC의 사용자 인증 보유 여부 참조 방식을 정한다. | SVC.A.300 예정 |
| 외부 연동 | 이메일 발송, 가상 SMS, Apple/Google, 네이버/토스/PASS/카카오톡, passkey 연동은 MVP와 후속 구현 범위를 나눠 설계한다. Audit Context 전송은 컨텍스트 간 이벤트 연동으로 분리한다. | API.A.300 예정 |
