...
| Policy | POLICY.A.07-02 | 조회수 중복 방지 | Context 랭킹 집계 | 반복 조회를 막지 않고 원문 기록 후, 랭킹 집계 시점에 `COUNT(DISTINCT user_id)`로 세어 같은 사용자를 1명으로 처리한다(2026-07-14 수정: Redis `SETNX` 방식에서 변경, `REQ.A.07` 수정 이력 참고). |
| Policy | POLICY.A.07-03 | 핫키 완충 처리 | Context 랭킹 집계 | 특정 드롭에 write가 몰려도 다른 드롭 조회 응답에 영향 없게 완충한다. |
| Business Rule | RULE.A.07-01 | 리셋 없는 누적 카운트 | Context 랭킹 집계 | 오픈 전 카운터는 리셋 없이 누적 활성 찜 수를 유지하고 감쇠 계산을 쓰지 않는다(2026-07-14 수정: 기존 "당일 00시 리셋"에서 변경, `REQ.A.07` 수정 이력 참고). |
...
| Read Model | RM.A.07-03 | 오픈 예정 랭킹 조회 | Context 랭킹 집계 | 드롭 상태 구분 없이 누적 활성 찜 수 기준 랭킹을 보여준다(2026-07-14 수정). |
...
| RULE.A.07-01 리셋 없는 누적 카운트 | `REQ.A.07.FR-003`, `REQ.A.07.NFR-006` | 오픈 전 랭킹은 감쇠 계산 없이 리셋 없는 누적 카운트만 사용해야 한다(2026-07-14 수정). |
...
| 오픈 전 랭킹 | 드롭 상태 구분 없이 누적 활성 찜 수(리셋 없음)로 정렬한 목록이다(2026-07-14 수정: `SCHEDULED` 필터·조회수·당일 리셋 제거, `REQ.A.07` 수정 이력 참고). | 급상승 랭킹(속도/감쇠 기반 알고리즘, 이 도메인에서는 사용하지 않음) |
...
| 항목 | 메모 | 연결 문서 |
| --- | --- | --- |
| 구현 스코프 축소(2026-07-14) | 1차 구현은 `CMD.A.07-05`(오픈전 카운터 반영, 리셋 없는 누적)만 다룬다. `CMD.A.07-02`(조회 기록, Redis 필요), `CMD.A.07-03`(랭킹 리스트 전환, catalog 이벤트 필요), `CMD.A.07-04`(오픈 후 점수 갱신, order-service 연동 방식 미확정)는 후속 스코프다. 다이어그램/Element Catalog의 해당 요소는 목표 설계로 남기고 지우지 않았다. | `REQ.A.07` 수정 이력, [SD.A.07](../50-service-design/A_07_interest_ranking/README.md) |
