...
| `ACTOR-ORDER-PAYMENT` | 주문/결제 시스템 | 보조 액터 | 쿠폰 조건과 할인 금액을 검증하고 예약한 뒤, 결제 최종 확정 또는 취소 결과를 전달한다. | `PAGE.A.11`, `UC.A.01` |
...
| `UC.A.19-04` | 쿠폰 수령 | 쿠폰 받기 또는 수령 액션으로 발급을 요청한다. 요청 직후에는 발급 대기를 확인하고, `UserCoupon` 생성과 수량 확정 뒤에 발급 완료를 확인한다. | `REQ.A.02.FR-003`, `REQ.A.02.FR-004`, `REQ.A.02.FR-005`, `REQ.A.02.FR-013` |
| `UC.A.19-05` | 쿠폰 코드 등록 | 이벤트나 프로모션으로 받은 쿠폰 코드를 입력해 보유 쿠폰으로 등록한다. | `REQ.A.02.FR-025`, `REQ.A.02.NFR-005` |
| `UC.A.19-06` | 주문 쿠폰 사용 | 주문/결제에서 현재 주문에 사용할 쿠폰을 선택하고 할인 결과를 확인한다. 선택한 쿠폰은 예약 시 다른 주문에서 쓰지 못하도록 잠기며 결제 최종 확정 뒤에만 사용 완료가 된다. | `REQ.A.02.FR-007`, `REQ.A.02.FR-008`, `REQ.A.02.FR-009`, `REQ.A.02.FR-010` |
...
| `UC.A.19-08` | 판매자 | 판매자 쿠폰 성과 조회 | 자신이 소유하거나 비용을 부담한 캠페인의 캠페인·드롭·상품·일자별 집계, 기준 시각과 자기 부담 비용을 확인한다. 사용자, 쿠폰 코드와 주문 상세는 확인하지 않는다. | `REQ.A.02.FR-014` |
...
| `UC.A.19-11` | 마케팅 담당자, DropMong 운영자 | 대량 발급 작업 등록 | `evaluationAsOf` 시점의 발급 대상을 고정해 작업을 등록하고 재시도·승인된 최종 실패 상태를 확인한다. | `REQ.A.02.FR-019`, `REQ.A.02.NFR-019` |
...
| UC ID | 액터 | 사용자 목표 | 설명 | 연결 요구사항 |
| --- | --- | --- | --- | --- |
| `UC.A.19-13` | DropMong 운영자 | 쿠폰 정책 변경 | 발급 쿠폰과 진행 중 예약에 고정된 정책 버전을 보존하고 새 발급·예약부터 새 버전을 적용한다. 긴급 중지는 별도 운영 제어로 처리한다. | `REQ.A.02.FR-020`, `REQ.A.02.NFR-020` |
| `UC.A.19-14` | DropMong 운영자 | 판매자/제휴 쿠폰 검토 | 부담 주체, 소유 범위, 승인 템플릿, 금액·수량과 증빙을 버전이 있는 위험 정책으로 검토해 승인, 반려, 보류한다. | `REQ.A.02.FR-023`, `REQ.A.02.NFR-024` |
...
| `UC.A.19-17` | DropMong 운영자, 온콜 담당자 | 쿠폰 이벤트 재처리 | 설정 기반 지수 백오프로 실패 이벤트를 재처리한다. 재시도 한도 소진 뒤에도 자동 종료하지 않고 승인된 최종 실패 명령으로만 종료한다. | `REQ.A.02.FR-015`, `REQ.A.02.NFR-010` |
...
| `UC.A.19-20` | CS 담당자 | 보상 쿠폰 발급 요청 | `caseRef`, 사유와 주문 또는 인시던트 참조를 첨부해 보상 쿠폰 발급을 요청한다. 고액·대량 보상과 정책 한도 초과 요청은 운영 승인을 받는다. | `REQ.A.02.FR-021`, `REQ.A.02.FR-023`, `REQ.A.04.FR-011` |
...
| `UC.A.19-24` | 주문/결제 시스템 | 쿠폰 사용 확정 | 결제 최종 확정 사건을 근거로 예약 쿠폰을 사용 완료로 전환한다. 예약 시점부터 다른 주문의 중복 사용을 막는다. | `REQ.A.02.FR-010`, `REQ.A.02.NFR-004` |
| `UC.A.19-25` | 주문/결제 시스템 | 쿠폰 사용 취소/회수 | 확정 실패·취소는 예약을 즉시 해제하고, 결과가 불명확할 때만 짧은 유예를 적용한다. 사용 완료 쿠폰은 검증된 취소·환불, 유효기간, 캠페인 상태와 운영 중지를 확인한 경우에만 다시 사용할 수 있게 한다. | `REQ.A.02.FR-011`, `REQ.A.02.NFR-014`, `REQ.A.02.NFR-018` |
...
| `REQ.A.02.FR-025~026` | `UC.A.19-05` | 코드 검증 결과는 쿠폰 코드 등록의 결과 상태로 다룬다. 자동 지급은 사용자 목표가 아니며 검증된 외부 사건의 처리 결과로만 다룬다. |
...
- 시스템 자동 지급은 생일·생년월일을 사용하거나 저장하지 않는다. 등급 변경, 캠페인 참여 완료, 승인된 보상처럼 개인정보 원문이 필요 없는 검증된 사건만 허용하며, 생산자 계약이 확정될 때까지 활성화하지 않는다.
...
- 위험 기반 승인 정책의 초기 금액·수량 한도와 승인자 배치를 정해야 한다.
...
판매자 성과 범위, 발급 상태 표현과 위험 기반 승인 원칙은 [Context 쿠폰 Hotspot 결정 기록](../50-service-design/A_19_coupon/hotspot-decisions.md)에서 확정했다.
