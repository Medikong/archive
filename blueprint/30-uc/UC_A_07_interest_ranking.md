...
flowchart LR
    ...
    subgraph InterestRankingSystem["DropMong 관심 신호 및 랭킹 시스템"]
        ...
        UC0704(("UC.A.07-04<br/>기다리는 상품 랭킹 조회"))
        UC0705(("UC.A.07-05<br/>드롭 관심도 통계 조회"))
        UC0706(("UC.A.07-06<br/>실시간 많이 보는 상품 랭킹 조회"))
    end
...
| `UC.A.07-04` | 구매자 | 기다리는 상품 랭킹 조회(구 "오픈 예정 랭킹") | 드롭 상태 구분 없이, 누적 활성 찜 수가 많은 순으로 정렬된 랭킹을 확인한다(2026-07-14 수정: `SCHEDULED` 필터/당일 리셋 제거하고 이름도 실제 의미에 맞게 개명, `REQ.A.07` 수정 이력 참고). | `REQ.A.07.FR-003` |
| `UC.A.07-05` | DropMong 운영자, 브랜드 운영자 | 드롭 관심도 통계 조회 | 드롭별 찜 수, 조회수 통계를 조회해 운영 판단이나 자사 드롭 반응 확인에 사용한다. | `REQ.A.07.FR-008` |
| `UC.A.07-06` | 구매자 | 실시간 많이 보는 상품 랭킹 조회(2026-07-14 신설) | 드롭 상태 구분 없이, 최근 3시간 내 서로 다른 조회자 수가 많은 순으로 정렬된 랭킹을 확인한다. | `REQ.A.07.FR-011` |

## 상태/결과 메모

- (2026-07-14 추가) `UC.A.07-03`(오픈 후 인기 랭킹 조회)은 order-service 연동 방식이 미확정이라 이번 구현 슬라이스에서 후속 스코프로 미뤘다. `UC.A.07-04`(오픈 예정 랭킹 조회)만 지금 구현한다. 자세한 내용은 `REQ.A.07`의 "2026-07-14 수정 이력" 참고.
