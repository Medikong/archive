...
보유 쿠폰 페이지는 구매자가 마이 페이지에서 진입해 발급 대기, 사용 가능, 사용 완료, 만료 쿠폰을 확인하고, 받을 수 있는 쿠폰을 수령하거나 이벤트와 프로모션으로 받은 쿠폰 코드를 등록하는 화면 묶음이다.
...
<div style="display: flex; gap: 12px; align-items: flex-start; padding: 8px 0;">
  <figure style="flex: 1 1 0; margin: 0;">
    <img src="../../../20-ui/buyer-mobile-web/assets/UI_A_10_my/UI_A_10_01_my.png" alt="마이 페이지" style="width: 100%; height: auto;" />
    <figcaption>1. 마이 페이지<br/>쿠폰함 진입</figcaption>
  </figure>
  <figure style="flex: 1 1 0; margin: 0;">
    <img src="../../../20-ui/buyer-mobile-web/assets/UI_A_19_coupon_wallet/UI_A_19_01_owned_coupon.png" alt="보유 쿠폰" style="width: 100%; height: auto;" />
    <figcaption>2. 보유 쿠폰</figcaption>
  </figure>
  <figure style="flex: 1 1 0; margin: 0;">
    <img src="../../../20-ui/buyer-mobile-web/assets/UI_A_19_coupon_wallet/UI_A_19_02_coupon_code_register.png" alt="쿠폰 코드 등록" style="width: 100%; height: auto;" />
...
| 보유 쿠폰 | 필터 탭 | 전체, 발급 대기, 사용 가능, 사용 완료, 만료 | 상태별 목록 필터링 | 쿠폰 목록 조회 |
...
| 쿠폰 수령 선택 | 현재 화면 내부 상태 변경 | 요청 접수 뒤 발급 대기를 표시하며, `UserCoupon` 생성과 수량 확정 뒤에만 발급 완료로 갱신 |
...
| 전체/발급 대기/사용 가능/사용 완료/만료 탭 선택 | 현재 화면 내부 상태 변경 | 쿠폰 상태 필터 적용 |
...
| 필터 상태 | 전체, 발급 대기, 사용 가능, 사용 완료, 만료 | 클라이언트 상태 |
...
| 수령 가능 쿠폰 있음 | 수령 CTA를 표시하고 요청 접수 시 발급 대기 상태로 목록을 갱신한다. | 선착순/대상 조건 검증 필요 |
| 쿠폰 수령 대기 | `발급 대기` 상태와 처리 안내를 표시한다. | 접수·대기와 발급 완료를 구분한다. |
| 쿠폰 발급 완료 | `UserCoupon` 생성과 수량 확정이 끝난 뒤 사용 가능 상태와 완료 안내를 표시한다. | 처리 완료 전에는 표시하지 않는다. |
...
🏷️ 요구사항 참조: [REQ.A.02](../../../00-requirements/REQ_A_02_coupon_benefit.md) | 플로우 참조: FLOW.A.19 | UI 참조: [UI.A.19](../../../20-ui/buyer-mobile-web/UI_A_19_coupon_wallet/UI_A_19_coupon_wallet.md) | UC 참조: [UC.A.19](../../../30-uc/UC_A_19_coupon_wallet.md) | 영속성 참조: PST.A.19 예정 | 서비스 참조: SVC.A.19 예정 | 시나리오 참조: SCN.A.19 예정 | API 참조: API.A.19 예정

## 결정 반영

- 쿠폰 수령 요청 직후에는 발급 대기를 표시하고, `UserCoupon` 생성과 수량 확정 뒤에만 발급 완료로 표시한다. 문구와 상태 의미는 [Context 쿠폰 Hotspot 결정 기록](../../../50-service-design/A_19_coupon/hotspot-decisions.md)을 따른다.
