...
- UI ID: `UI.A.01`
- 연관 Page: [PAGE.A.01](../../10-sitemap/buyer-mobile-web/PAGE_A_01_homepage.md)
...
- 파일 경로:
  ...
  - [구매자 모바일 웹 시안](assets/UI_A_01_homepage/UI_A_01_10_buyer_mobile_web.png)
- 원본 URL: local
- 작성 일시: 기존 근거 2026-07-07, 모바일 웹 시안 2026-07-10, 랭킹 섹션 개편 2026-07-14
- 기존 근거 조건: DropMong 메인 홈, 추천 드롭/오픈 예정/큐레이션/카테고리/브랜드관/실시간 랭킹 노출
- 모바일 웹 시안 조건: 390px 브라우저 화면, 전역 하단 내비게이션 생략, 페이지 내부 콘텐츠와 주요 CTA 중심
- 2026-07-14 수정: 기존 단일 "실시간 랭킹" 섹션을 `REQ.A.07`이 정의한 두 독립 랭킹 — "기다리는 상품"(누적 찜, `API.A.07-06`)과 "실시간 많이 보는 상품"(최근 3시간 조회자 수, `API.A.07-08`) — 로 분리했다. 이 구조를 홈 화면, 컴포넌트 시트, 구매자 모바일 웹 시안 세 파일에 모두 적용했다. 랭킹 썸네일은 세로 크롭과 상품별 스케일 차이를 없애기 위해 흰색 스튜디오 배경, 정면 구도, 동일 여백의 제품 컷으로 통일했다. 각 `전체 보기`는 [UI.A.09](UI_A_09_waiting_products.md)와 [UI.A.23](UI_A_23_trending_products.md)으로 연결한다.

## 연관 태그

🏷️ 요구사항 참조: [REQ.A.01](../../00-requirements/REQ_A_01_limited_drop_commerce.md) | 페이지 참조: [PAGE.A.01](../../10-sitemap/buyer-mobile-web/PAGE_A_01_homepage.md) | UC 참조: UC.A.01 | 영속성 참조: PST.A.01 | 서비스 참조: SVC.A.01 | 시나리오 참조: SCN.A.01 | API 참조: API.A.01

## 에셋

### 구매자 모바일 웹 시안

![구매자 모바일 웹 홈 시안](assets/UI_A_01_homepage/UI_A_01_10_buyer_mobile_web.png)
...
| 7a | 기다리는 상품 리스트(2026-07-14 개편, 구 "실시간 랭킹") | 누적 찜 수 기준 Top 3를 보여준다(`API.A.07-06`). | 상품 상세 이동, 찜 수 표시, 전체 보기(최대 100위) |
| 7b | 실시간 많이 보는 상품 리스트(2026-07-14 신설) | 최근 3시간 서로 다른 조회자 수 기준 Top 3를 보여준다(`API.A.07-08`, 3시간마다 갱신). | 상품 상세 이동, 조회자 수 표시, 전체 보기(최대 100위) |
...
| 기다리는 상품 | `UpcomingRankingItem[]`(`API.A.07-06`) | object[] | 기다리는 상품 리스트 표시(홈은 `limit=3`, 전체보기는 `limit` 최대 100) |
| 기다리는 상품 | `UpcomingRankingItem.interestCount` | number | 찜 수 표시 |
| 실시간 많이 보는 상품 | `TrendingRankingItem[]`(`API.A.07-08`) | object[] | 실시간 많이 보는 상품 리스트 표시 |
| 실시간 많이 보는 상품 | `TrendingRankingItem.rank` | number | 순위 표시 |
| 실시간 많이 보는 상품 | `TrendingRankingItem.viewerCount` | number | 조회자 수 표시 |
| 실시간 많이 보는 상품 | `bucketStart` | datetime | "최근 3시간 기준" 갱신 시각 안내(선택) |
...
- Read Model 후보: `RM.A.01 HomeReadModel`, `RM.A.07-03`(기다리는 상품, `DropInterestCounterRepository.listByInterestCount`), `RM.A.07-06`(실시간 많이 보는 상품, `DropViewRankingRepository.getLatestBucket`) — 2026-07-14 추가
...
- 권한 후보: 홈/상품 탐색은 비회원 조회 가능, 알림/마이/장바구니는 로그인 필요 가능. 랭킹 두 API(`API.A.07-06`/`-08`)는 인증 불필요.
...
- (2026-07-14 해소) ~~실시간 랭킹 산정 기준과 갱신 주기~~ — `REQ.A.07` 2026-07-14 수정 이력에서 확정(기다리는 상품: 누적 찜, 리셋 없음 / 실시간 많이 보는 상품: 최근 3시간 조회자 수, 3시간마다 배치 갱신).
- 비회원이 알림 신청을 눌렀을 때 로그인 후 의도를 이어갈지 여부
- (2026-07-14 해소) 각 `전체 보기`는 고유 URL의 `PAGE.A.09`/`PAGE.A.23`으로 이동하며, 두 화면 안의 공통 탭으로 서로 전환한다.
