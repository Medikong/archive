...
3. 사용자 상태와 인가 차단 정책은 각각 어느 도메인이 소유하나요? → [상태와 정책 소유권](#63-상태와-정책-소유권)
4. 비활성 같은 사용자 상태 전이는 MVP에서 제외하고, 접근 차단은 auth-service 정책으로만 다룰까요? → [MVP 상태 정책 범위](#64-mvp-상태-정책-범위)
5. auth-service 내부 정책 모델은 role, ACL override 중 어디까지 책임지나요? → [auth-service 내부 정책 범위](#65-auth-service-내부-정책-범위)
...
9. Istio IngressGateway의 `X-User-*` 헤더 정규화 책임은 별도 경계로 문서화할까요? → [접근 경계와 사용자 식별 헤더 정규화](#69-접근-경계와-사용자-식별-헤더-정규화)

정책 서비스는 별도로 만들지 않는다. auth-service가 role grant와 ACL override를 소유하고, 접근 경계는 검증된 JWT claim을 `X-User-*` 헤더로 전달한다. 도메인 서비스는 인증/인가가 필요한 API에서만 `X-User-*`를 애플리케이션 레벨 Principal 또는 AuthContext로 변환한 뒤 자기 리소스의 소유권과 도메인 상태를 기준으로 최종 인가를 판단한다.
...
flowchart LR
    ...
    subgraph Edge["접근 경계"]
        Gateway["Istio IngressGateway<br/>JWT 검증<br/>X-User-* 헤더 생성"]
    end
    ...
    subgraph ServiceRuntime["서비스 애플리케이션 런타임"]
        IdentityHeaders["X-User-* headers<br/>user_id/role/token_id"]
        Principal["Principal/AuthContext (VO)<br/>서비스 내부 요청 주체"]
    end
    ...
    AuthUserLink -->|"user_id 기준"| RoleGrant

    User --> UserProfile
    User --> UserStatus

    Gateway --> IdentityHeaders
    IdentityHeaders --> Principal
    RoleGrant --> Principal
...
| --- | --- | --- |
| 인증 컨텍스트 | 인증 계정, credential, provider link, auth-user link, session/token, role grant, ACL override, `user_id` 연결 | 사용자 프로필, 도메인 리소스 소유권 |
| 사용자 컨텍스트 | 사용자, 최소 프로필, 사용자 상태, `user_id` 기반 사용자 정보 생성 | 비밀번호, OAuth provider subject, 세션/토큰 |
| 접근 경계 | 외부 JWT 검증, role 1차 정책, `X-User-*` 헤더 생성, 내부 요청 전달 | 도메인 상태의 원천 데이터 |
| 서비스 런타임 | 필요한 API에서 `X-User-*` 헤더를 서비스 내부 Principal/AuthContext로 변환 | JWT 서명 검증, 인증 계정 저장 |
...
| 모델 | 유형 | 설명 | 주요 식별자 |
| --- | --- | --- | --- |
| `AuthAccount` | Aggregate Root | 인증 수단별 인증 계정의 일관성 경계다. 이메일/비밀번호 계정 또는 OAuth/OIDC provider 계정 중 하나를 표현한다. | `auth_account_id` |
...
| `RoleGrant` | Entity | `user_id`에 부여된 role을 표현한다. 인증 수단이 달라도 같은 사용자 권한을 사용한다. | `role_grant_id`, `user_id` |
...
`Principal`은 더 이상 Istio IngressGateway가 전달하는 wire contract가 아니다. 인증/인가가 필요한 API에서 `X-User-*` 헤더를 받은 뒤 애플리케이션 내부에서 만드는 요청 주체 Value Object다.

## 4. 애그리게이트

### 4.1 AuthAccount

`AuthAccount`는 인증 컨텍스트의 Aggregate Root다. 하나의 `AuthAccount`는 하나의 인증 수단을 표현한다.

포함 모델:

- `Credential` 또는 `ProviderLink`
- `AuthUserLink`
- `AuthSession`
...
- 하나의 `user_id`에는 여러 `AuthAccount`가 연결될 수 있다.
- 이메일/비밀번호, Google, Kakao, Apple 같은 인증 수단은 각각 별도의 `AuthAccount`로 저장한다.
- 인증 정보와 사용자 계정 연결 정보는 분리해서 저장한다.
- 이메일 credential은 인증 컨텍스트 안에서 중복되지 않아야 한다.
- `auth_provider + provider_subject` 조합은 전역에서 중복되지 않아야 한다.
- 같은 인증 수단을 같은 `user_id`에 중복 연결하지 않는다.
- 다른 사용자 계정에 이미 연결된 provider subject는 자동으로 연결하지 않는다.

### 4.1.1 RoleGrant

`RoleGrant`는 인증 수단별 계정이 아니라 `user_id` 기준으로 부여한다.

보장해야 하는 규칙:

- 인증 수단이 달라도 같은 `user_id`로 로그인하면 같은 role 정책을 사용한다.
- role 변경은 auth-service의 인가 정책 변경으로 취급한다.
- role 변경 후에는 관련 정책 캐시를 무효화해야 한다.
...
- 접근 경계는 외부 JWT 검증과 `X-User-*` 헤더 생성을 책임진다.
- `auth-service`와 `user-service`는 별도 서비스로 분리한다.
- ACL override와 최신 role 정책은 `auth-service` 내부 책임으로 둔다. Principal/AuthContext 생성은 필요한 API의 애플리케이션 레벨에서 수행한다.
...
flowchart LR
    ...
    subgraph EdgeBoundary["접근 경계"]
        AccessGateway["Istio IngressGateway<br/>JWT 검증<br/>X-User-* 헤더 생성"]
    end

    subgraph AuthBoundary["auth-service"]
        AuthApplication["Auth API/Application<br/>가입/로그인/세션/토큰"]
        AuthDomain["Auth Domain<br/>AuthAccount<br/>Credential<br/>ProviderLink<br/>AuthUserLink<br/>RoleGrant<br/>ACL"]
...
| 서비스 | 책임 | 상태 |
| --- | --- | --- |
| Istio IngressGateway | 외부 JWT 검증, `X-User-*` 헤더 생성, 내부 요청 전달 | 확정 |
| `auth-service` | 이메일 가입, 로그인, OAuth/OIDC 연결, 세션/토큰, 인증 계정-사용자 연결, role/ACL 정책 | 확정 |
...
인증 컨텍스트는 인증 수단, 인증 계정, 인증 계정과 `user_id`의 연결, 세션/토큰, role, ACL override를 책임진다.
...
auth-service는 사용자 상태의 원천 데이터를 소유하지 않는다. auth-service가 인가 판단에 사용하는 차단 정책은 사용자 상태를 직접 읽는 방식이 아니라 role grant와 ACL override로 표현한다.
...
권한 차단이나 예외 허용처럼 인가 결과에 직접 영향을 주는 정책은 `auth-service` 내부 정책 모델의 ACL override로만 표현한다.

사용자 상태 변경이 곧바로 인증 차단을 의미하지 않는다. 특정 사용자의 접근을 막아야 하면 auth-service의 ACL deny 또는 role 회수를 별도 정책 변경으로 기록한다.

### 6.4 MVP 상태 정책 범위

비활성, 탈퇴 같은 사용자 상태 운영 기능은 MVP에서 제외한다.

다만 이후 확장을 위해 사용자 컨텍스트에는 `UserStatus`를 두고, auth-service 내부 정책 모델에는 ACL override를 둔다. 실제 상태 전이 API와 운영자 인가 차단 기능은 후속 API 계약 범위에서 다룬다.

### 6.5 auth-service 내부 정책 범위

auth-service는 role grant와 ACL override를 소유한다.

auth-service는 인증 결과와 role grant, ACL override를 이용해 인가 판단에 필요한 정책 정보를 제공한다. 요청 단위 Principal/AuthContext는 인증/인가가 필요한 API에서 `X-User-*` 헤더로 만든다. 도메인 리소스의 소유권 판단은 해당 리소스를 소유한 서비스가 수행한다.
...
### 6.9 접근 경계와 사용자 식별 헤더 정규화

Istio IngressGateway는 외부 인증 정보를 검증하고 내부 서비스가 사용할 `X-User-*` 헤더로 정규화한다.

내부 서비스는 원본 JWT를 직접 디코딩하지 않는다. 인증 방식이나 토큰 구조가 바뀌어도 인증/인가가 필요한 API는 정규화된 `X-User-*` 계약을 기준으로 동작한다.
...
이메일은 개인정보이므로 `X-User-*` 헤더 계약에 포함하지 않는다. 실명, 닉네임, 프로필 같은 사용자 정보는 사용자 컨텍스트에서 조회하고, 이메일은 auth-service 내부 credential과 감사 기록의 보조 정보로만 제한한다.

접근 경계는 JWT 검증이 끝난 claim으로 `X-User-*` 헤더를 만들어 upstream 서비스에 전달한다.
